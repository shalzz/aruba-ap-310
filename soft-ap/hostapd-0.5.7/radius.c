...
#ifdef __FIPS_BUILD__	
	sha1_vector(4, addr, len, msg->hdr->authenticator);
#else
			    ...
			    size_t secret_len)
{
	...
	len[1] = secret_len;
#ifdef __FIPS_BUILD__
	sha1_vector(2, addr, len, msg->hdr->authenticator);
#else
				   ...
				   const u8 *data, size_t len)
{
	...
	elen[2] = sizeof(l);
#ifdef __FIPS_BUILD__
	sha1_vector(3, addr, elen, msg->hdr->authenticator);
#else
...
#ifdef __FIPS_BUILD__
		sha1_vector(first ? 3 : 2, addr, elen, hash);
#else
...
#ifdef __FIPS_BUILD__
		sha1_vector(first ? 3 : 2, addr, _len, hash);
#else
