...
int hostapd_setup_wpa_psk(struct hostapd_bss_config *conf)
{
	...
	if (ssid->wpa_passphrase != NULL) {
		if (ssid->wpa_psk != NULL) {
			if (meshd_recovery_valid) {
				wpa_printf(MSG_DEBUG, "Preserving Recovery PSK."
					   "Generating PSK from Passphrase");
			} else {
				wpa_printf(MSG_WARNING, "Warning: both WPA PSK and"
					   "passphrase set. Using passphrase.\n");
				free(ssid->wpa_psk);
			}
		}
		if ((wpa_psk = wpa_zalloc(sizeof(struct hostapd_wpa_psk))) == NULL) {
			wpa_printf(MSG_ERROR, "Unable to alloc space for PSK\n");
			return -1;
		}
		if (meshd_recovery_valid) {
			if (ssid->wpa_psk == NULL) {
				wpa_printf(MSG_ERROR, "Recovery is set but wpa_psk NULL\n");
				return -1;
			}
			wpa_psk->next = ssid->wpa_psk; /* make recov PSK 2nd */
			ssid->wpa_psk = wpa_psk;
		} else {
			ssid->wpa_psk = wpa_psk;
		}
...
struct hostapd_config * hostapd_config_read(const char *fname, struct meshd_params *mp)
{
	struct hostapd_config *conf;
	struct hostapd_bss_config *bss;
	FILE *f;
	...
	while (fgets(buf, sizeof(buf), f)) {
		...
		*pos = '\0';
		...
		if (strcmp(buf, "interface") == 0) {
			...
			snprintf(conf->bss[0].iface, sizeof(conf->bss[0].iface),mp->iface);
#else
...
#if defined(AP_PLATFORM) || defined(CONFIG_GRENACHE)
			bss->ssid.ssid_len = strlen(mp->ssid);
			memcpy(bss->ssid.ssid, mp->ssid,
			       bss->ssid.ssid_len);
...
#if defined(AP_PLATFORM) || defined(CONFIG_GRENACHE)
			if (mp->passphrase[0] == '\0') {
				continue;
			}
			int len = strlen(mp->passphrase);
			if (len < 8 || len > 63) {
				...
				bss->ssid.wpa_passphrase = strdup(mp->passphrase);
			}
#else
			int len = strlen(pos);
			if (len < 8 || len > 63) {
			...
			if (bss->ssid.wpa_psk == NULL)
				errors++;
			else if (hexstr2bin(wp, bss->ssid.wpa_psk->psk,
					    PMK_LEN) ||
				 pos[PMK_LEN * 2] != '\0') {
...
#if defined(AP_PLATFORM) || defined(CONFIG_GRENACHE)
	/*
 	* create meshd_recovery_ssid by copying ssid struct from active profile
 	* and changing ssid & psk.
 	*/
	if (conf && meshd_recovery_valid == TRUE) {
		char *wp;
		struct meshd_params *mrp;

		mrp = &meshd_recovery_params;
		if (meshd_recovery_ssid.wpa_psk) {
    			free(meshd_recovery_ssid.wpa_psk);
    			meshd_recovery_ssid.wpa_psk = NULL;
		}
		meshd_recovery_ssid = conf->bss[0].ssid;
		
		meshd_recovery_ssid.ssid_len = strlen(mrp->ssid);
		memcpy(meshd_recovery_ssid.ssid, mrp->ssid,
        		meshd_recovery_ssid.ssid_len);
		meshd_recovery_ssid.ssid[meshd_recovery_ssid.ssid_len] = '\0';
		meshd_recovery_ssid.ssid_set = 1;
		
		meshd_recovery_ssid.wpa_passphrase = NULL;
		
		wp = mrp->hexkey;
		meshd_recovery_ssid.wpa_psk =
    		wpa_zalloc(sizeof(struct hostapd_wpa_psk));
		if (meshd_recovery_ssid.wpa_psk == NULL)
    			errors++;
		else if (hexstr2bin(wp, meshd_recovery_ssid.wpa_psk->psk,
					    PMK_LEN) ||
        		wp[PMK_LEN * 2] != '\0') {
    			printf("Mesh Recovery: Invalid PSK '%s'.\n", wp);
    			errors++;
		} else {
    			meshd_recovery_ssid.wpa_psk->group = 1;
                /*
                 * Add recovery psk at the end of the list of psks in this config.
                 */
                if (bss->ssid.wpa_psk)
                    bss->ssid.wpa_psk->next = meshd_recovery_ssid.wpa_psk;
                else
                    bss->ssid.wpa_psk = meshd_recovery_ssid.wpa_psk;
		}
	}
#endif
