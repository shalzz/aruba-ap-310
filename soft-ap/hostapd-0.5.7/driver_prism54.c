...
static const struct driver_ops prism54_driver_ops;
				 ...
				 void *buf, size_t len)
{
	...
	if ((mlme->state == (int) htonl(DOT11_STATE_ASSOCING)) ||
		...
		if ((drv->hapd->conf->wpa & HOSTAPD_WPA_VERSION_WPA2) &&
		    elems.rsn_ie) {
			wpa_ie = elems.rsn_ie;
			wpa_ie_len = elems.rsn_ie_len;
		} else if ((drv->hapd->conf->wpa & HOSTAPD_WPA_VERSION_WPA) &&
			   elems.wpa_ie) {
			wpa_ie = elems.wpa_ie;
...
static int prism54_init_sockets(struct prism54_driver_data *drv)
{
	...
	if (hapd->conf->bridge[0] != '\0') {
		...
		strcpy(ifr.ifr_name, hapd->conf->bridge);
	} else {
        	strcpy(ifr.ifr_name, drv->iface);
	}
...
static int prism54_driver_init(struct hostapd_data *hapd)
{
...
{
	driver_register(prism54_driver_ops.name, &prism54_driver_ops);
}
