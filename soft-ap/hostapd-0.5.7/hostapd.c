...
static char *debug_timestamp()
{
    ...
    if (gettimeofday(&cur_time, &cur_zone)) {
        /* gettimeofday may return an error */
        strcpy(time_stamp, "unknown time");
        return (time_stamp);
    }   

    tm_time = localtime(&cur_time.tv_sec);
    if (tm_time) {
        int year;

        year = tm_time->tm_year+1900;
        if (year < 2000) {
            /* This could happen if RTC is not present or was never set */
            year = 2000;
        }   
...
static FILE * xf_file = NULL;
void hostapd_write_to_file(char * fmt, ...)
{
	va_list args;
	char buffer[2048 +1];
	int size = 0;
...
struct meshd_params meshd_params;
struct meshd_params meshd_recovery_params;
struct hostapd_ssid meshd_recovery_ssid;
Boolean             meshd_recovery_valid = FALSE;

void hostapd_logger(struct hostapd_data *hapd, const u8 *addr,
		    unsigned int module, int level, const char *fmt, ...)
{
	char *format, module_str[32] = "";
	int maxlen, remlen;
	va_list ap;
...
#define CHECK_REMLEN(ret, remlen) do {                \
            if (ret > remlen) {                                         \
                wpa_printf(MSG_ERROR, "%s:%d truncation: rem:%d, ret:%d", \
                           __func__, __LINE__, remlen, ret);            \
                goto fail;                                              \
            }                                                           \
            remlen -= ret;                                              \
        } while (0);
	...
	case HOSTAPD_MODULE_IEEE80211:
		strncpy(module_str, "IEEE 802.11", sizeof(module_str));
		break;
	case HOSTAPD_MODULE_IEEE8021X:
		strncpy(module_str, "IEEE 802.1X", sizeof(module_str));
		break;
	case HOSTAPD_MODULE_RADIUS:
		strncpy(module_str, "RADIUS", sizeof(module_str));
		break;
	case HOSTAPD_MODULE_WPA:
		strncpy(module_str, "WPA", sizeof(module_str));
		break;
	case HOSTAPD_MODULE_DRIVER:
		strncpy(module_str, "DRIVER", sizeof(module_str));
		break;
	case HOSTAPD_MODULE_IAPP:
		strncpy(module_str, "IAPP", sizeof(module_str));
		break;
	case HOSTAPD_MODULE_MLME:
		strncpy(module_str, "MLME", sizeof(module_str));
		break;
	default:
		break;
	}
	...
	if (hapd && hapd->conf && addr) {
            ret = snprintf(format, remlen, "%s: STA " MACSTR "%s%s: %s",
                           hapd->conf->iface, MAC2STR(addr),
                           module_str[0] != 0 ? " " : "", module_str, fmt);
            CHECK_REMLEN(ret, remlen);
        } else if (hapd && hapd->conf) {
            ret = snprintf(format, remlen, "%s:%s%s %s",
                           hapd->conf->iface, module_str[0] != 0 ? " " : "",
                           module_str, fmt);
            CHECK_REMLEN(ret, remlen);
        } else if (addr) {
            ret = snprintf(format, remlen, "STA " MACSTR "%s%s: %s",
                           MAC2STR(addr), module_str[0] != 0 ? " " : "",
                           module_str, fmt);
            CHECK_REMLEN(ret, remlen);
	} else {
            ret = snprintf(format, remlen, "%s%s%s",
                           module_str, module_str[0] != 0 ? ": " : "", fmt);
            CHECK_REMLEN(ret, remlen);
        }
	if ((conf_stdout & module) && level >= conf_stdout_level) {
		/* wpa_debug_print_timestamp(); */
#if defined(AP_PLATFORM) || defined(CONFIG_GRENACHE)
		wpa_printf(level, "%s", format);
#else
		va_start(ap, fmt);
		vprintf(format, ap);
		va_end(ap);
...
void handle_reload(int sig, void *eloop_ctx, void *signal_ctx)
{
	...
	for (i = 0; i < hapds->count; i++) {
		...
		newconf = hostapd_config_read(hapds->iface[i]->config_fname, &meshd_params);
		if (newconf == NULL) {
...
#ifndef __OEM_siemens__
extern int mmRetrieveStats(PAPI_Info *pi, char **resp_buf, int *resp_len);
#endif
static void handle_dump_memory(int sig, void *eloop_ctx, void *signal_ctx)
{
#ifndef __OEM_siemens__
	char *resp_buf;
	int resp_len;
	if (xf_file) {
		fclose(xf_file);
		unlink("/tmp/hostapd_memory");
		xf_file = fopen("/tmp/hostapd_memory", "w");
	} else {
		xf_file = fopen("/tmp/hostapd_memory", "w");
	}
	if (!xf_file)
		return;
	if (mmRetrieveStats(NULL, &resp_buf, &resp_len)) {
		hostapd_write_to_file("mmRetrieveStats failed to get memory info\n");
		return;
	}
	if (!resp_buf || resp_len == 0) {
		hostapd_write_to_file("resp_buf is NULL or resp_len is 0\n");
		return;
	}

	hostapd_write_to_file("%s \n", debug_timestamp());
	mm_print_info_by_fun(resp_buf, resp_len, hostapd_write_to_file);
	free(resp_buf);
#endif
}
...
void
hostapd_set_unique_psk (struct hostapd_data *hapd, const u8 *peer, const u8 *key, int keylen)
{
    int i;
    struct hostapd_wpa_psk *psk, *newpsk;

    if ((newpsk = malloc(sizeof(struct hostapd_wpa_psk))) == NULL) {
        return;
    }
    newpsk->group = 0;
    memcpy(newpsk->psk, key, PMK_LEN);
    memcpy(newpsk->addr, peer, ETH_ALEN);
    newpsk->next = hapd->conf->ssid.wpa_psk;
    hapd->conf->ssid.wpa_psk = newpsk;

    for (i = 0, psk = hapd->conf->ssid.wpa_psk; psk != NULL; psk = psk->next, i++) {
        if (psk->group) {
            wpa_hexdump_ascii(MSG_DEBUG, "group key", psk->psk, PMK_LEN);
        } else {
            wpa_hexdump_ascii(MSG_DEBUG, "non-group key! for: ", psk->addr, ETH_ALEN);
            wpa_hexdump_ascii(MSG_DEBUG, "non-group key", psk->psk, PMK_LEN);
        }
    }
}
 ...
 */
static struct hostapd_iface * hostapd_init(const char *config_file, struct meshd_params *mp)
{
	...
	if (hapd_iface->config_fname == NULL)
		goto fail;
	conf = hostapd_config_read(hapd_iface->config_fname, mp);
...
int main(int argc, char *argv[])
{
	...
	const char *pid_file = HOSTAPD_PID_FILE;
	int restarted;
#ifdef CONFIG_GRENACHE
    cpu_set_t grenache_ap_cpu_set;
#endif
...
#ifdef CONFIG_GRENACHE
    CPU_ZERO(&grenache_ap_cpu_set);
    CPU_SET(GRENACHE_AP_CPU, &grenache_ap_cpu_set);
    if (sched_setaffinity(0, sizeof(grenache_ap_cpu_set), &grenache_ap_cpu_set) < 0) {
    }
#endif

#if defined(AP_PLATFORM) || defined(CONFIG_GRENACHE)
	if ((meshd_ctrl_sock = create_meshd_sync_sock()) < 0) {
		wpa_printf(MSG_ERROR, "Can't create meshd sync socket");
		exit(1);
	}
	restarted = is_pid_file_present(pid_file);
	write_pid_file(pid_file);
	/* If re-started, contact meshd and get mesh
	 * iface, ssid, psk, passphrase values
	 */
	if (restarted) {
		if (request_meshd_configs(meshd_ctrl_sock, &meshd_params,
                    &meshd_recovery_params) < 0) {
			wpa_printf(MSG_ERROR, "request_meshd_configs failed. Exiting\n");
			exit(1);
		}
	} else {
		if (wait_for_meshd(meshd_ctrl_sock, &meshd_params,
                    &meshd_recovery_params) < 0) {
			wpa_printf(MSG_ERROR, "wait_for_meshd failed. Exiting\n");
			exit(1);
		}
	}
    memset(&meshd_recovery_ssid, 0, sizeof(meshd_recovery_ssid));
#endif
	...
	eloop_register_signal(SIGUSR2, handle_dump_memory, NULL);

	hostapd_meshd_register_cb(meshd_ctrl_sock);
	...
	for (i = 0; i < interfaces.count; i++) {
		...
		interfaces.iface[i] = hostapd_init(argv[optind + i], &meshd_params);
