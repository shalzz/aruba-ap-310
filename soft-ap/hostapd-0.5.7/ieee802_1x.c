				 ...
				 struct sta_info *sta)
{
	...
	if (hapd->conf && hapd->conf->eap_server) {
				  ...
				  u8 *key_data, size_t key_len)
{
	...
	if (hapd->conf && hapd->conf->eapol_key_index_workaround) {
	...
	printf("ieee802_1x_tx_key_one(): Error: RC4 not supported in FIPS build") ;	
	return ;
#else
			...
			size_t len)
{
	...
	if (!hapd->conf || (!hapd->conf->ieee802_1x && !hapd->conf->wpa))
		return;
					  ...
					  struct radius_msg *msg)
{
	...
	if (!hapd->conf || !hapd->conf->radius || !hapd->conf->radius->acct_server || hapd->radius == NULL ||
	    sm == NULL)
		return;
