...
void wpa_debug_print_timestamp(void)
{
	...
	os_get_time(&tv);
#ifdef CONFIG_DEBUG_FILE
	wpa_debug_get_out_file();
	if (out_file) {
#if !defined(AP_PLATFORM) && !defined(CONFIG_GRENACHE)
		fprintf(out_file, "%ld.%06u: ", (long) tv.sec,
			(unsigned int) tv.usec);
#else
                {   // User friendly time format (Bug: 26893).
                    time_t now = tv.sec;
                    struct tm *tm = localtime(&now);
                    char timestr[32];
                    strftime(timestr, sizeof(timestr), "%F %T", tm);
                    fprintf(out_file, "[%d]%s.%03lu: ", pid, timestr, tv.usec/1000);
                }
#endif /* CONFIG_MERLOT */
	} else
#endif /* CONFIG_DEBUG_FILE */
...
#else
		printf("[%d]%ld.%06u: ", pid, (long) tv.sec, (unsigned int) tv.usec);
#endif /* CONFIG_MERLOT */
}
...
int wpa_debug_get_out_file(void) 
{
#ifdef CONFIG_DEBUG_FILE
    static int wpa_cur_log_file = 0;
    char fname[64];

    if (out_file == NULL) {
        os_snprintf(fname, sizeof(fname), HOSTAPD_DEBUG_LOG_FILE);
        out_file = fopen(fname, "a");
    }

    if (out_file) {
        if (ftell(out_file) >= HOSTAPD_MAX_DEBUG_LOG_FILE_SIZE) {
            fclose(out_file);
            wpa_cur_log_file++;
            if (wpa_cur_log_file == HOSTAPD_MAX_DEBUG_LOG_FILES) {
                wpa_cur_log_file = 0;
                os_snprintf(fname, sizeof(fname), HOSTAPD_DEBUG_LOG_FILE);
            } else {
                os_snprintf(fname, sizeof(fname), "%s.%1u",
                        HOSTAPD_DEBUG_LOG_FILE, wpa_cur_log_file);
            }
...
int wpa_debug_open_file(void)
{
	...
	os_snprintf(fname, sizeof(fname), "\\Temp\\wpa_supplicant-log-%d.txt",
		    count++);
#else /* _WIN32 */
	os_snprintf(fname, sizeof(fname), "/tmp/hostapd-log.txt");
#endif /* _WIN32 */
	out_file = fopen(fname, "a+");
	return out_file == NULL ? -1 : 0;
#else /* CONFIG_DEBUG_FILE */
...
void wpa_msg(void *ctx, int level, char *fmt, ...)
{
	...
	va_end(ap);
        if (len > buflen) {
            wpa_printf(MSG_ERROR, "%s: vsnprintf() truncated. max:%d, return:%d\n",
                       __func__, buflen, len);
        } else {
