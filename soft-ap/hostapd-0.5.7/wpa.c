...
#include "rc4.h"
#ifdef __NO_OPENSSL__
#include <crypto-util/aes_wrap.h>
#else
#include "aes_wrap.h"
#endif
#include "crypto.h"
			     ...
			     const char *fmt, ...)
{
	...
	va_start(ap, fmt);
	ret = vsnprintf(format, maxlen, fmt, ap);
	va_end(ap);
        if (ret > maxlen) {
            wpa_printf(MSG_ERROR, "%s:%d. Truncation. maxlen:%d, return:%d", 
                       __func__, __LINE__, maxlen, ret);
        } else {
...
static void wpa_sta_disconnect(struct wpa_authenticator *wpa_auth,
			       const u8 *addr)
{
    wpa_printf(MSG_DEBUG, "%s: WPA authenticator requests disconnect: STA" MACSTR,
...
#ifdef __NO_OPENSSL__
 //function is called from cmn/crypto-util
	sha1_prf(rkey, sizeof(rkey), "Init Counter", strlen("Init Counter"), buf, sizeof(buf),
...
#ifdef __NO_OPENSSL__
 //function is called from cmn/crypto-util
 sha1_prf(smk, WPA_PMK_LEN, "SMK Derivation", strlen("SMK Derivation"), buf, sizeof(buf),
		 smk, WPA_PMK_LEN);
#else
	...
	case PAIRWISE_2:
		...
		if (sm->wpa_ie == NULL ||
			...
			wpa_sta_disconnect(wpa_auth, sm->addr);
                        /*
                        if (meshd_ctrl_sock > 0) {
                            inform_meshd_sta_auth_status(meshd_ctrl_sock, sm->addr, MESHD_WPA_HOSTAPD_STA_AUTH_FAIL);
                            } */
			return;
		}
		break;
...
#ifdef __NO_OPENSSL__
 //function is called from cmn/crypto-util
	sha1_prf(pmk, WPA_PMK_LEN, "Pairwise key expansion", strlen("Pairwise key expansion"),
...
#ifdef __NO_OPENSSL__
 //function is called from cmn/crypto-util
	sha1_prf(gmk, WPA_GMK_LEN, "Group key expansion", strlen("Group key expansion"), 
...
)
{
	SM_ENTRY_MA(WPA_PTK, DISCONNECT, wpa_ptk);
        if (meshd_ctrl_sock > 0 && (sm->sNonceFailures)) { // signal fail for error other than timeout.
            inform_meshd_sta_auth_status(meshd_ctrl_sock, sm->addr, MESHD_WPA_HOSTAPD_STA_AUTH_FAIL);
        } 
	sm->Disconnect = FALSE;
        sm->sNonceFailures = 0;
...
SM_STATE(WPA_PTK, PTKCALCNEGOTIATING)
{
	...
	for (;;) {
		if (sm->wpa_key_mgmt == WPA_KEY_MGMT_PSK) {
			...
			if (pmk == NULL) {
                                sm->sNonceFailures++;
                                wpa_printf(MSG_DEBUG, "%s: pmk NULL!, sNonceFailures: %d", __func__, sm->sNonceFailures);
				break;
                        }
		} else
			pmk = sm->PMK;
		...
		if (wpa_verify_key_mic(&PTK, sm->last_rx_eapol_key,
			...
			ok = 1;
                        wpa_printf(MSG_DEBUG, "%s: wpa_verify_key_mic returned OK.", __func__);
			break;
		}

		if (sm->wpa_key_mgmt != WPA_KEY_MGMT_PSK) {
                        wpa_printf(MSG_DEBUG, "%s: wpa_key_mgmt != WPA_KEY_MGMT_PSK", __func__);
			break;
                }
	}
...
SM_STATE(WPA_PTK, PTKINITDONE)
{
	...
	if (sm->Pair) {
		...
		if (sm->wpa_key_mgmt == WPA_KEY_MGMT_PSK) {
			...
			if (meshd_ctrl_sock > 0) {
                          wpa_printf(MSG_ERROR, "WPA: Send Success %02X:%02X:%02X\n",sm->addr[0],sm->addr[1],sm->addr[2]);

                            inform_meshd_sta_auth_status(meshd_ctrl_sock, sm->addr, MESHD_WPA_HOSTAPD_STA_AUTH_SUCCESS);
			}
#endif
		}
	}
	...
	case WPA_PTK_INITPMK:
		if (wpa_auth_get_eapol(sm->wpa_auth, sm->addr,
			...
			wpa_auth->dot11RSNA4WayHandshakeFailures++;
                        sm->Disconnect = TRUE; // To inform meshd.
			SM_ENTER(WPA_PTK, DISCONNECT);
		}
		break;
