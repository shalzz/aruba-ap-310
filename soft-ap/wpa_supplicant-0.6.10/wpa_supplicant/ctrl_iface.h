...
void notify_sapd_ready(struct ctrl_iface_global_priv *g);
void notify_sapd_eap_success(struct wpa_supplicant *wpa_s);
int request_meshd_configs(struct ctrl_iface_global_priv *gp);
void inform_meshd_status(struct wpa_supplicant *ctx, unsigned char cmdid);

#ifdef __FAT_AP__
int request_wifi_uplink_configs(struct ctrl_iface_global_priv *gp);
void inform_wifi_uplink_status(struct wpa_supplicant *ctx, unsigned int cmdid, unsigned int reason_id);
void inform_ap1x_status(struct wpa_supplicant *ctx, unsigned int cmdid);
#endif
