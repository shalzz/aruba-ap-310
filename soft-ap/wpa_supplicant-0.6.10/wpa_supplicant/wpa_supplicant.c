...
#ifdef __FAT_AP__
int wpa_sapd_socket_init(struct wpa_supplicant * wpa_s);
void wpa_sapd_socket_deinit(struct wpa_supplicant * wpa_s);
int wpa_sapd_socket_tx_radio_used(struct wpa_supplicant * wpa_s, int up);
int wpa_sapd_socket_tx_radio_channel(struct wpa_supplicant * wpa_s, int freq);
#endif
...
#ifdef __FAT_AP__
    if(!IS_WIFI_UPLINK(wpa_s))
        wpa_msg(wpa_s, MSG_ERROR, "Need to tell meshd to re-associate");
    else
...
void wpa_clear_keys(struct wpa_supplicant *wpa_s, const u8 *addr)
{
	...
	if (wpa_s->keys_cleared) {
		...
		wpa_printf(MSG_INFO, "No keys have been configured - "
			   "skip key clearing");
#ifdef __FAT_AP__
    if(!IS_WIFI_UPLINK(wpa_s))
		return;
...
void wpa_supplicant_set_state(struct wpa_supplicant *wpa_s, wpa_states state)
{
	...
	if (state == WPA_COMPLETED && wpa_s->new_connection) {
        ...
        {
            case WPA_COMPLETED:
                if(WPA_COMPLETED != wpa_s->wpa_state)
                    inform_wifi_uplink_status(wpa_s, WIFI_UPLINK_SUPPLICANT_SUCCESS, WIFI_UPLINK_REASON_OK);
                break;

            case WPA_DISCONNECTED:
                if(WPA_COMPLETED == wpa_s->wpa_state)
                {
                    inform_wifi_uplink_status(wpa_s, WIFI_UPLINK_SUPPLICANT_FAIL, WIFI_UPLINK_REASON_DEAUTH);
                }
                else if(WPA_ASSOCIATED == wpa_s->wpa_state ||
                        WPA_4WAY_HANDSHAKE == wpa_s->wpa_state ||
                        WPA_GROUP_HANDSHAKE == wpa_s->wpa_state)
                {
                    inform_wifi_uplink_status(wpa_s, WIFI_UPLINK_SUPPLICANT_FAIL, WIFI_UPLINK_REASON_EAPOL_HANDSHAKE);
                }
                else
                {
                    ;
...
/*        if (IS_WIFI_UPLINK(wpa_s)) {
	...
	if (ssid->key_mgmt & WPA_KEY_MGMT_PSK) {
            /*
             * if a unique PSK hasn't already been set then use the one for this SSID
             */
            if (!wpa_s->unique_psk_set) {
...
#ifdef __FAT_AP__
/*
 * send notification to sapd using PAPI
 */
void ws_send_notification_to_sapd(int notif_type, const char *caller, int lnnum)
{
#if 0
    char *msgbuf;
    __u32 msgsize;
    ...
    msgsize = sizeof(msgHndl_Hdr);
    if (notif_type == MESH_MSG_CHANNEL_NOTIF) {
        msgsize += SXDR_SIZE_U16;
    }

    if (!(msgbuf = PAPI_Alloc(meshd_papi_id, msgsize))) {
        return;
    }

    msgHdr = (msgHndl_Hdr *) msgbuf;
    msgHdr->DestIpAddr = htonl(INADDR_LOOPBACK);
    msgHdr->DestPortNum = htons(SAPM_CLIENT);

    if (notif_type == MESH_MSG_CHANNEL_NOTIF) {
        __u16 nchan = NCFG_NCHANNEL_CREATE(m_cfg.channel,m_cfg.ht.ht_extoffset);
        if ((m_cfg.mode != MESH_POINT) || !m_cfg.parent) {
            return;
        }
        msgHdr->MessageCode = htons(MESH_MSG_CHANNEL_NOTIF);
        bp = msgbuf + sizeof(msgHndl_Hdr);
        bl += sxdr_write_u16(nchan, &bp[bl]);
        MESHD_DEBUG_LOG("%s: channel-change %d", __func__, nchan);
    }
    else 
    { 
        PAPI_Free(meshd_papi_id, msgbuf);
        MESHD_DEBUG_LOG("Notification to send to sapd not recognized");
        return;
    }

    args.timeout = 5000;    /* 5 sec */
    //args.retryCount = (MESHD_SAPD_PENDING_MAX * 1000)/args.timeout - 1;
    args.retryCount = (120 * 1000)/args.timeout - 1;
    args.PAPI_SendStatusCallback = meshd_papi_snd_cb;

    if (PAPI_Send(meshd_papi_id, NULL, msgbuf, msgsize,
                NON_BLOCKING_W_ACK, &args) == (char *) -1) {
        MESHD_COMMUNICATION_ERROR_SAPD;
    }
#endif
    return;
}
...
int
ws_sysctl_write_param(char *nodename, char *value)
{
#if 0
    int fd, rc;
    ...
    rc = write(fd, value, strlen(value) + 1); // must also write NULL
    close(fd);
    return rc;
#else
    return 0;
			      ...
			      struct wpa_scan_res *bss, struct wpa_ssid *ssid)
{
...
#ifdef __FAT_AP__
        else /* ring debug, notify sapd channel */
        {
#if 0
            char node[128], value[128];

            snprintf(node, sizeof(node), PROCSYSDEV "%s" WIFI_DEVNUM_FMT "/"
                WIF_SYSCTL_NCHANNEL, WIFI_DEVNAME, m->phy_id);
            snprintf(value, sizeof(value), "%d", NCFG_NCHANNEL_CREATE(channel, m->ht.ht_extoffset));
            rc = ws_sysctl_write_param(node, value);

            ws_send_notification_to_sapd(MESH_MSG_CHANNEL_NOTIF, __func__, __LINE__);
#endif
        }
#endif
	...
	wpa_printf(MSG_INFO, "Calling w_s_initiate_eapol");
	wpa_supplicant_initiate_eapol(wpa_s);
#ifdef __FAT_AP__
        if(wpa_sapd_socket_tx_radio_channel(wpa_s, chan_freq))
            wpa_sapd_socket_tx_radio_channel(wpa_s, chan_freq);
#endif
}
...
/*        if(IS_WIFI_UPLINK(wpa_s))
        {
            const char * wifi_sta_event_tag = "AI-WIFI-UPLINK:Titan";
            u8 * event_buf = (u8 *)(buf + sizeof(struct ieee80211_frame));

            src_addr = wpa_s->bssid;

            if (0==memcmp(event_buf, wifi_sta_event_tag, 20))
            {
                extern void wpa_driver_wext_event_receive(void *, u8 *, int);
                int left  = len - sizeof(struct ieee80211_frame) - 20;

                if (left > 0)
                {
                     wpa_driver_wext_event_receive(ctx, &(event_buf[20]), left);
                }
                return;
             }
          }*/
...
static void
aruba_vap_to_wif_name(const char *ifname, char *wif)
{
	int x, r;

	x = strlen(VAP_DEVNAME);
	r = ifname[x] - '0';
	if (r >= 0 && r < MAX_WIFS) {
		sprintf(wif, WIFI_DEVNAME WIFI_DEVNUM_FMT, r); 
	} else {
		sprintf(wif, "wifi0");
	}
}
...
int wpa_supplicant_driver_init(struct wpa_supplicant *wpa_s)
{
	...
	if (wpa_s->driver->send_eapol) {
		...
		if (addr)
			os_memcpy(wpa_s->own_addr, addr, ETH_ALEN);
                ...
                if (wpa_s->driver->is_wired_iface == 0) {
                        aruba_vap_to_wif_name(wpa_s->ifname, wpa_s->wif);
                        wifname = wpa_s->wif;
                        protocol = ETH_P_IEEE80211;
                } else
                        l2_hdr = 0;
                
		wpa_s->l2 = l2_packet_init(wpa_s->ifname, wifname,
					   wpa_drv_get_mac_addr(wpa_s),
					   protocol,
					   wpa_supplicant_rx_eapol, wpa_s, l2_hdr);
		if (wpa_s->l2 == NULL)
			return -1;
	}
	...
	if (wpa_s->bridge_ifname[0]) {
		...
		wpa_s->l2_br = l2_packet_init(wpa_s->bridge_ifname, wpa_s->bridge_ifname,
					      wpa_s->own_addr,
					      ETH_P_EAPOL,
				     ...
				     struct wpa_interface *iface)
{
	...
	if (iface->confname) {
		...
		wpa_s->conf = wpa_config_read(wpa_s->confname, iface);
		if (wpa_s->conf == NULL) {
						 ...
						 struct wpa_interface *iface)
{
	...
	wpa_s->global = global;
        ...
        if (wpa_supplicant_init_iface(wpa_s, iface) != 0) {
        ...
        if ((wpa_ss = wpa_s->conf->ssid) == NULL) {
		wpa_printf(MSG_DEBUG, "w_s_init_iface produced NULL ssid conf %s",
			   iface->ifname);
		wpa_supplicant_deinit_iface(wpa_s);
		os_free(wpa_s);
		return NULL;
...
#ifndef __FAT_AP__
        if (iface->identity) {
                wpa_ss->eap.identity = strdup(iface->identity);
                wpa_ss->eap.identity_len = strlen(iface->identity);
        }
        if (iface->password) {
                wpa_ss->eap.password = strdup(iface->password);
                wpa_ss->eap.password_len = strlen(wpa_ss->eap.password);
        }
#endif
	if (wpa_supplicant_init_iface2(wpa_s)) {
		wpa_printf(MSG_DEBUG, "Failed to add interface %s",
			   iface->ifname);
		wpa_supplicant_deinit_iface(wpa_s);
		os_free(wpa_s);
...
#ifdef __FAT_AP__
        if(0 == strcmp(iface->drvmode, "wifi-uplink"))
        {
            wpa_printf(MSG_INFO, "%s   %d  wifi-uplink", __func__, __LINE__);
            wpa_s->drvmode = 1;
            if(wpa_sapd_socket_init(wpa_s)) {
                wpa_printf(MSG_ERROR, "%s   %d  set up wpa-sapd socket failed!", __func__, __LINE__);
                wpas_dbus_unregister_iface(wpa_s);
               wpa_supplicant_deinit_iface(wpa_s);
               os_free(wpa_s);
               return NULL;
            ...
            if(wpa_sapd_socket_tx_radio_used(wpa_s, 1))
                (void)wpa_sapd_socket_tx_radio_used(wpa_s, 1);   /* try agian */
        }
        else
        {
            wpa_printf(MSG_INFO, "%s   %d  mesh or ap1x", __func__, __LINE__);
            wpa_s->drvmode = 0;
        }
#endif
...
#ifdef __FAT_AP__
	if (wpa_sapd_socket_tx_radio_used(wpa_s, 0))
            (void)wpa_sapd_socket_tx_radio_used(wpa_s, 0);   /* try agian */
#endif
...
struct wpa_global * wpa_supplicant_init(struct wpa_params *params)
{
	...
	if (params->pid_file)
        ...
        global->params.notify_sapd_on_eapol_success = params->notify_sapd_on_eapol_success;
	...
	if (global->params.wait_for_interface && global->params.daemonize &&
		...
		os_open_pid_file(global->params.pid_file);
	}
	return global;
}
...
int wpa_sapd_socket_init(struct wpa_supplicant * wpa_s) 
{
    ...
    wpa_s->sapd_socket = socket(PF_UNIX, SOCK_DGRAM, 0);
    if(wpa_s->sapd_socket < 0)
    {
        wpa_printf(MSG_ERROR, "%s  %d  open sapd socket error!", __func__, __LINE__);
        return -1;
    }

    addr = (struct sockaddr_un *)malloc(sizeof(* addr)); 
    if(!addr)
    {
        wpa_printf(MSG_ERROR, "%s  %d  malloc error!", __func__, __LINE__);
        return -1;
    }

    memset(addr, 0, sizeof(* addr));
    addr->sun_family = AF_UNIX;
    snprintf(addr->sun_path, sizeof(addr->sun_path), "/tmp/wpa_sapd_sock");
    wpa_s->sapd_addr = (void *)addr;
...
void wpa_sapd_socket_deinit(struct wpa_supplicant * wpa_s)
{
    free(wpa_s->sapd_addr);
    wpa_s->sapd_addr = NULL;
    close(wpa_s->sapd_socket);
}
...
int wpa_sapd_socket_tx_radio_used(struct wpa_supplicant * wpa_s, int up)
{
    ...
    x = strlen(VAP_DEVNAME);
    rid = wpa_s->ifname[x] - '0';
    if (rid >= 0 && rid < MAX_WIFS) 
       ;
    else {
       wpa_printf(MSG_ERROR, "%s  %d   Invlalid aruba radio number\n", __func__, __LINE__);
       return -1;
    }
    
    if(1 == up)
        msg_len = snprintf(buf, 256, "%d|UP|0", rid);
    else if(0 == up)
        msg_len = snprintf(buf, 256, "%d|DW|0", rid);
    else
    {
       wpa_printf(MSG_ERROR, "%s  %d   Invlalid aruba radio status!\n", __func__, __LINE__);
       return -1;
    }

    addr = wpa_s->sapd_addr; 
    if(!addr)
    {
        wpa_printf(MSG_ERROR, "%s  %d   sapd socket address not initialized.\n", __func__, __LINE__);
        return -1;
    }

    ret  = sendto(wpa_s->sapd_socket, buf, msg_len, 0, (struct sockaddr *)addr, sizeof(* addr));

    if(ret < 0)
    {
        wpa_printf(MSG_ERROR, "%s  %d   send msg to sapd error!\n", __func__, __LINE__);
        return -1;
    }
...
int wpa_sapd_socket_tx_radio_channel(struct wpa_supplicant * wpa_s, int freq)
{
    ...
    msg_len = snprintf(buf, 256, "%d|CH|%d", rid, freq);
