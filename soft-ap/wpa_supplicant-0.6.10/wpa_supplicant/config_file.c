                                                 ...
                                                 struct wpa_interface *iface)
{
	...
	while (wpa_config_get_line(buf, sizeof(buf), f, line, &pos)) {
		...
		*pos2++ = '\0';
		if (iface != NULL) {
			if (strcmp(pos, "ssid") == 0) {
				pos2 = iface->essid;
			} else if (strcmp(pos, "psk") == 0) {
				if (iface->hexkey != NULL)
					pos2 = iface->hexkey;
				else if (iface->passphrase != NULL)
					pos2 = iface->passphrase; /* must be in quotes */
			}
		}
		if (*pos2 == '"') {
...
struct wpa_config * wpa_config_read(const char *name, struct wpa_interface *iface)
{
	FILE *f;
	char buf[256], *pos;
	int errors = 0, line = 0;
	...
	while (wpa_config_get_line(buf, sizeof(buf), f, &line, &pos)) {
		if (os_strcmp(pos, "network={") == 0) {
			ssid = wpa_config_read_network(f, &line, id++, iface);
			if (ssid == NULL) {
				wpa_printf(MSG_ERROR, "Line %d: failed to "
