...
int wpa_supplicant_do_associate_cmd(struct wpa_supplicant *wpa_s, char *cmd);
int wpa_supplicant_do_disassociate_cmd(struct wpa_supplicant *wpa_s);
...
static int
wpa_supplicant_ctrl_iface_peer_key (struct wpa_supplicant *wpa_s, char *cmd)
{
    ...
    wpa_sm_set_pmk(wpa_s->wpa, ptr, PMK_LEN);
    wpa_s->unique_psk_set = 1;
					 ...
					 char *buf, size_t *resp_len)
{
	...
	if (os_strncmp(buf, WPA_CTRL_RSP, os_strlen(WPA_CTRL_RSP)) == 0 ||
		...
		wpa_hexdump_ascii_key(MSG_INFO, "RX ctrl_iface",
				      (const u8 *) buf, os_strlen(buf));
	} else {
		wpa_hexdump_ascii(MSG_INFO, "RX ctrl_iface",
				  (const u8 *) buf, os_strlen(buf));
	}
	...
	} else if (os_strncmp(buf, ASSOCIATE_INFORM_CMD, 11) == 0) {
		wpa_supplicant_do_associate_cmd(wpa_s, buf+11);
	} else if (os_strcmp(buf, DISASSOCIATE_INFORM_CMD) == 0) {
		wpa_supplicant_do_disassociate_cmd(wpa_s);
#endif
	...
	} else if (os_strncmp(buf, WPA_CTRL_RSP, os_strlen(WPA_CTRL_RSP)) == 0)
	{
        ...
        } else if (os_strncmp(buf, "PEER_KEY ", 9) == 0) {
            if (wpa_supplicant_ctrl_iface_peer_key(wpa_s, buf + 9)) {
                reply_len = -1;
            }
	} else {
					   ...
					   char *cmd)
{
	...
	strlcpy(safe_cmd, cmd, sizeof(safe_cmd));
	if ((pos = os_strchr(safe_cmd, '\t')) && 
	    (pos = os_strchr(pos+1, '\t')) &&
	    (pos = os_strchr(pos+1, '\t')) &&
	    (pos = os_strchr(pos+1, '\t'))) {
		*pos++ = '\0';
		if ((p = os_strchr(pos, '\t'))) {
			if ((p-pos) != 1 )
				strlcat(safe_cmd, " ***", sizeof(safe_cmd));
			else
				strlcat(safe_cmd, " -", sizeof(safe_cmd));
			if (!strcmp(p+1, "-"))
				strlcat(safe_cmd, " -", sizeof(safe_cmd));
		...
		wpa_printf(MSG_INFO, "CTRL_IFACE GLOBAL INTERFACE_ADD '%s'", safe_cmd);
	} else {
                wpa_printf(MSG_INFO, "CTRL_IFACE GLOBAL INTERFACE_ADD '%s'", cmd);
        }
	/*
	 * <ifname>TAB<confname>TAB<driver>TAB<essid>TAB<hexkey>
	 * TAB<passphrase>
	 */
	...
	do {
		...
		if (pos)
			*pos++ = '\0';
		if (iface.hexkey[0] == '\0' || iface.hexkey[0] == '-')
			iface.hexkey = NULL;
		if (pos == NULL)
			break;

		iface.passphrase = pos;
		pos = os_strchr(pos, '\t');
		if (pos)
			*pos++ = '\0';
		if (iface.passphrase[0] == '\0' || iface.passphrase[0] == '-')
...
static int wpa_supplicant_global_wired_iface_add(struct wpa_global *global,
                                                 char *cmd)
{
	struct wpa_interface iface;
	char *pos;
	...
	/*
	 * <ifname>TAB<confname>TAB<driver>TAB<ctrl_interface>TAB<peap-identity>
	 * TAB<peap-passwd>
	 */
#if !defined(AP_PLATFORM)
	wpa_printf(MSG_DEBUG, "CTRL_IFACE GLOBAL WIRED_INTERFACE_ADD '%s'", cmd);
#endif
	...
	do {
		...
		if (pos)
			*pos++ = '\0';
		if (iface.password[0] == '\0')
			iface.password = NULL;
		if (pos == NULL)
			break;
#ifdef __FAT_AP__
                os_snprintf(iface.drvmode, sizeof(iface.drvmode), "%s", pos);
                pos = os_strchr(iface.drvmode, '\t');
                if (pos)
                     *pos++ = '\0';
                if (pos == NULL)
                     break;
						...
						char *buf, size_t *resp_len)
{
	...
	wpa_hexdump_ascii(MSG_DEBUG, "RX global ctrl_iface",
			  (const u8 *) buf, os_strlen(buf));
#endif
        /* NOTE: For wired .1x see WIRED_INTERFACE_ADD below */
	if (os_strncmp(buf, "INTERFACE_ADD ", 14) == 0) {
		wpa_supplicant_global_iface_add(global, buf + 14);
		reply_len = 0;
	} else if (os_strncmp(buf, "INTERFACE_REMOVE ", 17) == 0) {
		wpa_supplicant_global_iface_remove(global, buf + 17);
		reply_len = 0;
	} else {
                reply = os_malloc(reply_size);
                if (reply == NULL) {
                ...
                if (os_strcmp(buf, "PING") == 0) {
                        os_memcpy(reply, "PONG\n", 5);
                        reply_len = 5;
                } else if (os_strncmp(buf, "WIRED_INTERFACE_ADD ", 20) == 0) {
                        if (wpa_supplicant_global_wired_iface_add(global, buf + 20))
                                reply_len = -1;
                } else if (os_strcmp(buf, "INTERFACE_LIST") == 0) {
                        reply_len = wpa_supplicant_global_iface_list(
