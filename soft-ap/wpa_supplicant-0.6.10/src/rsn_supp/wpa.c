					  ...
					  u16 ver)
{
	...
	wpa_printf(MSG_INFO, "WPA: RX message 1 of 4-Way Handshake from "
					  ...
					  u16 ver)
{
	...
	wpa_sm_set_state(sm, WPA_4WAY_HANDSHAKE);
	wpa_printf(MSG_INFO, "WPA: RX message 3 of 4-Way Handshake from "
	...
	if (ieee80211w_set_keys(sm, &ie) < 0) {
        ...
        inform_meshd_status(((struct wpa_supplicant *)sm->ctx->ctx), MESHD_WPA_SUPPLICANT_SUCCESS);
	sm->msg_3_of_4_ok = 1;
        return;

failed:
        inform_meshd_status(((struct wpa_supplicant *)sm->ctx->ctx), MESHD_WPA_SUPPLICANT_FAIL);
}
...
static void wpa_eapol_key_dump(const struct wpa_eapol_key *key)
{
	...
	wpa_printf(MSG_INFO, "  EAPOL-Key type=%d", key->type);
	wpa_printf(MSG_INFO, "  key_info 0x%x (ver=%d keyidx=%d rsvd=%d %s"
		   "%s%s%s%s%s%s%s)",
	...
	wpa_hexdump(MSG_INFO, "  replay_counter",
		    key->replay_counter, WPA_REPLAY_COUNTER_LEN);
	wpa_hexdump(MSG_INFO, "  key_nonce", key->key_nonce, WPA_NONCE_LEN);
	wpa_hexdump(MSG_INFO, "  key_iv", key->key_iv, 16);
		    ...
		    const u8 *buf, size_t len)
{
	...
	if (len < sizeof(*hdr) + sizeof(*key)) {
		wpa_printf(MSG_ERROR, "WPA: EAPOL frame too short to be a WPA "
	...
	wpa_printf(MSG_INFO, "IEEE 802.1X RX: version=%d type=%d length=%lu",
	...
	if (hdr->type != IEEE802_1X_TYPE_EAPOL_KEY) {
		wpa_printf(MSG_ERROR, "WPA: EAPOL frame (type %u) discarded, "
			"not a Key frame", hdr->type);
		ret = 0;
		goto out;
	}
	if (plen > len - sizeof(*hdr) || plen < sizeof(*key)) {
		wpa_printf(MSG_ERROR, "WPA: EAPOL frame payload size %lu "
	...
	{
		wpa_printf(MSG_ERROR, "WPA: EAPOL-Key type (%d) unknown, "
			   "discarded", key->type);
		ret = 0;
		goto out;
	}
...
out:
        if(ret <0 && ((struct wpa_supplicant *)sm->ctx->ctx)->global->ctrl_iface) {
            inform_meshd_status(((struct wpa_supplicant *)sm->ctx->ctx), MESHD_WPA_SUPPLICANT_FAIL);
            wpa_printf(MSG_WARNING, "WPA-%s: error, inform meshd", __func__);
        } // else success is notified from within wpa_supplicant_process_3_of_4().
	os_free(tmp);
	return ret;
}
...
void wpa_sm_notify_assoc(struct wpa_sm *sm, const u8 *bssid)
{
	...
	wpa_printf(MSG_INFO, "WPA: Association event - clear replay counter");
