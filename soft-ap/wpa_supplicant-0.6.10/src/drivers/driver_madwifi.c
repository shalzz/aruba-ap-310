...
#else
#include "../../wireless_tools.28/wireless.h"
#endif
			   ...
			   const u8 *addr)
{
	...
	if ((memcmp(addr, zmac, 6) != 0) && (memcmp(addr, bcast, 6) != 0))
		key_idx = IEEE80211_KEYIX_NONE;
...
static void * wpa_driver_madwifi_init(void *ctx, const char *ifname)
{
	...
	if (drv->sock < 0)
		goto fail2;
#if defined(AP_PLATFORM)
	/* This is initially set by writing to the 'encryption' sysctl node
	 * in meshd_cfg.c. However, when meshd tells supplicant to remove
	 * and add interface, the flag gets reset. So add this back
	 */
	if (set80211param(drv, IEEE80211_PARAM_PRIVACY, 1, 1) < 0) {
...
#if defined(AP_PLATFORM)
	/*Similar to IEEE80211_PARAM_PRIVACY, IEEE80211_PARAM_WPA need to be reset when interface added*/
