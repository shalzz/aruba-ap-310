...
int wpa_driver_wext_get_ssid(void *priv, u8 *ssid)
{
	...
	if (ioctl(drv->ioctl_sock, SIOCGIWESSID, &iwr) < 0) {
		wpa_printf(MSG_ERROR, "ioctl[SIOCGIWESSID] err=%d", errno);
		ret = -1;
	} else {
...
#if defined(AP_PLATFORM)
	wpa_printf(MSG_DEBUG, "%s: Ignoring Wireless Events\n", __FUNCTION__);
	return;
#endif
...
int wpa_driver_is_dev_present(const char *ifname)
{
	struct ifreq ifr;
	int sock, dev_found=1;

	if ((sock = socket(PF_INET, SOCK_DGRAM, 0)) < 0) {
		wpa_printf(MSG_ERROR, "%s: Cannot create socket: errno=%d %s\n",
	...
	if (ioctl(sock, SIOCGIFFLAGS, (caddr_t) &ifr) < 0) {
		if (errno != ENODEV) {
			wpa_printf(MSG_ERROR, "%s: Dev %s present but error=%s\n",
