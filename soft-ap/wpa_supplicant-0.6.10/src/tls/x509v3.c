...
int x509_name_compare(struct x509_name *a, struct x509_name *b)
{
    ...
    wpa_printf(MSG_ERROR, "cn is %s, %s",a->cn,b->cn);
	res = x509_str_compare(a->cn, b->cn);
	if (res)
		return res;
    wpa_printf(MSG_ERROR, "c is %s, %s",a->c,b->c);
	res = x509_str_compare(a->c, b->c);
	if (res)
		return res;
    wpa_printf(MSG_ERROR, "l is %s, %s",a->l,b->l);
	res = x509_str_compare(a->l, b->l);
	if (res)
		return res;
    wpa_printf(MSG_ERROR, "st is %s, %s",a->st,b->st);
	res = x509_str_compare(a->st, b->st);
	if (res)
		return res;
    wpa_printf(MSG_ERROR, "o is %s, %s",a->o,b->o);
	res = x509_str_compare(a->o, b->o);
	if (res)
		return res;
    wpa_printf(MSG_ERROR, "ou is %s, %s",a->ou,b->ou);
	res = x509_str_compare(a->ou, b->ou);
	if (res)
		return res;
    wpa_printf(MSG_ERROR, "email is %s, %s",a->email,b->email);
				    ...
				    int *reason)
{
	...
	*reason = X509_VALIDATE_OK;
	...
	for (cert = chain, idx = 0; cert; cert = cert->next, idx++) {
		...
		if (chain_trusted)
			continue;
        /* skip the timestamp check for IAP since IAP does the ap1x authentication
         * during the bootup at which stage the timestape is quite small. */
#ifndef __FAT_AP__
        ...
        if (trusted == NULL ){
				wpa_printf(MSG_ERROR, "Trusted is NULL ");
        }
