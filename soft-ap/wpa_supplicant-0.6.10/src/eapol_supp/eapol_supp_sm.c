...
extern void notify_sapd_eap_success(void *);
#ifdef __FAT_AP__
extern void inform_ap1x_status(void *,unsigned int); 
#endif
SM_STEP(SUPP_PAE)
{
	...
	case SUPP_PAE_AUTHENTICATING:
        ...
        if (sm->eapSuccess && sm->portValid) {
            SM_ENTER(SUPP_PAE, AUTHENTICATED);
            notify_sapd_eap_success(sm->ctx->ctx);
#ifdef __FAT_AP__
            inform_ap1x_status(sm->ctx->ctx,AP1X_AUTH_MSG_OK);
#endif
        } else if (sm->eapFail || (sm->keyDone && !sm->portValid)) {
			SM_ENTER(SUPP_PAE, HELD);
#ifdef __FAT_AP__
            inform_ap1x_status(sm->ctx->ctx,AP1X_AUTH_MSG_FAIL);
#endif
        }
		else if (sm->suppTimeout){
			SM_ENTER(SUPP_PAE, CONNECTING);
#ifdef __FAT_AP__
            inform_ap1x_status(sm->ctx->ctx,AP1X_AUTH_MSG_TIMEOUT);
#endif
        }
		break;
	case SUPP_PAE_HELD:
