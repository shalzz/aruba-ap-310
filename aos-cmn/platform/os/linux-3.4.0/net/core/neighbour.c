...
#if defined(CONFIG_ARUBA_NET)
typedef __u16 ip_t[2];
int (*asap_del_rc_stale_arp_entry)(ip_t);
EXPORT_SYMBOL(asap_del_rc_stale_arp_entry);
		...
		while ((n = rcu_dereference_protected(*np,
			...
			if (atomic_read(&n->refcnt) == 1 &&
				...
				*np = n->next;
				...
				neigh_cleanup_and_release(n);
#if defined(CONFIG_ARUBA_NET)
				memcpy(&tmpaddr, n->primary_key, sizeof(int));
				FRAME_IP_SET(ipaddr, tmpaddr); 
				if(asap_del_rc_stale_arp_entry)
				{
					asap_del_rc_stale_arp_entry(ipaddr);
				}
#endif
				continue;
			}
