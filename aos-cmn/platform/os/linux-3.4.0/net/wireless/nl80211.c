			    ...
			    gfp_t gfp)
{
	...
	if (wdev) {
		if (nla_put_u64(skb, NL80211_ATTR_WIPHY,
				rdev->wiphy_idx))
			goto nla_put_failure;
		if (wdev->netdev &&
...
{
	struct cfg80211_registered_device *rdev = info->user_ptr[0];
	struct net_device *ndev = info->user_ptr[1];
				       ...
				       struct wireless_dev **wdev)
{
	...
	ndev = dev_get_by_index(sock_net(skb->sk), nla_get_u32(nl80211_fam.attrbuf[NL80211_ATTR_IFINDEX]));
	*wdev = ndev->ieee80211_ptr;
	if (IS_ERR(*wdev))
		*wdev = NULL;

	*rdev = cfg80211_rdev_by_wiphy_idx(nla_get_u32(nl80211_fam.attrbuf[NL80211_ATTR_WIPHY]));
	if (IS_ERR(*rdev)) {
		err = PTR_ERR(*rdev);
		goto out_unlock;
	}
	...
	cb->args[1] = nla_get_u32(nl80211_fam.attrbuf[NL80211_ATTR_IFINDEX]);//*wdev ? (*wdev)->identifier + 1 : 0;
	cb->args[2] = vcmd_idx;
