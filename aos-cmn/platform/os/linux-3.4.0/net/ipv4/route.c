...
#endif
#include "../../../../../../cmn/include/kernel_compat.h"
...
#define ARUBA_RAP_OVERHEAD	96

#if defined(CONFIG_ARUBA_AP) && !defined(CONFIG_GRENACHE)
void (*asap_pmtu_change_fp)(u32 daddr, u32 pmtu);
void
icmp_asap_pmtu_register_cb(void (*asap_pmtu_cb)(u32 daddr, u32 pmtu))
{
        asap_pmtu_change_fp = asap_pmtu_cb;
}
EXPORT_SYMBOL(icmp_asap_pmtu_register_cb);
#endif
				 ...
				 struct net_device *dev)
{
	...
	struct inet_peer *peer;
#if defined(CONFIG_ARUBA_NET) && !defined(CONFIG_GRENACHE)
    u32    tun_saddr=0;
    ...
    int ipsec_enabled = 0;
        ...
        if ((tdev = DEV_GET_BY_NAME("tun0")) && (bdev = DEV_GET_BY_NAME("br0"))) {
                struct in_device *bin_dev;
                struct in_device *tin_dev;
                struct in_ifaddr *ifa_tmp;
            if ((bin_dev = __in_dev_get_rtnl(bdev)) != NULL) {
                    for (ifa_tmp = bin_dev->ifa_list; ifa_tmp; ifa_tmp = ifa_tmp->ifa_next) {
                    //if (bin_dev->ifa_list->ifa_local == iph->saddr) {
                    /*bugfix 34254, since we create shadow interface on br0
                     *for rap-dhcp-server, br0 ip can get pushed by br0:1 ip
                     *check all the ip-addr in ifa_list*/
                        if (ifa_tmp->ifa_local == iph->saddr) {
                            if ((tin_dev = __in_dev_get_rtnl(tdev)) != NULL) {
                                tun_saddr = tin_dev->ifa_list->ifa_local;
                            }
                        }
                    }
                }
	...
	if (peer) {
        ...
        /* ASAP will update the tun device mtu only if
           LMS and daddr is same. */
        if (asap_pmtu_change_fp) {
            int asap_mtu = mtu;
            if (ipsec_enabled) {
                asap_mtu -= ARUBA_RAP_OVERHEAD;
                if (asap_mtu < ip_rt_min_pmtu)
                    asap_mtu = ip_rt_min_pmtu;
            }
            asap_pmtu_change_fp(iph->daddr, asap_mtu);
        }
        if (rtnl_held)
            rtnl_unlock();
#endif
...
#if defined(CONFIG_ARUBA_AP)
            || ((strncmp(dst->dev->name, "tun", 3) == 0) && 
               (peer->pmtu_learned < orig_dst_mtu + ARUBA_RAP_OVERHEAD))
#endif
            ) {
			if (!peer->pmtu_orig)
...
#if defined(CONFIG_ARUBA_AP)
    {
        if (dst->dev && strncmp(dst->dev->name, "tun", 3) == 0 && 
            !dst_metrics_read_only(dst)) {
            if ((mtu - ARUBA_RAP_OVERHEAD) < dst->dev->mtu)
                return (mtu - ARUBA_RAP_OVERHEAD);
            else 
                return dst->dev->mtu;;
        } else {
            return mtu;
...
#endif
#if defined (CONFIG_ARUBA_NET) && !defined(CONFIG_GRENACHE) && !defined(CONFIG_ARUBA_CONTROLLER)
	/* Bug 67276: selecting the default even if oif is specified in asap_gre_tx */
	if (!res.prefixlen && res.table->tb_num_default > 1 && res.type == RTN_UNICAST)
#else
	if (!res.prefixlen &&
...
struct rtable *ip_route_output_key_aruba(struct net *net, struct flowi4 *flp)
{
    struct rtable *rt = ip_route_output_flow(net, flp, NULL);
    return rt;
}
EXPORT_SYMBOL(ip_route_output_key_aruba);
#endif
