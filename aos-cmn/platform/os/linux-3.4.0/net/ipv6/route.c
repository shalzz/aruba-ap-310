						    ...
						    int flags)
{
	...
	if (!oif && ipv6_addr_any(saddr)
            ...
            && !(flags & RT6_LOOKUP_F_SADDR_IGNORE)
#endif
           )
		goto out;
...
restart:
    ...
    /* Bug185406: dropping route entry if rt6i_table is NULL and print some
     * useful message */
    if (rt->rt6i_table == NULL) {
        int i = 0;

        pr_err("ERROR: rt6i_table is NULL! rt6_info dump:\n");
        for (i = 0; i < sizeof(struct rt6_info); i += 8) {
            char *ptr = &((char *)rt)[i];
...
#ifdef CONFIG_ARUBA_AP
        if (fl6->flowi6_flags & RT6_LOOKUP_F_SADDR_IGNORE) {
            flags |= RT6_LOOKUP_F_SADDR_IGNORE;
        }
#endif
