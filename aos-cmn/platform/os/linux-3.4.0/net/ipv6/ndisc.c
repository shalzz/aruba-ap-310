...
struct neigh_table *asap_nd_tbl = &nd_tbl;
EXPORT_SYMBOL(asap_nd_tbl);
#endif /* CONFIG_ARUBA_AP && !__FAT_AP__ */
...
static void ndisc_ra_useropt(struct sk_buff *ra, struct nd_opt_hdr *opt)
{
    ...
    /* ARUBA: in here, we use nduseropt_pad1 to store 
     * M/O flags in icmp6hdr for user APP used */
    {
        ndmsg->nduseropt_pad1 = (icmp6h->icmp6_addrconf_managed ?
                IF_RA_MANAGED : 0) |
            (icmp6h->icmp6_addrconf_other ?
                IF_RA_OTHERCONF : 0);
