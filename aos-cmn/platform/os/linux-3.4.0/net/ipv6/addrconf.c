...
/*
 * ipv6_dev_find()
 *	Find (and hold) net device that has the given address.  Or NULL on failure.
 */
...
void addrconf_prefix_rcv(struct net_device *dev, u8 *opt, int len, bool sllao)
{
    ...
    /* aruba: if Managed-Config-Flag is set, use DHCPv6 to get address.
     * RFC 4861 (IPv6 Neighbor Discovery) in Section 4.2 
     * "M flag indicates addresses are available DHCPv6" */
...
#ifdef CONFIG_ARUBA_AP
/*
 * resurrected this to keep ASAP_mod cleaner
 */
		   ...
		   const struct in6_addr *daddr, struct in6_addr *saddr)
{
	struct net *net;
	if (dst) {
		net = dev_net(ip6_dst_idev(dst)->dev);
	} else {
		net = &init_net;
	}
	return ipv6_dev_get_saddr(net, dst ? ip6_dst_idev(dst)->dev : NULL, 
		daddr, 0/* XXX */, saddr);
}
EXPORT_SYMBOL(ipv6_get_saddr);

static struct inet6_ifaddr *ipv6_get_linklocal_ifaddr(struct net_device *dev)
{
        struct inet6_ifaddr *ifp, *result = NULL;
...
static int inet6_trigger_router_discovery(struct net *net, int ifindex)
{
       struct inet6_ifaddr *ifp;
       struct net_device *dev;
       struct in6_addr lladdr;
       ...
       ifp = ipv6_get_linklocal_ifaddr(dev);
       if (!ifp) {
               return -EADDRNOTAVAIL;
       }


       ndisc_send_rs(dev, &ifp->addr, &in6addr_linklocal_allrouters);
...
int addrconf_trigger_router_discovery(struct net *net, void __user *arg)
{
        struct in6_ifreq ireq;
        int err;
        ...
        rtnl_lock();
        err = inet6_trigger_router_discovery(net, ireq.ifr6_ifindex);
        rtnl_unlock();
        return err;
}
#endif
