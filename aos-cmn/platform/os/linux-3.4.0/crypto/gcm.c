...
static void fips_selftest_gcm(struct work_struct *);
static DECLARE_WORK(fips_gcm_kat_work, fips_selftest_gcm);
#endif
...
static void fips_selftest_gcm(struct work_struct *w)
{
    int err;
    int fail_test = 0;

    printk("Starting Kernel AESGCM KAT ...");
    err = alg_test("gcm(aes)", "gcm(aes)", 0, 0);
    if(fips_fail_test && !memcmp(fips_fail_test, "gcm(aes)", 8))
        fail_test = 1;
    if (err || fail_test) {
        printk("Failed Kernel AESGCM KAT \n");
        aruba_save_kernel_kat_reboot_reason("Failed Kernel AESGCM KAT ");
        kernel_restart(NULL);
    } else {
        printk("Completed Kernel AESGCM KAT \n");
    }
}

void handle_gcm_selftest(void)
{
    schedule_work(&fips_gcm_kat_work);
}

EXPORT_SYMBOL(handle_gcm_selftest);
#endif
