...
static void fips_selftest_des(struct work_struct *w);
static DECLARE_WORK(fips_des_kat_work, fips_selftest_des);
#endif
...
static const u8 key[24] = {
	0x03, 0x52, 0x02, 0x07, 0x67, 0x20, 0x82, 0x17,
	0x86, 0x02, 0x87, 0x66, 0x59, 0x08, 0x21, 0x98,
	0x64, 0x05, 0x6A, 0xBD, 0xFE, 0xA9, 0x34, 0x57
};

static const u8 plaintext[8] = {
	0x73, 0x71, 0x75, 0x69, 0x67, 0x67, 0x6C, 0x65 
};

static const u8 ciphertext[8] = {
...
static void fips_selftest_des(struct work_struct *w)
{
	...
	printk("Starting Kernel DES KAT ...");
	...
	if (ret) {
		printk("Failed Kernel DES KAT \n");
		aruba_save_kernel_kat_reboot_reason("DES set key failed");
		kernel_restart(NULL);
		return;
	}
	crypto_cipher_encrypt_one(tfm, buf, plaintext);
	if (memcmp(buf, ciphertext, sizeof(buf)))
	{
		printk("Failed Kernel DES KAT \n");
		aruba_save_kernel_kat_reboot_reason("DES Encrypt Failed");
		kernel_restart(NULL);
		return;
	}
	crypto_cipher_decrypt_one(tfm, buf, ciphertext);
	if (memcmp(buf, plaintext, sizeof(buf)))
	{
		printk("Failed Kernel DES KAT \n");
		aruba_save_kernel_kat_reboot_reason("DES Decrypt Failed");
		kernel_restart(NULL);
		return;
	}
	crypto_free_cipher(tfm);
    if(fips_fail_test && !memcmp(fips_fail_test, "des3-ede", 8))
    {
        printk("Failed Kernel DES KAT \n");
        aruba_save_kernel_kat_reboot_reason("DES Decrypt Failed");
        kernel_restart(NULL);
	...
	printk("Completed Kernel DES KAT \n");
}

void handle_des_selftest(void)
{
	schedule_work(&fips_des_kat_work);
}
EXPORT_SYMBOL(handle_des_selftest);
