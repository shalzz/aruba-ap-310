...
* a == 3: trap on any
*/
void arch_set_watchpoint(void *v, unsigned bytes, int a)
{
	int ctrl_reg, val_reg;
	u32 addr;
	struct arch_hw_breakpoint_ctrl ctrl;

	ctrl_reg = ARM_BASE_WCR;
	val_reg = ARM_BASE_WVR;

	addr = (u32)v;

	if (bytes > 8)
		bytes = 8;
	if (bytes > max_watchpoint_len)
		bytes = max_watchpoint_len;

	/* GB2015 based on arch_build_bp_info */
	...
	case 3:
	default:
		ctrl.type = ARM_BREAKPOINT_LOAD | ARM_BREAKPOINT_STORE;
		break;
	}
	...
	/* Privilege */
	ctrl.privilege = ARM_BREAKPOINT_PRIV | ARM_BREAKPOINT_USER;
	...
	/* Setup the address register. */
	write_wb_reg(val_reg, addr);

	/* Setup the control register. */
	write_wb_reg(ctrl_reg, encode_ctrl_reg(ctrl));

	aruba_watch_set = 1;
}

void arch_clear_watchpoint(void)
{
...
#ifdef CONFIG_ARUBA_AP
    if (unlikely(aruba_watch_set)) {
		arch_clear_watchpoint(); /* avoid hang while printing watched data */
        die("debug exception",regs,SIGSEGV);
    }
#endif
