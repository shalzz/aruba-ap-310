...
static int get_nand_partitions(void)
{
...
#ifdef CONFIG_ARUBA_AP
    /* For aruba AP, we only use one MTD partition. Here are temporary hardcode
     * We won't use the code after NAND flash works */
    if (xml_ap_model != AP_148) {
        ...
        ptn++;
        name = msm_nand_names + (msm_nand_data.nr_parts * SMEM_MAX_PART_NAME);
        strcpy(name, "aos1");
        ptn->name = name;
        ptn->size = 0x100;
        ptn->offset = 0x100;
