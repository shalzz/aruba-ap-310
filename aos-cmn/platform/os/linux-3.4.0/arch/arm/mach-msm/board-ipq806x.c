...
#include <oem/aphw.h>
extern int aruba_get_i2c_bus_for_device(char *);
extern void aruba_get_i2c_bus_info(unsigned, unsigned *, unsigned *);
...
#if defined(CONFIG_ARUBA_AP) && defined(CONFIG_ARUBA_80211AD_SUPPORT)
static void __init reserve_wigig_dump_memory(void)
{
#define IPQ_WIGIG_DUMP_ADDR	0x44800000
#define IPQ_WIGIG_DUMP_SIZE	(2 << 20)

	msm_reserve_memory(IPQ_WIGIG_DUMP_ADDR, IPQ_WIGIG_DUMP_SIZE);
}
#endif
...
void ipq806x_pcie_pdata_fixup(void)
{
	...
	/*For sierra, swap PCIE rst pin according to hw*/
	if ( xml_ap_model == AP_374 || xml_ap_model == AP_375 || xml_ap_model == AP_377 || xml_ap_model == AP_318 )
	{
		msm_pcie_platform_data[0].gpio = msm_pcie_gpio_info[1];
		msm_pcie_platform_data[1].gpio = msm_pcie_gpio_info[0];
	}
}
...
#ifdef CONFIG_ARUBA_AP
static struct i2c_gpio_platform_data ipq806x_i2c_gpio_data_ap32x = {
    .sda_pin = 0,
    .scl_pin = 0,
};

struct platform_device ipq806x_device_i2c_gpio_ap32x = {
	.name		= "i2c-gpio",
	.id		= 0,
	.dev		= {
		.platform_data = &ipq806x_i2c_gpio_data_ap32x,
	},
};

static struct platform_device *cdp_i2c_ipq806x_devices_ap32x[] __initdata = {
	&ipq806x_device_i2c_gpio_ap32x,
};

static struct i2c_board_info i2c_devices[] = {
#if defined(CONFIG_TPM_AT97SC3203)
    {
        I2C_BOARD_INFO("at97sc3203", 0x29),
    },
#endif
};
static struct i2c_board_info ap37x_i2c_devices[] = {
	{
		I2C_BOARD_INFO("tca9539", 0x77),
	},
	{
		I2C_BOARD_INFO("isl28022", 0x40),
	}
};
static struct pca953x_platform_data ap385_ioexp_data = {
	.gpio_base = FIRST_BOARD_GPIO,
	.invert = 0,
	.irq_base = -1,
};
static struct i2c_board_info ap385_i2c_devices[] = {
	{
...
#endif
	{
		I2C_BOARD_INFO("ad7416", 0x48),
	},
	{
		I2C_BOARD_INFO("m41t83", 0x68),
	}
};
static struct i2c_board_info ap31x_i2c_devices[] = {
	{
		I2C_BOARD_INFO("isl28022", 0x40),
	}
};
...
#if defined(CONFIG_OCTOMORE)
/* GPIO configuration for GSBI UART2 - configured as a 2-wire UART */
static struct msm_serial_hslite_platform_data ipq806x_uart_pdata = {
	.config_gpio		= 2,
	.uart_tx_gpio		= 22,
	.uart_rx_gpio		= 23,
        .line                   = 2,
...
/* GPIO configuration for GSBI UART7 - configured as a 2-wire UART */
static struct msm_serial_hslite_platform_data ap385_uart_cm_pdata = {
	.config_gpio        = 2,
	.uart_tx_gpio       = 6,
	.uart_rx_gpio       = 7,
	.line               = 0,
...
/* GPIO configuration for GSBI UART7 - configured as a 2-wire UART */
static struct msm_serial_hslite_platform_data sierra_uart_ble_pdata = {
	.config_gpio        = 2,
	.uart_tx_gpio       = 6,
	.uart_rx_gpio       = 7,
	.line               = 2,
...
static void __init ipq806x_common_init(void)
{
	...
	*((uint32_t *)(ipq806x_lpass_lpaif.dev.platform_data)) = socinfo_get_version();
	...
	if (machine_is_ipq806x_ap148() || machine_is_ipq806x_ap148_1xx() ||
        ...
        if (xml_ap_model != AP_148 && xml_ap_model != AP_160) {
            unsigned busnum = 0;

            busnum = aruba_get_i2c_bus_for_device("tpm");

            aruba_get_i2c_bus_info(busnum, &(ipq806x_i2c_gpio_data_ap32x.scl_pin), 
                        &(ipq806x_i2c_gpio_data_ap32x.sda_pin));
            
            platform_add_devices(cdp_i2c_ipq806x_devices_ap32x,
                    ARRAY_SIZE(cdp_i2c_ipq806x_devices_ap32x));
            if (xml_ap_model == AP_385) {
                ap385_i2c_devices[0].platform_data = &ap385_ioexp_data;
                i2c_register_board_info(busnum, ap385_i2c_devices, ARRAY_SIZE(ap385_i2c_devices));
                sfp_i2c_busnum = busnum;
            }
            else
            i2c_register_board_info(busnum, i2c_devices, ARRAY_SIZE(i2c_devices));

            if ((xml_ap_model == AP_314 || xml_ap_model == AP_315 || xml_ap_model == AP_387)) {
                int rc, pm_gpio;

                /* request gpio for power monitor interrupt line */
                pm_gpio = aruba_get_gpio_pin("pm-int");
                if (pm_gpio < 0)
                    pr_err("%s: power monitor irq not defined\n",__func__);
                else {
                    rc = gpio_request(pm_gpio, "pm-int");
                    if (rc < 0)
                        pr_err("%s: gpio %d request failed\n", __func__, pm_gpio);
                    else
                        /* assign interrupt line in I2C device */
                        ap31x_i2c_devices[0].irq = gpio_to_irq(pm_gpio);
                 }
                 i2c_register_board_info(busnum, ap31x_i2c_devices,
                                    ARRAY_SIZE(ap31x_i2c_devices));
             }
            if ((xml_ap_model == AP_374 || xml_ap_model == AP_375 || xml_ap_model == AP_377 || xml_ap_model == AP_318)) {
                int rc, pm_gpio;
                ...
                if (pm_gpio < 0)
                    ...
                    if (rc < 0)
                        ...
                        /* assign interrupt line in I2C device */
                        ap37x_i2c_devices[1].irq = gpio_to_irq(pm_gpio);
                 }
				ap37x_i2c_devices[0].platform_data = &ap385_ioexp_data;
                 i2c_register_board_info(busnum, ap37x_i2c_devices,
                                    ARRAY_SIZE(ap37x_i2c_devices));
             }
        }
#endif
...
static void nss_gmac_init(void)
{
...
#ifdef CONFIG_OCTOMORE
	if (machine_is_ipq806x_ap148()) {
        if (xml_ap_model == AP_324 || xml_ap_model == AP_325) {
            mdata->irqs[0] = MSM_GPIO_TO_INT(aruba_get_gpio_pin("phy0-int"));
            mdata->irqs[1] = MSM_GPIO_TO_INT(aruba_get_gpio_pin("phy1-int"));
            irq_set_irq_type(mdata->irqs[0], IRQ_TYPE_EDGE_FALLING);
            irq_set_irq_type(mdata->irqs[1], IRQ_TYPE_EDGE_FALLING);
            ...
            mdata->irqs[0] = MSM_GPIO_TO_INT(aruba_get_gpio_pin("phy0-int"));
            //mdata->irqs[1] = MSM_GPIO_TO_INT(aruba_get_gpio_pin("phy0-int"));
            irq_set_irq_type(mdata->irqs[0], IRQ_TYPE_EDGE_FALLING);
            //irq_set_irq_type(mdata->irqs[1], IRQ_TYPE_EDGE_FALLING);
            ...
            pdata->mmds_mask = 0;
            pdata->sfp_port = 1;
            pdata->sfp_i2c_busnum = sfp_i2c_busnum;

            platform_device_register(&nss_gmac_0);
            platform_device_register(&nss_gmac_1);
        } else if (xml_ap_model == AP_314 || xml_ap_model == AP_315) {
            mdata->irqs[5] = MSM_GPIO_TO_INT(aruba_get_gpio_pin("phy0-int"));
            irq_set_irq_type(mdata->irqs[5], IRQ_TYPE_EDGE_FALLING);
			...
			platform_device_register(&nss_gmac_3);
        } else if (xml_ap_model == AP_374 || xml_ap_model == AP_375 ||  xml_ap_model == AP_377 ||  xml_ap_model == AP_318 ) {
            mdata->irqs[5] = MSM_GPIO_TO_INT(aruba_get_gpio_pin("phy0-int"));
			//mdata->irqs[5] = 15;
            irq_set_irq_type(mdata->irqs[5], IRQ_TYPE_EDGE_FALLING);
	...
	else if (machine_is_ipq806x_ap160() || machine_is_ipq806x_ap161()) {
...
#ifdef CONFIG_ARUBA_AP
extern unsigned aruba_mac_count;
unsigned char *aruba_get_mac_addr(int idx);
extern void aruba_get_env_mac(uint8_t *mac);
#endif
				...
				struct msm_nss_gmac_platform_data *pdata)
{
    ...
    if ((xml_ap_model == AP_324 || xml_ap_model == AP_325)) {
        if (id >= 2) {
            memcpy(mac, aruba_get_mac_addr(id - 2), sizeof(pdata->mac_addr));
        }
        return;
    }
	else if (xml_ap_model == AP_385) {
		if (id < 2) {
			memcpy(mac, aruba_get_mac_addr(id), sizeof(pdata->mac_addr));
		}
		return;
	}
    else if ((xml_ap_model == AP_314 || xml_ap_model == AP_315)) {
        if (id == 3) {
            memcpy(mac, aruba_get_mac_addr(3 - id), sizeof(pdata->mac_addr));
        }
		return;
	}
	else if (xml_ap_model == AP_160)  {
        if (id == 1) {
			aruba_get_env_mac(aruba_get_mac_addr(0));
            memcpy(mac, aruba_get_mac_addr(0), sizeof(pdata->mac_addr));
        }
		return;
	}
	else if (xml_ap_model == AP_387)  {
...
static void __init ipq806x_init(void)
{
	...
	if (machine_is_ipq806x_ap148() ||
		...
		if (machine_is_ipq806x_ap148()) {
#if defined(CONFIG_OCTOMORE)
			/* Enable internal UART - GSBI7 UART for sierra ble port. Toba use the same ble uart port as sierra */
			if (xml_ap_model == AP_374 ||xml_ap_model == AP_375 || xml_ap_model == AP_377 || xml_ap_model == AP_318 || xml_ap_model == AP_387)
			{
				ipq806x_device_uart_gsbi7.id = 0;
				ipq806x_device_uart_gsbi7.dev.platform_data = &sierra_uart_ble_pdata;
				platform_device_register(&ipq806x_device_uart_gsbi7);
			}
			else{
				/* Enable internal UART - GSBI UART2 for GPS and GSBI UART7 for CM */
				if (xml_ap_model == AP_385) {
					ipq806x_device_uart_gsbi7.id = 0;
					ipq806x_device_uart_gsbi7.dev.platform_data = &ap385_uart_cm_pdata;
					platform_device_register(&ipq806x_device_uart_gsbi7);
				}
				/* Enable internal UART - GSBI UART2 */
				ipq806x_device_uart_gsbi2.id = 2;
				ipq806x_device_uart_gsbi2.dev.platform_data = &ipq806x_uart_pdata;
				platform_device_register(&ipq806x_device_uart_gsbi2);
			}
#else
...
#ifdef CONFIG_OCTOMORE
MACHINE_START(IPQ806X_AP148, "IPQ806X wave-2 board")
#else
