...
#include <oem/aphw.h>
extern int aruba_get_gpio_pin(char *name);
extern void aruba_gpio_out(unsigned pin, unsigned val);
extern void aruba_gpio_config_output(unsigned pin, unsigned value);
#endif
...
void msm_restart(char mode, const char *cmd)
{
	...
	if (xml_ap_model == AP_385) {
		aruba_gpio_out(aruba_get_gpio_pin("reset"), 1);
	}
	else if (xml_ap_model != AP_148) {
        aruba_gpio_out(aruba_get_gpio_pin("reset"), 0);
    }
#endif
