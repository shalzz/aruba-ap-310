...
static int msm_pcie_setup(int nr, struct pci_sys_data *sys)
{
	...
	PCIE_DBG("bus %d\n", nr);
	if (nr < 0 && nr >= CONFIG_MSM_NUM_PCIE)
		return 0;
...
#ifdef CONFIG_ARUBA_AP
	if(dev->bus == 1 && !sierra_ap_board ) {    //cap PCIe signal amplitude to 0x50 on 5G radio,don't apply this configuration to Sierra
		writel_relaxed(PCIE20_PARF_PCS_SWING_TX_SWING_FULL(0x50) |
				PCIE20_PARF_PCS_SWING_TX_SWING_LOW(0x50),
...
#ifdef CONFIG_ARUBA_AP
	if(dev->bus == 2 && !sierra_ap_board) {   //force PCIe 1.1 on 2G radio,don't apply this configuration to Sierra
		writel_relaxed((readl_relaxed(dev->pcie20 + PCIE20_LNK_CAS2) | 1 ), dev->pcie20 + PCIE20_LNK_CAS2);
	}
#else
