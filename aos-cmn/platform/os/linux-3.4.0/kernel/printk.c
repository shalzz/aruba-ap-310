...
char *kernel_log_buf = __log_buf;
unsigned *kernel_log_idx = &log_start;	/* Index into log_buf: next char to be read by syslog() */
#endif
...
void
kernel_log_memcpy(char *buf, int len)
{
	int i;

	for (i = 0; i < len; i++)
		buf[i] = LOG_BUF(log_end - len + i);
}
EXPORT_SYMBOL(kernel_log_memcpy);

unsigned
kernel_get_logged_chars(void)
{
	return logged_chars;
}
EXPORT_SYMBOL(kernel_get_logged_chars);
