...
#else
static ssize_t aruba_vol_cdev_write(struct file *file, const char __user *buf,
			      size_t count, loff_t *offp)
{
	int err = 0;
	struct ubi_volume_desc *desc = file->private_data;
	...
	if (!vol->updating && !vol->changing_leb) {
        ...
        if (err < 0) {
            ubi_err("cannot get exclusive\n");
            return err;
        }
        ubi_start_update(ubi, vol, count);
    }
