...
static uint32_t calculate_bitflip_cnt(uint8_t data)
{
	uint8_t check_ruler;
	uint32_t i,cnt = 0;
	for (i=0; i< 8;i++)
	...
	while (page_count-- > 0) {
		...
		if (rawerr) {
			if (ops->datbuf && ops->mode != MTD_OPS_RAW) {
				...
				for (n = 0,bitflips_per_cw = 0; n < mtd->writesize; n++) {
...
#ifdef CONFIG_ARUBA_AP				
                    /*
                     * The nand Flash can't gurantee all of the bytes in a erased page are all 0xff
                     * If the bitflip numbers are lower than ecc capablity,these  bits can be corrected by ecc when there is  some data written in. 
                     * In that case,we should also ignore it.
                     */
                        bitflips_per_cw += calculate_bitflip_cnt(datbuf[n]);
                        /*Correct bitflip here avoid error when UBIFS checking empty page */
                        datbuf[n] = 0xff;
					}
                    /*
                    * reset the bitflip counters for every codeword
                    */
                    if (bitflips_per_cw && !(n%(mtd->writesize/cwperpage))) {
                        pr_crit("Nand: corrected %d bitflips in the %d cw in a erased page\n",bitflips_per_cw,(n-1)/512);
                        if (bitflips_per_cw > 3) {
                            pageerr = rawerr;
                            break;
                        }
                        bitflips_per_cw = 0;
...
#ifdef CONFIG_ARUBA_AP
				if (ecc_errors) {
					pr_info("NAND: %d bit%s corrected at (blk:0x%03x, page:0x%02x)\n",
						ecc_errors, (ecc_errors > 1) ? "s" : "",
						page >> 6, page & ((1<<6)-1));
					/*
					 * Reduce the amount of UBI scrubbing activity by only
					 * reporting 4 or more bit-flips
					 */
					if (ecc_errors > 3) {
	      ...
	      size_t *retlen, u_char *buf)
{
	...
	struct mtd_oob_ops ops;
	int (*read_oob)(struct mtd_info *, struct mtd_ecc_stats *, loff_t, struct mtd_oob_ops *);
...
{
	struct msm_nand_chip *chip = NULL;
