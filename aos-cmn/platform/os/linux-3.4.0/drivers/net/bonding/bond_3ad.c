...
EXPORT_SYMBOL_GPL(bond_register_cb);
...
EXPORT_SYMBOL_GPL(bond_unregister_cb);
...
#ifdef CONFIG_ARUBA_AP
struct aruba_lacp_tx_ctr aruba_lacp_tx_ctrs[3][2];
#endif
/*
	...
	bond_for_each_slave_from(bond, slave, i, start_at) {
		...
		if (SLAVE_IS_OK(slave) && agg && (slave_agg_id == agg_id)) {
                    ...
                    if (AP_MODEL_SUPPORTS_DOT3AD(__ap_hw_info[xml_ap_model])) {
                        int dev_idx = slave->dev->ifindex;
                        int idx;
                        if (unlikely(-1 == aruba_eth0_ifindex)) {
                            if (strcmp(__ap_hw_info[xml_ap_model].enets[0].ifname, slave->dev->name) == 0) {
                                aruba_eth0_ifindex = dev_idx;
                            }
                        }
                        if (skb) {
                            idx = (skb->flags & TUNNEL_SKB_FLAG_GRE_RADIOA) ? 0 : ((skb->flags & TUNNEL_SKB_FLAG_GRE_RADIOG) ? 1 : 2);
                            if (dev_idx == aruba_eth0_ifindex) {
                                ++aruba_lacp_tx_ctrs[idx][0].count;
                            } else {
                                ++aruba_lacp_tx_ctrs[idx][1].count;
                            }
                        }
                    }
#endif
