...
static ssize_t bonding_show_lacp_pducounter(struct device *d,
				 struct device_attribute *attr,
				 char *buf)
{
	struct bonding *bond = to_bond(d);
	unsigned int    pdus = bond->lacp_pducounter;
...
static ssize_t bonding_store_lacp_pducounter(struct device *d,
				  struct device_attribute *attr,
				  const char *buf, size_t count)
{
	int ret = count;
...
static DEVICE_ATTR(lacp_pducounter, S_IRUGO | S_IWUSR,
		   bonding_show_lacp_pducounter, bonding_store_lacp_pducounter);

static ssize_t bonding_show_lacp_txstats(struct device *d,
				 struct device_attribute *attr,
				 char *buf)
{
	struct bonding *bond = to_bond(d);
        ...
        for (i = 0; i < 2; ++i) {
                offs += sprintf(buf+offs, "GRE Tx striping: radio=%d enet0=%u enet1=%u\n",
                        i, aruba_lacp_tx_ctrs[i][0].count, aruba_lacp_tx_ctrs[i][1].count);
        }
        offs += sprintf(buf+offs, "GRE Tx striping: other=2 enet0=%u enet1=%u\n",
                        aruba_lacp_tx_ctrs[2][0].count, aruba_lacp_tx_ctrs[2][1].count);
        return offs;
}

static ssize_t bonding_store_lacp_txstats(struct device *d,
				  struct device_attribute *attr,
				  const char *buf, size_t count)
{
	int ret = count;
    ...
    for (i = 0; i < 3; ++i) {
        for (j = 0; j < 2; ++j) {
            aruba_lacp_tx_ctrs[i][j].count = 0; 
        }
    }

	return ret;
}
static DEVICE_ATTR(lacp_txstats, S_IRUGO | S_IWUSR,
		   bonding_show_lacp_txstats, bonding_store_lacp_txstats);
#endif
...
#ifdef CONFIG_ARUBA_AP
	&dev_attr_lacp_pducounter.attr,
	&dev_attr_lacp_txstats.attr,
#endif
