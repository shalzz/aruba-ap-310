...
extern int (*ppp_asap_firewall_netif_rx)(struct sk_buff *skb); 
extern int is_3g; 
extern int is_pppoe_uplink; 
#endif 
...
#if defined(CONFIG_ARUBA_AP) && !defined(CONFIG_GRENACHE) 
/* This function is the default xmit function for ppp0 for all cases. In case 
 * of non-3G RAPs with ppp uplinks, the packets will be sent to the original xmit fn 
 * (ppp_start_xmit) */ 
static int 
redir_ppp_start_xmit(struct sk_buff *skb, struct net_device *dev) 
{ 
     struct sk_buff *new_skb; 
   
     if (!(is_3g||is_pppoe_uplink) || skb->protocol != htons(ETH_P_IP) ||  
             skb->rap_flags & ASAP_SKB_RAP_FLAG_PKT_FWALLED) { 
        
 #if defined(CONFIG_ARUBA_AP) && !defined(CONFIG_GRENACHE) && !defined(CONFIG_ARUBA_CONTROLLER) 
         if (skb->rap_flags & ASAP_SKB_RAP_FLAG_PKT_FWALLED) { 
             asap_port_mirror_packet_to_dev(skb, 1, 0); 
         } 
 #endif 
         return ppp_start_xmit(skb, dev); 
     } 
   
     new_skb = skb_realloc_headroom(skb, skb_headroom(skb) + 64); 
     ...
     if(new_skb != NULL) { 
         ...
         if (is_3g) { 
             ...
             return ppp_asap_firewall_netif_rx(skb); 
     }  
     else if (is_pppoe_uplink) { 
             skb->rap_flags |= ASAP_SKB_RAP_FLAG_PPPOE_PKT_TO_BE_FWALLED; 
             skb->protocol = htons (ETH_P_IP); 
             if (dev->asap_netif_rx) { 
                 return dev->asap_netif_rx(skb); 
             }  
         else { 
                 printk(KERN_ALERT "pppoe uplink setup without asap_netif_rx Line %d\n", __LINE__); 
             } 
...
#if defined(CONFIG_ARUBA_AP) && !defined(CONFIG_GRENACHE) 
        .ndo_start_xmit = redir_ppp_start_xmit, 
#else 
...
/*
 * __ppp_is_multilink()
 *	Returns >0 if the device is a multilink PPP netdevice,
 *	0 if not or < 0 if the device is not PPP. This should be only
 *	called with ppp_lock held.
 */
