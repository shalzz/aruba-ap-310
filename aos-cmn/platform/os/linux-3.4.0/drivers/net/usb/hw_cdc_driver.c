...
static void rx_submit (struct hw_cdc_net *dev, struct urb *urb, gfp_t flags)
{
    ...
    if ((skb = alloc_skb (size + NEW_SKB_HEADER, flags)) == NULL) { 
...
int redir_hw_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
    ...
    pkt_type = ntohs(*(unsigned short *)(skb->data + 12));
    if (pkt_type == 0x0806) {
        kfree_skb(skb);
        return 0;
    }
...
EXPORT_SYMBOL(redir_hw_start_xmit);
...
void hw_disconnect (struct usb_interface *intf)
{
    ...
    /*Õ¨≤Ω»°œ˚ø…ƒ‹◊¢≤·µƒ—”≥Ÿπ§◊˜Ω¯≥Ã£¨»Áπ˚∏√π§◊˜Ω¯≥Ã“—æ≠‘⁄÷¥––¡À£¨
    ‘Ú‘⁄’‚¿Ô£¨–Ë“™µ»¥˝∏√π§◊˜Ω¯≥Ã÷¥––ÕÍ≥…÷Æ∫Û£¨discconect≤≈ª·ºÃ–¯Õ˘œ¬÷¥––°£*/
...
static const struct usb_device_id    hw_products [] = {
    /*…æ≥˝∂‘PRODUCT IDµƒ±»Ωœ£¨ƒ¨»œƒ‹πª÷ß≥÷À˘”–HUAWEI_ETHER_INTERFACE Ω”ø⁄¿‡–ÕµƒNDIS…Ë±∏*/
    /* delete by lKF36757 2011/12/26,prevent hilink load hw_cdc_driver.ko*/
    /*
    {
