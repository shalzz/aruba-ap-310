...
static void aruba_msm_spi_process_transfer(struct msm_spi *dd)
{
    u8  bpw;
    u32 spi_ioc;
    u32 spi_iom;
    ...
    /* DMA mode can't be used in interrupt context */
    dd->mode = SPI_FIFO_MODE;
    if (dd->multi_xfr) {
        dd->read_len = dd->cur_transfer->len;
        dd->write_len = dd->cur_transfer->len;
    ...
    do {
        ...
        do {
            ...
            count--;
            udelay(10);
        } while ((op & (SPI_OP_INPUT_SERVICE_FLAG | SPI_OP_OUTPUT_SERVICE_FLAG)) == 0 && count);
        ...
        if (op & SPI_OP_INPUT_SERVICE_FLAG) {
             ...
             */
            mb();
            msm_spi_input_irq(dd->irq_in, dd);
        }
        ...
        if (op & SPI_OP_OUTPUT_SERVICE_FLAG) {
             ...
             */
            mb();
            msm_spi_output_irq(dd->irq_in, dd);
        }
...
static int msm_spi_direct_transfer(struct spi_device *spi, struct spi_message *msg)
{
    struct msm_spi	*dd;
    unsigned long    flags;
    struct spi_transfer *tr;
    ...
    if (!mutex_trylock(&dd->core_lock)) {
        printk("%s: spi is busy!\n", __func__);
    }
    ...
    if (dd->use_rlock) {
        if (!remote_mutex_trylock(&dd->r_lock)) {
            printk("%s: spi remote locked!\n", __func__);
        }
    }
    ...
    /* start transfer */
    aruba_msm_spi_process_transfer(dd);
...
#ifdef CONFIG_ARUBA_AP
	master->direct_xfer       = msm_spi_direct_transfer;
#endif
