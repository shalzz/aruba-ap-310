...
int is_e3372_m150_2=0;
EXPORT_SYMBOL(is_e3372_m150_2);
...
int usb_new_device(struct usb_device *udev)
{
	...
	/* HuaWei modem descriptor display errorcode, so we modify the content to make it right */
	if(le16_to_cpu(udev->descriptor.idVendor) == 0x12d1)
	{
		strcpy(udev->manufacturer,"HUAWEI");
        if (udev->serial) {
            str_len = strlen(udev->serial);

            if (str_len < 31 || strlen > 0) {
                for (i=0;i<str_len - 1;i++) {
                    if (!isalnum(udev->serial[i])){
                        udev->serial[0] = '\0';
                        break;
                    }
                }
       ...
       /* 
        * Franklin U770 modem, 
        * Pantech UML-295 modem,
        * Alcatel L800 modem,
        * Novatel U620 modem,
        * Novatel U730 modem,
        * ZTE MF831/2 modem,
        * theses modem have router inside
        */
       if ((le16_to_cpu(udev->descriptor.idVendor) == 0x10a9 && le16_to_cpu(udev->descriptor.idProduct) == 0x6064) ||
           (le16_to_cpu(udev->descriptor.idVendor) == 0x1bbb && le16_to_cpu(udev->descriptor.idProduct) == 0x0195) ||
           (le16_to_cpu(udev->descriptor.idVendor) == 0x1410 && le16_to_cpu(udev->descriptor.idProduct) == 0x9022) ||
           (le16_to_cpu(udev->descriptor.idVendor) == 0x1410 && le16_to_cpu(udev->descriptor.idProduct) == 0x9032) ||
           (le16_to_cpu(udev->descriptor.idVendor) == 0x1bbb && le16_to_cpu(udev->descriptor.idProduct) == 0xf000) ||
           (le16_to_cpu(udev->descriptor.idVendor) == 0x19d2 && le16_to_cpu(udev->descriptor.idProduct) == 0x1408) ||
           (le16_to_cpu(udev->descriptor.idVendor) == 0x19d2 && le16_to_cpu(udev->descriptor.idProduct) == 0x1405) ||
           (le16_to_cpu(udev->descriptor.idVendor) == 0x1fac && le16_to_cpu(udev->descriptor.idProduct) == 0x0232) ||
           (le16_to_cpu(udev->descriptor.idVendor) == 0x1fac && le16_to_cpu(udev->descriptor.idProduct) == 0x0233)) {
               is_modem_a_router = 1;
       }
       
       /*
        * For 341U the dns server has different IP than gateway
        * Need to handle this special case
        */
       if ((le16_to_cpu(udev->descriptor.idVendor) == 0x1199 && le16_to_cpu(udev->descriptor.idProduct) == 0x9055)) {
               is_modem_a_router = 1;
               to_learn_dns = 1;
       }
 
       /*
        * For E3372:MegaFon M150-2 
        */
       if ((le16_to_cpu(udev->descriptor.idVendor) == 0x12d1 && le16_to_cpu(udev->descriptor.idProduct) == 0x14fe)) {
           // Same preIDs and postIDs shared by E3276-150, E3372, E352S-5, need more if to judge.
           // New string "HUAWEI_MOBILE" used for E3372,retlaced "HUAWEI MOBILE" for E3276-150 & E352S-5
           if (!strcmp(udev->product, "HUAWEI_MOBILE")) {
               is_e3372_m150_2 = 1;
           }
       }
