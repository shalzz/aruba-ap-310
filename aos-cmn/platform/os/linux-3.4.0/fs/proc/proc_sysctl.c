...
#ifdef CONFIG_ARUBA_AP
    if (head) {
        struct ctl_table_set *set = head->set;
        if (!set && !head->unregistering) {
            printk( KERN_WARNING "%s:%d set is null for control table %s(%d)\n", __func__, __LINE__, str, len );
        }
    }
#endif
...
#ifdef CONFIG_ARUBA_AP
#define aruba_kern_addr_valid(kaddr) ((unsigned long)(kaddr) >= 0xc0000000 && (unsigned long)(kaddr) < 0xe0000000) 
#endif
