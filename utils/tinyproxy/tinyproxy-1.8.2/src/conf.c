...
#ifdef __FAT_AP__
static HANDLE_FUNC (handle_maxconnperclient);
static HANDLE_FUNC (handle_serverfailthrough);
static HANDLE_FUNC (handle_serveroffload);
static HANDLE_FUNC (handle_preventframeoverlay);
static HANDLE_FUNC (handle_useredirect);
static HANDLE_FUNC (handle_usehttps);
static HANDLE_FUNC (handle_switchip);
static HANDLE_FUNC (handle_autowhitelist);
static HANDLE_FUNC (handle_authhost);
static HANDLE_FUNC (handle_whitelist);
static HANDLE_FUNC (handle_blacklist);
static HANDLE_FUNC (handle_profile);
static HANDLE_FUNC (handle_profile_start);
static HANDLE_FUNC (handle_profile_end);
static HANDLE_FUNC (handle_redirecturl);
static HANDLE_FUNC (handle_oosurl);
static HANDLE_FUNC (handle_apmac);
#endif
...
static HANDLE_FUNC (handle_apname);
static HANDLE_FUNC (handle_vcname);
static HANDLE_FUNC (handle_auth_url);
static HANDLE_FUNC (handle_login_success_string);
...
ecp_cfg_t *ecp_cfg = NULL;
ecp_proxy_info_t *ecp_proxy_info = NULL;
...
struct {
        ...
        STDCONF ("apname", STR, handle_apname),
        STDCONF ("vcname", STR, handle_vcname),
        STDCONF ("authurl", STR, handle_auth_url),
        STDCONF ("loginsuccessstring", STR, handle_login_success_string),
#ifdef __FAT_AP__
        STDCONF ("redirecturl", STR, handle_redirecturl),
        STDCONF ("oosurl", STR, handle_oosurl),
        STDCONF ("apmac", STR, handle_apmac),
#endif
        ...
        STDCONF ("maxconnperclient", INT, handle_maxconnperclient),
	...
	/* auth host */
        {
                BEGIN "(authhost)" WS "(" IP "|" ALNUM ")" ":" INT "(" WS STR
                      ")?" END, handle_authhost, NULL
        },
        STDCONF ("serverfailthrough", BOOL, handle_serverfailthrough),
        STDCONF ("serveroffload", BOOL, handle_serveroffload),
        STDCONF ("preventframeoverlay", BOOL, handle_preventframeoverlay),
        STDCONF ("useredirect", BOOL, handle_useredirect),
        STDCONF ("usehttps", BOOL, handle_usehttps),
        STDCONF ("switchip", BOOL, handle_switchip),
        STDCONF ("autowhitelist", BOOL, handle_autowhitelist),
        STDCONF ("whitelist", STR, handle_whitelist),
        STDCONF ("blacklist", STR, handle_blacklist),
        STDCONF ("profile", STR, handle_profile),
        {
                BEGIN "\\{", handle_profile_start, NULL
        },
        {
                BEGIN "\\}", handle_profile_end, NULL
        },
#endif
        /* loglevel */
...
#ifdef __FAT_AP__
//struct upstream *current_upstream = NULL;
struct upstream * current_prof = NULL;
#endif
...
static void free_config (struct config_s *conf)
{
        ...
        safefree (conf->ap_name);
        safefree (conf->vc_name);
        safefree (conf->ap_mac);
        free_upstream_list (conf->auth_host);
...
static int config_parse (struct config_s *conf, FILE * f)
{
        ...
        while (fgets (buffer, sizeof (buffer), f)) {
                ...
                if (check_match (conf, buffer)) {
                        log_message (LOG_ERR, "Syntax error on line %ld: '%s'\n", lineno, buffer);
                }
                ++lineno;
        }
        return 0;
...
#ifdef __FAT_AP__
        flock(fileno(config_file), LOCK_EX);
#endif
                                      ...
                                      struct config_s *defaults)
{
        ...
        if (defaults->ap_name) {
                conf->ap_name = encode_str_dup (defaults->ap_name);
        }

        if (defaults->ap_mac) {
                conf->ap_mac = encode_str_dup (defaults->ap_mac);
        }

        if (defaults->vc_name) {
                conf->vc_name = encode_str_dup (defaults->vc_name);
        }
...
#ifdef __FAT_AP__
    if (ecp_cfg) {
        /* Copy config to shared memory and set up reverse linkage */
        int i = 0;
        struct upstream *up;

        if (!is_child) {
            memset(ecp_cfg, 0, sizeof(*ecp_cfg));
            if (conf->ap_name) {
                strlcpy(ecp_cfg->ap_name, conf->ap_name, sizeof(ecp_cfg->ap_name));
            }

            if (conf->vc_name) {
                strlcpy(ecp_cfg->vc_name, conf->vc_name, sizeof(ecp_cfg->vc_name));
            }
            if (conf->ap_mac) {
                strlcpy(ecp_cfg->ap_mac, conf->ap_mac, sizeof(ecp_cfg->ap_mac));
            }
        }

        for (up = conf->auth_host; up && i < 16; up = up->next) {
            ecp_server_cfg_t *ecp_server_cfg = &(ecp_cfg->ecp_server[i]);
            up->query_in_auth_url = strchr(up->auth_url, '?') != NULL ? 1 : 0;
            if (!up->useredirect) {
                up->ecp_server_cfg = NULL;
                continue;
            }
            if (!is_child) {
                strlcpy(ecp_server_cfg->name, up->domain, sizeof(ecp_server_cfg->name));
                strlcpy(ecp_server_cfg->server, up->host, sizeof(ecp_server_cfg->server));
                ecp_server_cfg->port = up->port;
                strlcpy(ecp_server_cfg->auth_url, up->auth_url, sizeof(ecp_server_cfg->auth_url));
                strlcpy(ecp_server_cfg->oos_url, up->oosurl, sizeof(ecp_server_cfg->oos_url));
                ecp_server_cfg->query_in_auth_url = up->query_in_auth_url;
                ecp_server_cfg->use_https = up->usehttps;
                ecp_server_cfg->switch_ip = up->switchip;
                ecp_server_cfg->server_failthrough = up->serverfailthrough;
                ecp_server_cfg->cp_offload = up->serveroffload;
                ecp_server_cfg->prevent_frame_overlay = up->preventframeoverlay;
                strlcpy(ecp_server_cfg->redirect_url, up->redirecturl ? up->redirecturl : "", sizeof(ecp_server_cfg->redirect_url));
            }
            up->ecp_server_cfg = ecp_server_cfg;
            i++;
        }
    }
#endif
...
{
        char *tmp = NULL;
        int r = set_string_arg (&tmp, line, &match[2]);

        if (r)
                return r;
        conf->ap_name = encode_str_dup(tmp);
        safefree(tmp);
        log_message (LOG_INFO,
                     "Setting \"AP\" Name to '%s'",
                     conf->ap_name);
        return 0;
}
...
static HANDLE_FUNC (handle_apmac)
{
        ...
        if (r)
                return r;
        conf->ap_mac = encode_str_dup(tmp);
        safefree(tmp);
        log_message (LOG_INFO,
                     "Setting \"AP\" MAC '%s'",
                     conf->ap_mac);
...
static HANDLE_FUNC (handle_vcname)
{
        ...
        if (r)
                return r;
        conf->vc_name = encode_str_dup(tmp);
        safefree(tmp);
        log_message (LOG_INFO,
                     "Setting \"VC\" Name to '%s'",
                     conf->vc_name);
        return 0;
}
...
static HANDLE_FUNC (handle_maxconnperclient)
{
        child_configure (CHILD_MAXCONNPERCLIENT, get_long_arg (line, &match[2]));
        return 0;
}

struct upstream * profile_find (char *profile, struct upstream *list)
{
    struct upstream *tmp;

    tmp = upstream_get(profile, list);
    if (!tmp) {
        error_log("tinyproxy", "%s: %d: could not find profile='%s'",
            __func__, __LINE__, profile);
        return NULL;
    }
    if (!tmp->domain || strcmp(tmp->domain, profile) != 0) {
        error_log("tinyproxy", "%s: %d: found incorrect profile, expected '%s' got '%s'",
            __func__, __LINE__, profile, tmp->domain);
        return NULL;
    }
    return tmp;
}
...
static HANDLE_FUNC (handle_profile)
{
    ...
    r = set_string_arg (&prof_name, line, &match[2]);
    if (r)
        return r;
    upstream_add ("localhost", 80, prof_name, &conf->auth_host);
    current_prof = profile_find (prof_name, conf->auth_host);
    assert(current_prof != NULL);
    safefree(prof_name);
    debug_log("tinyproxy", "Profile name is '%s'", current_prof != NULL ? current_prof->domain : "(NULL)");
    return 0;
}
...
{
        if (!current_prof) {
            error_log("tinyproxy", "%s: %d: Not in an active profile", __func__, __LINE__);
            return 0;
        }

        current_prof->host = get_string_arg (line, &match[2]);
        if (!current_prof->host)
                return -1;
        current_prof->port = (int) get_long_arg (line, &match[7]);

#if 0
        system("echo \"authhost\" > //tmp//tinyproxy");
	    wg_add_wl(current_prof->host);
        auto_wg_add_wl(current_prof->host);
#endif
        return 0;
}

#define DEF_PROFILE_FUNC(field, type, match_pos, format)  \
static HANDLE_FUNC (handle_ ## field) \
{ \
    int r; \
\
    if (!current_prof) { \
    ...
    r = set_ ## type ## _arg (&current_prof->field, line, &match[match_pos]); \
    if (r) { \
        error_log("tinyproxy", "%s: %d: parse error %d on line '%s'", \
            __func__, __LINE__, r, line); \
        return r; \
    } \
    debug_log("tinyproxy", "%s: %d: value for profile '%s' set to '" format "'", \
        __func__, __LINE__, current_prof->domain, current_prof->field); \
    return 0; \
}

DEF_PROFILE_FUNC(useredirect, bool, 2, "%u")
DEF_PROFILE_FUNC(usehttps, bool, 2, "%u")
DEF_PROFILE_FUNC(autowhitelist, bool, 2, "%u")
DEF_PROFILE_FUNC(serverfailthrough, bool, 2, "%u")
DEF_PROFILE_FUNC(serveroffload, bool, 2, "%u")
DEF_PROFILE_FUNC(preventframeoverlay, bool, 2, "%u")
DEF_PROFILE_FUNC(switchip, bool, 2, "%u")
DEF_PROFILE_FUNC(auth_url, string, 2, "%s")
DEF_PROFILE_FUNC(login_success_string, string, 2, "%s")
DEF_PROFILE_FUNC(redirecturl, string, 2, "%s")
DEF_PROFILE_FUNC(oosurl, string, 2, "%s")
#endif
...
static HANDLE_FUNC (handle_upstream)
{
        ...
        if (match[10].rm_so != -1) {
                ...
                if (domain) {
#ifdef __FAT_AP__
                        upstream_add (ip, port, domain, NULL, &conf->upstream_list);
#else
...
#ifdef __FAT_AP__
                upstream_add (ip, port, NULL, NULL, &conf->upstream_list);
#else
...
#ifdef __FAT_AP__
        upstream_add (NULL, 0, domain, NULL, &conf->upstream_list);
#else
