...
#ifdef __FAT_AP__
        up->auth_url = safestrdup("/");
        up->oosurl = safestrdup("/");
        up->login_success_string = NULL;
        up->redirecturl = NULL;
#endif
        if (domain == NULL) {
                   ...
                   struct upstream **upstream_list)
{
        ...
        if (up == NULL) {
                log_message (LOG_ERR, "build upstream %s fail", domain);
                return;
        }
...
void free_upstream_list (struct upstream *up)
{
        while (up) {
                ...
                safefree (tmp->host);
#ifdef __FAT_AP__
                safefree (tmp->auth_url);
                safefree (tmp->oosurl);
                safefree (tmp->login_success_string);
                safefree (tmp->redirecturl);
#endif
                safefree (tmp);
        }
}
