...
short int child_configure (child_config_t type, unsigned int val)
{
        ...
        case CHILD_MAXREQUESTSPERCHILD:
                ...
                break;
        case CHILD_MAXCONNPERCLIENT:
                child_config.maxconnperclient = val;
                break;
        default:
 ...
 */
#ifdef __FAT_AP__
static void child_sigchld_handler (int sig)
{
    if (sig == SIGCHLD) {
        int stat;
...
#ifdef __FAT_AP__
                SERVER_DEC ();
                if (child_received_sighup) {
                    reload_config ();
#ifdef FILTER_ENABLE
                    filter_reload ();
#endif /* FILTER_ENABLE */
                    child_received_sighup = FALSE;
                }
#endif
...
static pid_t child_make (struct child_s *ptr)
{
         ...
         */
#ifdef __FAT_AP__
        set_signal_handler (SIGCHLD, child_sigchld_handler);
#else
        ...
        while (1) {
...
#ifdef __FAT_AP__
            {
                ...
                for (i = 0; i < SWARM_ECP_PROXY_NUMBER; i ++){
                    ...
                    if (client->pid){
                        for (j = 0; j < child_config.maxclients; j++) {
                            if (child_ptr[j].status != T_EMPTY && client->pid == child_ptr[j].tid) {
                                break;
                            }
                        }
                        if (j == child_config.maxclients ){
                            log_message(LOG_INFO, "%s, %d: Remove client sport %d", __func__, __LINE__, client->sport);
                            memset(client, 0, sizeof(ecp_proxy_t));
                        }
                    }
                }
                log_message(LOG_INFO, "%s, %d: current entry is %d", __func__, __LINE__, ecp_proxy_number());
           }
#endif
