...
{
#ifdef __FAT_AP__
        return opensock_w_timeout("tinyproxy", host, port, bind_to, config.bind_address);
#else
        int sockfd, n;
        struct addrinfo hints, *res, *ressave;
        char portstr[6];
        ...
        do {
                ...
                if (bind_to) {
                        if (bind_socket (sockfd, bind_to,
                                         res->ai_family) < 0) {
                                error_log("tinyproxy", "%s: %d: failed to binding socket %d", __func__, __LINE__, sockfd);
                                close (sockfd);
                        ...
                        if (bind_socket (sockfd, config.bind_address,
                                         res->ai_family) < 0) {
                                error_log("tinyproxy", "%s: %d: failed to binding address", __func__, __LINE__);
                                close (sockfd);
                                continue;       /* can't bind, so try again */
                        }
                }
 ...
 */
int getsock_ip (int fd, char *ipaddr, int * port)
{
        ...
        if (get_ip_string ((struct sockaddr *) &name, ipaddr, IP_LENGTH, port) ==
            NULL)
                return -1;
 ...
 */
int getpeer_information (int fd, char *ipaddr, char *string_addr, int * port)
{
        ...
        if (get_ip_string ((struct sockaddr *) &sa, ipaddr, IP_LENGTH, port) == NULL)
                return -1;

#ifdef __FAT_AP__
        strcpy(string_addr, ipaddr);
        return 0;
#else
