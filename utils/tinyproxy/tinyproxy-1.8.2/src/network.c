...
#ifdef __FAT_AP__
ssize_t recv_w_timeout(int s, void *buf, size_t len, int flags, int timeout)
{
    fd_set rfds;
    struct timeval tv;
    int ret;
    ...
    if (timeout > 0) {
        ...
        if (ret == 0) {
            ...
            log_message (LOG_INFO,
                    "%s: %d: read timed out", __func__, __LINE__);
            return 0;
        }
    }
...
ssize_t read_w_timeout(int s, void *buf, size_t count, int timeout)
{
    fd_set rfds;
    struct timeval tv;
    int ret = 0;
    ...
    if (timeout > 0) {
        ...
        if (ret == 0) {
            /* timed out */
            debug_log("tinyproxy", "%s: %d: read timed out",
                __func__, __LINE__);
            return 0;
        }
    }
    do {
        len = read (s, buf, count);
    } while (len < 0 && errno == EINTR);
...
#ifdef __FAT_AP__
ssize_t readline_w_timeout (int fd, char **whole_buffer, int timeout)
#else
...
#ifdef __FAT_AP__
                ret = recv_w_timeout (fd, buffer, SEGMENT_LEN, MSG_PEEK, timeout);
#else
 ...
 * hex string.
 */
char *get_ip_string (struct sockaddr *sa, char *buf, size_t buflen, int * port)
{
        assert (sa != NULL);
        assert (buf != NULL);
        assert (buflen != 0);
