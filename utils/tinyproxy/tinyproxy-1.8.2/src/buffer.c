...
int insert_into_line(struct bufline_s *line, struct buffer_s *buf, unsigned int pos, const char *insert_str)
{
    unsigned char *new_buf;
    int   insert_len;
    int   suffix_len;
    int   new_len;
    if (!line || !line->string || !insert_str) {
        return -1;
    } 
    if (line->length <= pos) {
        return -2;
    ...
    new_len = line->length + insert_len;
    new_buf = (unsigned char *) safemalloc(new_len + 1);
    if (!new_buf) {
        return -3;
    }
    if (pos) {
        memcpy(new_buf, line->string, pos);
    }
    memcpy(new_buf + pos, insert_str, insert_len);
    suffix_len = line->length - pos;
    if (suffix_len) {
        memcpy(new_buf + pos + insert_len, line->string + pos, suffix_len);
    }
    new_buf[new_len] = 0;
    buf->size += insert_len;
...
static struct bufline_s *makenewline (unsigned char *data, size_t length)
{
        ...
        if (!newline)
                return NULL;
#ifdef __FAT_AP__
        newline->string = (unsigned char *) safemalloc (length + 1);
#else
...
token_state_rule_t state_rule[][2] = {
    /* State INIT */
    {
        {LEN1, FILTER},   /* Default */
        {INIT, ERROR}   /* New line */
    ...
    /* State LINE */
    {
        {LINE, NO_FILTER},      /* Default */
        {LINE_SEP, NO_FILTER},  /* New line */
    },

    /* State LINE_SEP */
    {
        {LINE_SEP, ERROR},      /* Default */
        {INIT, FILTER}              /* new line */
    }
};

static inline ssize_t _read_buffer (int fd, struct buffer_s * buffptr, char filter_chunked, int *token_state)
#else
...
#endif
{
        ...
        if (bytesin > 0) {
            ...
            if (filter_chunked) {
                int i;
                unsigned char *buffer1 = (unsigned char *) safemalloc (bytesin);
                unsigned char *ptr = buffer1;
                if (ptr) {
                    for (i=0; i<bytesin; i++) {
                        char c = buffer[i];
                        int old_state;
                        token_state_rule_t *rule;
                        if (c == '\r') {
                            continue;
                        }
                        old_state = *token_state;
                        rule = &(state_rule[*token_state][(c == '\n')? 1: 0]);
                        *token_state = rule->new_state;
                        switch (rule->filter_char) {
                            case FILTER:
                                continue;
                            case ERROR:
                                error_log("tinyproxy", "%s: %d: encountered illegal char '%c' in state %d",
                                    __func__, __LINE__, c, old_state);
                        }
                        if (c == '\n') {
                            *ptr = '\r';
                            ptr++;
...
ssize_t read_buffer (int fd, struct buffer_s * buffptr)
{
    return _read_buffer(fd, buffptr, 0, NULL);
}

ssize_t read_buffer_from_server (int fd, struct buffer_s * buffptr, char filter_chunk_encoding, int *token_state)
{
    return _read_buffer(fd, buffptr, filter_chunk_encoding, token_state);
}
#endif
/*
