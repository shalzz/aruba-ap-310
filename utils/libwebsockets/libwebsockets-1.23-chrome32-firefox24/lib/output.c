...
#define SSL_MAX_CONTENT_LEN 16384
        ...
        while (offset < len) {
            if (len - offset > SSL_MAX_CONTENT_LEN)
                reclen = SSL_MAX_CONTENT_LEN;
            else
                reclen = len - offset;
            while ( (ret = SSL_write ( wsi->ssl, &buf[offset], reclen)) <= 0 ) {
                tries++;
                
                lwsl_debug("SSL_Write returned=%d errcode=%d errstr='%s' tries=%d",
                            ret, SSL_get_error( wsi->ssl, ret ), strerror(errno), tries);
                if (ret == 0 
#ifdef USE_CYASSL
                        || ret == SOCKET_ERROR_E
#endif
                ...
                if (tries >= 25) {
                    lwsl_debug("Writing on SSL connection failed even after %d tries, Error code: %d",
                            tries, SSL_get_error( wsi->ssl, ret ) );
			        return -1;
                }
            }
		...
		if (n < 0) {
            ...
            lwsl_debug("ERROR writing to socket, n %d, errcode=%d errstr='%s', buf %p, len %d.\n", 
                    n, SSL_get_error(wsi->ssl, n), strerror(errno), buf, len);
			return -1;
		}
#endif
	} else {
						       ...
						       const char *content_type)
{
	...
	if (wsi->u.http.fd < 1) {
		...
		p += sprintf((char *)p,
		 "HTTP/1.1 400 Bad\x0d\x0aServer: libwebsockets\x0d\x0a\x0d\x0a"
		);
		wsi->u.http.fd = 0;
	...
	p += sprintf((char *)p,
"HTTP/1.1 200 OK\x0d\x0aServer: libwebsockets\x0d\x0a""Content-Type: %s\x0d\x0a",
								  content_type);
	p += sprintf((char *)p,
