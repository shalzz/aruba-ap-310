				...
				struct libwebsocket *wsi, struct pollfd *pollfd)
{
	...
	case LWS_CONNMODE_WS_CLIENT_ISSUE_HANDSHAKE:
		...
		if (wsi->use_ssl && !wsi->ssl) {
            ...
            /* get hostname for support SNI (bug 195532)*/
            const char *hostname = lws_hdr_simple_ptr(wsi, _WSI_TOKEN_CLIENT_PEER_ADDRESS);

			wsi->ssl = SSL_new(context->ssl_client_ctx);

            /* Enable SNI (bug 195532)*/
#ifdef USE_CYASSL
		...
		if (wsi->use_ssl) {
			...
			if (n < 0) {
				...
				if (n == SSL_ERROR_WANT_READ ||
                    ...
                    /*
                     * Bug 174662
                     * An enhancement. On handshake, if SSL_ERROR_WANT_READ, no need to write.
                     * Only SSL_ERROR_WANT_WRITE trigger libwebsocket_callback_on_writable
                     */
                    if (n == SSL_ERROR_WANT_WRITE) {
                        lwsl_info("SSL_connect WANT_WRITEd\n");
					    libwebsocket_callback_on_writable(
	                                  context, wsi);
					}

					wsi->mode = LWS_CONNMODE_WS_CLIENT_WAITING_SSL;
					lwsl_debug("Connect mode=LWS_CONNMODE_WS_CLIENT_WAITING_SSL\n");
					return 0; /* no error */
				}
				n = -1;
			}
	...
	case LWS_CONNMODE_WS_CLIENT_WAITING_SSL:
			...
			if (wsi->mode == LWS_CONNMODE_WS_CLIENT_WAITING_SSL) {
				...
				if (n < 0) {
					...
					if (n == SSL_ERROR_WANT_READ ||
                         ...
                         */
                        if (n == SSL_ERROR_WANT_WRITE) {
                            lwsl_info("SSL_connect WANT_WRITE\n");
						    libwebsocket_callback_on_writable(
							      							  context, wsi);
						}
						wsi->mode = LWS_CONNMODE_WS_CLIENT_WAITING_SSL;
						lwsl_debug("Connect mode=LWS_CONNMODE_WS_CLIENT_WAITING_SSL retrying\n");
						return 0; /* no error */
					}
					n = -1;
				}
		...
		lwsl_debug("Connect mode=LWS_CONNMODE_WS_CLIENT_WAITING_SERVER_REPLY\n");
...
check_accept:
	...
	lwsl_debug("Connect mode=LWS_CONNMODE_WS_CLIENT\n");
	/* union transition */
	...
	if (setsockopt(wsi->sock, SOL_SOCKET, SO_RCVBUF,  &n, sizeof(n))) {
		lwsl_warn("Failed to set SO_RCVBUF to %d", n);
		goto bail3;
	}
...
bail2:
	WSC_ERROR(AWC_HANDSHACK_ERROR, NULL);
	if (wsi->protocol)
		wsi->protocol->callback(context, wsi,
			LWS_CALLBACK_CLIENT_CONNECTION_ERROR,
						      wsi->user_space, wsc_err_str, sizeof(wsc_err_str));
	WSC_ERROR_CLEAR();
