...
struct libwebsocket *libwebsocket_client_connect_2(
	...
	if (context->http_proxy_port) {
        if (context->http_proxy_username[0] == '\0'){
		    plen = sprintf((char *)context->service_buffer,
			    "CONNECT %s:%u HTTP/1.0\x0d\x0a"
			    "User-agent: libwebsockets\x0d\x0a"
                /*Proxy-authorization: basic aGVsbG86d29ybGQ= */
			    "\x0d\x0a",
			    lws_hdr_simple_ptr(wsi, _WSI_TOKEN_CLIENT_PEER_ADDRESS), wsi->u.hdr.ah->c_port);
		        ads = context->http_proxy_address;
		        server_addr.sin_port = htons(context->http_proxy_port);
        } else {
            char auth[512] = {0};
            char auth_tmp[512] = {0};
            snprintf(auth_tmp, sizeof(auth_tmp), "%s:%s", context->http_proxy_username, context->http_proxy_password);
            lws_b64_encode_string(auth_tmp, sizeof(auth), auth,sizeof(auth));
             
            lwsl_info("Send Proxy-authorization");
            plen = sprintf((char *)context->service_buffer,
                    "CONNECT %s:%u HTTP/1.0\x0d\x0a"
                    "User-agent: libwebsockets\x0d\x0a"
                    /*Proxy-authorization: basic aGVsbG86d29ybGQ= */
                    ...
                    "\x0d\x0a",
                    lws_hdr_simple_ptr(wsi, _WSI_TOKEN_CLIENT_PEER_ADDRESS), wsi->u.hdr.ah->c_port, auth);
		    ads = context->http_proxy_address;
		    server_addr.sin_port = htons(context->http_proxy_port);
        }
	} else {
	...
	server_hostent = gethostbyname_with_best_effort(ads);
	if (server_hostent == NULL) {
		lwsl_err("Unable to get host name from %s\n", ads);
                /* BUG117778 When mode is LWS_CONNMODE_WS_CLIENT_WAITING_CONNECT, we should close session */
                WSC_ERROR(AWC_DNS_ERROR, NULL);
	...
	if (wsi->sock < 0) {
		...
		if (wsi->sock < 0) {
			...
			WSC_ERROR(AWC_SOCKET_ERROR, NULL);
			goto oom4;
		}
		...
		lwsl_debug("Connect mode=LWS_CONNMODE_WS_CLIENT_WAITING_CONNECT\n");
	...
	if (context->http_proxy_port) {
		...
		if (n < 0) {
			WSC_ERROR(AWC_PROXY_ERROR, NULL);
		...
		lwsl_debug("Connect mode=LWS_CONNMODE_WS_CLIENT_WAITING_PROXY_REPLY\n");
	...
	lwsl_debug("Connect mode=LWS_CONNMODE_WS_CLIENT_ISSUE_HANDSHAKE\n");
	pfd.fd = wsi->sock;
	pfd.revents = POLLIN;
			      ...
			      int ietf_version_or_minus_one)
{
	...
	if (handled) {
		...
		lwsl_debug("Connect mode=LWS_CONNMODE_WS_CLIENT_WAITING_EXTENSION_CONNECT\n");
		return wsi;
	}
#endif
