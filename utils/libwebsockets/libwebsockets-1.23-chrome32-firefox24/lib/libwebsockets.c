...
awc_error_t wsc_err = AWC_NO_ERROR;
char wsc_err_str[256] = "";

char * wsc_connect_fail_to_str(int code) 
{
    switch(code) {
        case AWC_DNS_ERROR:
            return "dns error";
        case AWC_SOCKET_ERROR:
            return "socket error";
        case AWC_CONNECT_ERROR:
            return "tcp connect error";
        case AWC_PROXY_ERROR:
            return "HTTPS proxy error";
        case AWC_SSL_ERROR:
            return "SSL create error";
        case AWC_SSL_CONNECT_ERROR:
            return "SSL connect error";
        case AWC_SSL_CERT_VERIFICATION_ERROR:
            return "SSL cert verify error";
        case AWC_SSL_CERT_CN_NAME_ERROR:
            return "SSL cert cn name error";
        case AWC_SSL_TPM_CLIENTAUTH_ERROR:
            return "SSL tpm cert error";
        case AWC_SSL_CACERT_ERROR:
            return "SSL ca cert error";
        case AWC_HANDSHACK_ERROR:
            return "Handshack error";
        default:
            return "Unknow error";
    }
    return "Unknow error";
...
#define AWC_CUSTOMER_CACERT_PATH "/aruba/conf/CUST_CA"

void
WSC_ERROR(awc_error_t arg, const char *str)
{
    wsc_err = arg; 
    strlcpy(wsc_err_str, str ? str : wsc_connect_fail_to_str(arg), sizeof(wsc_err_str)); 
}

void
WSC_ERROR_CLEAR()
{
    wsc_err = AWC_NO_ERROR;
    wsc_err_str[0] = 0;
}

void WSC_ERROR_GET(char *str, int len)
{
    strlcpy(str, wsc_err_str, len);
}
...
{
	int m;
        /* BUG117778 We should check whether fds_count is zero firstly, then decrease it */
			 ...
			 struct libwebsocket *wsi, enum lws_close_status reason)
{
	...
	if (wsi->mode == LWS_CONNMODE_WS_CLIENT_WAITING_CONNECT ||
		...
		if (wsi->mode == LWS_CONNMODE_WS_CLIENT_WAITING_CONNECT)
			WSC_ERROR(AWC_CONNECT_ERROR, NULL);
		if (wsi->mode == LWS_CONNMODE_WS_CLIENT_ISSUE_HANDSHAKE)
			WSC_ERROR(AWC_SSL_ERROR, NULL);

		context->protocols[0].callback(context, wsi,
			LWS_CALLBACK_CLIENT_CONNECTION_ERROR, NULL, wsc_err_str, sizeof(wsc_err_str));
		WSC_ERROR_CLEAR();
...
libwebsocket_create_context(struct lws_context_creation_info *info)
{
	...
	context->http_proxy_username[0] = '\0';
	context->http_proxy_password[0] = '\0';
	...
	/* split the proxy <ads:port> or <ads:port,username:password> if given */
	...
	if (p) {
        ...
        tmp = strstr(p, ",");
        if (!tmp){
		    strncpy(context->http_proxy_address, p, sizeof(context->http_proxy_address) - 1);
	    	context->http_proxy_address[sizeof(context->http_proxy_address) - 1] = '\0';
	    	...
	    	*p = '\0';
		    ...
		    lwsl_notice(" Proxy %s:%u\n", context->http_proxy_address, context->http_proxy_port);
        } else {
            tmp[0] = '\0';
            strncpy(context->http_proxy_address, p, sizeof(context->http_proxy_address) - 1);
            context->http_proxy_address[sizeof(context->http_proxy_address) - 1] = '\0';
            ...
            *p = '\0';
            ...
            strncpy(context->http_proxy_username, ++tmp, sizeof(context->http_proxy_username) - 1);
            context->http_proxy_username[sizeof(context->http_proxy_username) - 1] = '\0';

            tmp = strchr(context->http_proxy_username, ':');
            if (tmp == NULL) {
                lwsl_err("http_proxy needs to be username:password\n");
                goto bail;
            }
            *tmp = '\0';
            strncpy(context->http_proxy_password, ++tmp, sizeof(context->http_proxy_password) - 1);
            lwsl_notice(" Proxy %s:%u,username:password\n", context->http_proxy_address, context->http_proxy_port);
