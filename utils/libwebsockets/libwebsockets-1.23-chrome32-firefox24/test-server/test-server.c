					       ...
					       void *user, void *in, size_t len)
{
	...
	case LWS_CALLBACK_SERVER_WRITEABLE:
		n = sprintf((char *)p, "%s(%d) %d", __func__, __LINE__, pss->number++);
	...
	case LWS_CALLBACK_RECEIVE:
		fprintf(stderr, "%s(%d) rx len %d, %s\n", __func__, __LINE__, (int)len, (const char *)in);
		if (len < 6)
			break;
					       ...
					       void *user, void *in, size_t len)
{
	...
	case LWS_CALLBACK_SERVER_WRITEABLE:
		...
		while (pss->ringbuffer_tail != ringbuffer_head) {
            ...
            fprintf(stderr, "%s(%d) write %s to client\n", __func__, __LINE__, 
	...
	case LWS_CALLBACK_RECEIVE:
        ...
        fprintf(stderr, "%s(%d) receive %s from client\n", __func__, __LINE__, (char *)in);
...
struct per_session_data_zgj_test {
	int number;
};
...
{
	int n;
	struct per_session_data_zgj_test *pss = (struct per_session_data_zgj_test *)user;
	...
	case LWS_CALLBACK_SERVER_WRITEABLE:
        sprintf((char *)(buf + LWS_SEND_BUFFER_PRE_PADDING), "%d", pss->number);
			n = libwebsocket_write(wsi, buf,
                    strlen((char *)(buf + LWS_SEND_BUFFER_PRE_PADDING)),
					LWS_WRITE_TEXT);
...
static struct libwebsocket_protocols protocols[] = {
	...
	},
	{
		"central-protocol",
		callback_zgj_test,
		sizeof(struct per_session_data_zgj_test),
		128,
	},
