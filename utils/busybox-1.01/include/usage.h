...
#define ping_full_usage \
	...
	"\t-s SIZE\t\tSend SIZE data bytes in packets (default=56, minimum=0, maximum=65507).\n" \
	"\t-f SOURCE-IP\tUse the source ip address to send ping.\n" \
	"\t-Q NUM\t\tSets IP TOS byte to NUM (0 thru 254)\n" \
	"\t-I IFACE\tSets Interface\n" \
...
#define ping6_full_usage \
	...
	"Options:\n" \
	"\t-c COUNT\tSend only COUNT pings.\n" \
	"\t-s SIZE\t\tSend SIZE data bytes in packets (default=56, minimum=0, maximum=65527).\n" \
	"\t-q\t\tQuiet mode, only displays output at start\n" \
	"\t\t\tand when finished."
#endif
...
#define telnet_full_usage \
	...
	"\tHOST\t\tThe official name, alias or the IPv4/IPv6 address of the\n" \
	"\t\t\tremote host.\n" \
...
#ifdef __FAT_AP__
 #define USAGE_TFTP_INTF(a) a
#else
 #define USAGE_TFTP_INTF(a)    
#endif
...
#define top_trivial_usage \
	"[-d <seconds>] [-n <iterations>] -N"
#else
#define top_trivial_usage \
	"[-d <seconds>] [-n <iterations>]"
#endif
