...
static void snprint_ip_port(char *ip_port, int size, struct sockaddr *addr, int port, char *proto, int numeric)
{
    ...
    /*for fix bug146884, for AP platform, get it at least can fully display ipv4->ipv6 mapped address "::ffff:xxx.xxx.xxx.xxx:yyyyy"*/
	if ((strlen(ip_port) + strlen(port_name)) > 27)
		ip_port[27 - strlen(port_name)] = '\0';
#else
...
#if defined(AP_PLATFORM)
		printf("tcp   %6ld %6ld %-28s %-28s %-12s\n",
...
#if defined(AP_PLATFORM)
		printf("udp   %6ld %6ld %-28s %-28s %-12s\n",
			   rxq, txq, local_addr, rem_addr, state_str);
#else
...
#if defined(AP_PLATFORM)
		printf("raw   %6ld %6ld %-28s %-28s %-12s\n",
			   rxq, txq, local_addr, rem_addr, state_str);
#else
