...
#ifndef  __FAT_AP__
static int resolve_hostname(char *hostname, char *ipaddr, char ipv4, char ipv6);
#endif
...
static int subneg(byte c)
{
	...
	case TS_SUB1:
		...
		if (c == TELOPT_TTYPE && ttype)
			putiac_subopt(TELOPT_TTYPE,ttype);
#endif
#ifdef CONFIG_FEATURE_TELNET_AUTOLOGIN
		else
		if (c == TELOPT_NEW_ENVIRON && autologin)
...
extern int telnet_main(int argc, char** argv)
{
    ...
    struct hostent *host;
    int remote_sys_addr_family;
    remote_sys_addr_family = 0;
    host = NULL;
    memset(&s_in, 0, sizeof(s_in));
    memset(&s_in6, 0, sizeof(s_in6));
#ifndef  __FAT_AP__
    char ipaddr[INET6_ADDRSTRLEN], *ip;
	...
	if (optind < argc) {
        ...
        host = aruba_gethostbyname(argv[optind++]);
        ...
        if (host->h_addrtype == AF_INET) {
            ...
            memcpy(&s_in6.sin6_addr, &ipv4_mapped_v6, sizeof(s_in6.sin6_addr));
        } else {
            memcpy(&s_in6.sin6_addr, host->h_addr, sizeof(s_in6.sin6_addr));
        }
		s_in6.sin6_port = bb_lookup_port((optind < argc) ? argv[optind++] :
				"telnet", "tcp", 23);
#else
        // Controller
        resolve_hostname(argv[1], ipaddr, TRUE, FALSE);
        if(strlen(ipaddr))
           ip=ipaddr;
        else
           ip=argv[1];

        if(inet_pton(AF_INET6, ip, &(s_in6.sin6_addr)) == 1)
        {
            remote_sys_addr_family = AF_INET6;
        }
        else if(inet_pton(AF_INET, ip, &(s_in.sin_addr)) == 1)
        {
            remote_sys_addr_family = AF_INET;
        }
        else
        {
            printf("Unable to connect to remote host (%s):DNS failure", argv[1]);
            exit(EXIT_FAILURE);
        }

        if(remote_sys_addr_family == AF_INET6){
            s_in6.sin6_family = AF_INET6;
            s_in6.sin6_port = bb_lookup_port((argc == 3) ? argv[2]: "telnet", "tcp", 23);
            G.netfd = socket(AF_INET6, SOCK_STREAM, 0);
            if (connect(G.netfd, (struct sockaddr *)&s_in6, sizeof(struct sockaddr_in6)) < 0)
            {
                bb_perror_msg_and_die("Unable to connect to remote host (%s)", argv[1]);
            }
        }
        else if(remote_sys_addr_family == AF_INET){
	...
	if (connect(G.netfd, (struct sockaddr *)&s_in6, sizeof(struct sockaddr_in6)) < 0)
    {
        ...
        if (inet_ntop(AF_INET6, &(s_in6.sin6_addr), str, INET6_ADDRSTRLEN) == NULL) {
            perror("inet_ntop");
            exit(EXIT_FAILURE);
        }
        bb_perror_msg_and_die("Unable to connect to remote host (%s)", str);
    }
#endif
...
static int resolve_hostname(char *hostname, char *ipaddr, char ipv4, char ipv6)
{
   struct addrinfo hints, *res = NULL, *resptr = NULL;
   char ipstr[INET_ADDRSTRLEN], ip6str[INET6_ADDRSTRLEN];
   struct sockaddr_in *ip;
   struct sockaddr_in6 *ip6;
   int result = -1;
   char ip4found = FALSE, ip6found = FALSE;
   void *tmp;

   res_init();
   if(strcmp(hostname, "localhost") && system("[ -s /etc/resolv.conf ]"))
   {
      return result;
   }
   ...
   if (result == 0)  {
      for(resptr = res; resptr; resptr=resptr->ai_next)
      {
         if(resptr->ai_family == AF_INET)
         {
            if(ip4found)
               continue;
            ip = (struct sockaddr_in *)resptr->ai_addr;
            tmp = &(ip->sin_addr);
            inet_ntop(AF_INET, tmp, ipstr, INET_ADDRSTRLEN);
            ip4found = TRUE;
         }
         else
         {
            if(ip6found)
               continue;
            ip6 = (struct sockaddr_in6 *)resptr->ai_addr;
            tmp = &(ip6->sin6_addr);
            inet_ntop(AF_INET6, tmp, ip6str, INET6_ADDRSTRLEN);
            ip6found = TRUE;
         }
      } 
      ...
      if(ipv4 && !ipv6)
      {
         // Preference for ipv4 when ipv6 is disabled.
         // We could have a situation wherein ipv4 is configured
         // and the fqdn would resolve only to ipv6, in that case
         // we store the ipv6 address.
         if(*ipstr != 0) {
             strncpy(ipaddr, ipstr, INET_ADDRSTRLEN);
         } else if(*ip6str !=0) {
             strncpy(ipaddr, ip6str, INET6_ADDRSTRLEN);
         } else {
             // No ip address to store
             freeaddrinfo(res);
             return -1;
         }
      }
      else if((!ipv4 && ipv6) || (ipv4 && ipv6))
      {
          if(*ip6str != 0) {
             strncpy(ipaddr, ip6str, INET6_ADDRSTRLEN);
          } else if(*ipstr !=0) {
