...
static int calc_length(struct dhcpMessage *payload)
{
	int payload_length;
#ifdef __FAT_AP__
        payload_length = sizeof(struct dhcpMessage) - (MAX_OPTIONS_LENGTH - 200);
#else	
		   ...
		   uint32_t dest_ip, int dest_port, uint8_t *dest_arp, int ifindex)
{
	...
	struct udp_dhcp_packet packet;
        int payload_length = calc_length(payload);
	...
	packet.udp.len = htons(sizeof(packet.udp) + payload_length); /* cheat on the psuedo-header */
		   ...
		   uint32_t dest_ip, int dest_port)
{
    ...
    strncpy(interface.ifr_ifrn.ifrn_name, client_config.interface, IFNAMSIZ);
