...
#else
	{ /* ARUBA: above did not compile on newer toolchains: un-aligned access */
	uint32_t first = SWAP(ctx->total[0] << 3);
	uint32_t second = SWAP(((ctx->total[1] << 3) | (ctx->total[0] >> 29)));
	memcpy(& ctx->buffer[bytes + pad], &first, sizeof(uint32_t));
	memcpy(& ctx->buffer[bytes + pad + 4], &second, sizeof(uint32_t));
	}
#endif
