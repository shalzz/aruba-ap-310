...
extern int umount(__const char *_special_file);
/* From <linux/vt.h> */
struct vt_stat {
...
#define _PATH_STDPATH	"/usr/local/bin:/usr/local/sbin:/usr/bin:/bin:/usr/sbin:/sbin:/aruba/bin:/aruba:/etc/init.d"

#else

#define _PATH_STDPATH	"/mswitch/bin:/mswitch/mysql/bin:/usr/local/bin:/usr/local/sbin:/usr/bin:/bin:/usr/sbin:/sbin"
#endif
...
#if defined(__FAT_AP__) && defined(CONFIG_OCTOMORE)
static void 
ble_ap_sysctl_write_param(void)
{
    int fd, rc ;
    char nodename[] = "/proc/sys/net/aruba_asap/ble_reset";
    char value[] = "1";

    //value = (value == NULL ? "" : value);

    if ((fd = open(nodename, O_RDWR, 0)) < 0) {
        //BD_LOG((BD_ERROR | BD_APB), NULL, "Error opening %s : %s",
        //        nodename, strerror(errno));
        message(CONSOLE | LOG, "Error opening ble_reset file !!");
        //return -1 ;
    }
    rc = write(fd, value, strlen(value));
    if (rc < 0 && errno != 0) {
        //BD_LOG((BD_ERROR | BD_APB), NULL, "Error writing %s : %s",
        //        nodename, strerror(errno));
        message(CONSOLE | LOG, "Error writing to ble_reset !!");
        if (errno == ENOMEM)
            rc = ENOMEM;
        else
            rc = -1 ;
    ...
    close(fd) ;
    if (rc < 0) {
        message(CONSOLE | LOG, "ble_reset error !!");
    } else {
        message(CONSOLE | LOG, "ble_reset  success!!");
    }
    //return rc ;
}
#endif
...
static void shutdown_system(void)
{
    ...
    /* 
     * flash is already unmounted in rebootme, 
     * No point in sending SIGTERM/SIGKILL to all. 
     */
...
#if defined(__FAT_AP__) && defined(CONFIG_OCTOMORE)
    if (stat("/tmp/ble_ready", &st_ble1) == 0) {
        message(CONSOLE | LOG, "BLE ready present");
        unlink("/tmp/ble_ready");
    }
    if (stat("/tmp/ble_ready", &st_ble2) == 0) {
        message(CONSOLE | LOG, "Cmon.... BLE ready present");
    }
    if (stat("/tmp/has_ble_support", &st_ble3) == 0) {
        ble_ap_sysctl_write_param();
    }
    sleep(5);
#endif
	...
	kill(-1, SIGKILL);
	sleep(1);
	/*if DIR "/aruba/fs" is exist,the UBIFS is mounted ,umount here*/
	dirptr = opendir("/aruba/fs");
	if(dirptr != NULL)
	{
		closedir(dirptr);
		sync();
		if(umount("/aruba/fs")<0)
			message(CONSOLE|LOG, "\rUnmounting UBIFS failed.  Please report this error (%s).\n", strerror(errno));
		else
			message(CONSOLE | LOG, "\rUnmounting UBIFS completed.");
			/* continue on down */
	}
#ifdef UC_CROSS
	/* ARUBA Wireless Systems - Sameer manually mounted by rcS - we umount here. */
	sync();
	if(umount("/flash")<0){
		if (errno != EINVAL) {
			// SCA;
			// - since earlier script processing umounts /flash in normal
			//   circumstances, don't complain if we see EINVAL here
			//
			message(CONSOLE|LOG, "\rUnmounting CompactFlash failed.  Please report this error (%s).\n", strerror(errno));
		}
		/* continue on down */
	}
#endif
