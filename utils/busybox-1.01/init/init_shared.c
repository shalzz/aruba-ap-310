...
#define RB_ENABLE_CAD	0x89abcdef
	...
	if(umount("/flash")<0){
       //No need to write message as syslog would have been killed by now.
	}
#if defined(CONFIG_PORFIDIO) || defined(CONFIG_MILAGRO)
    //Here we aren't checking whether flash1 is not present as we are in the end of shutdown and even if we do unmount non existing
    //mount it would not be an issue
	if(umount("/flash1")<0){
       //No need to write message as syslog would have been killed by now.
	}

#endif /* defined(CONFIG_PORFIDIO) || defined(CONFIG_MILAGRO) */
