...
month_str_to_num(char *mstr)
{
    ...
    for (i = 0; i < 12; i++) {
        if (strncmp(mstr, month_names[i], 3) == 0) {
            return i;
        }
    }
...
static int compare_month(const void *x, const void *y)
{
	int z = month_str_to_num(*(char **)x) - month_str_to_num(*(char **)y);
	return z ? z : strcmp(*(char **)x, *(char **)y);
}
...
int sort_main(int argc, char **argv)
{
	...
	flags = bb_getopt_ulflags(argc, argv, "nruM");
	if (flags & 1) {
