...
static unsigned long display_generic(int scr_width)
{
	...
	if (fscanf(fp, "MemTotal: %lu %s\n", &total, buf) != 2) {
        ...
        /* 
         * Starting from kernel 3.14, /proc/meminfo contains MemAvailable after MemFree
         * We need to skip MemFree before reading Buffers and Cached values
         */
...
int top_main(int argc, char **argv)
{
...
#endif /* CONFIG_FEATURE_USE_TERMIOS */
    int iteration = 0;
    char *siteration = NULL;

	/* do normal option parsing */
#ifndef __FAT_AP__
	opt = bb_getopt_ulflags(argc, argv, "d:n:", &sinterval, &siteration);
	if ((opt & 1)) {
...
#else
    opt = bb_getopt_ulflags(argc, argv, "d:n:N", &sinterval, &siters);
    if ((opt & 1)) {
        interval = atoi(sinterval);
    } else {
        /* Default update rate is 5 seconds */
    ...
    if ((opt & 2)) {
        if ((iters = atoi(siters)) <= 0) {
            printf("top: bad iterations arg '0'\n");
            return EXIT_FAILURE;
        }
    } else {
        iters = 0;
    ...
    if ((opt & 2)) {
        iteration = atoi(siteration);
    }

	/* change to /proc */
	chdir ("/proc") ; //bb_xchdir("/proc");
#ifdef CONFIG_FEATURE_USE_TERMIOS
