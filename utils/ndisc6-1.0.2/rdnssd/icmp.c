...
static int icmp_recv (int fd)
{
        ...
        {
            unsigned flags;
            flags = icmp6.nd_ra_flags_reserved;
            M_f = O_f = 0;
...
static int icmp_socket()
{
    ...
    /* set ICMPv6 filter */
    {
        ...
        if (setsockopt (fd, SOL_ICMPV6, ICMP6_FILTER, &f, sizeof (f)) != 0) {
            syslog (LOG_DEBUG, "setsockopt: set ICMP6_FILTER fail");
        }
    }
    
    if (setsockopt (fd, SOL_IPV6, IPV6_RECVHOPLIMIT, &(int){ 1 }, sizeof (int)) != 0) {
        syslog (LOG_DEBUG, "setsockopt: set IPV6_RECVHOPLIMIT fail");
    }
