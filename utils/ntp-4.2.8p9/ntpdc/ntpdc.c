...
#endif
        if(ai_fam_templ == AF_INET) 
            a_info = getaddrinfo("127.0.0.1", service, &hints, &ai); 
        else 
            a_info = getaddrinfo("::1", service, &hints, &ai);
	}
	...
	)
{
	...
	*rdata = datap = pktdata;
	...
	n = recv(sockfd, (char *)&rpkt, sizeof(rpkt), 0);
    if (n == -1) {
        (void) fprintf(stderr, "%s. ", "NTP configuration is being fetched, please try later"); 
        exit(1);
    }
