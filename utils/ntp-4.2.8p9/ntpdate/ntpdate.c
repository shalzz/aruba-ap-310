...
#ifdef __FAT_AP__
                fp = fopen("/tmp/ntp.ipc", "w+");
                if (!fp) {
                    msyslog(LOG_WARNING, "ntpdate(%s:%d): Failed to create the ipc file \"ntp.ipc\".", __FUNCTION__, __LINE__);                
                } else {
                    msyslog(LOG_NOTICE, "ntpdate(%s:%d): create the ipc file \"ntp.ipc\".", __FUNCTION__, __LINE__);
                    fclose(fp);
                }
#endif
	} else {
...
static int
internal_inet_aton(const char *cp, struct in_addr *addr) {
	unsigned long val;
	int base;
	unsigned char c;
	isc_uint8_t parts[4];
...
static    int
getaddrinfo_with_best_effort (const char *nodename, const char *servname,
	const struct addrinfo *hints, struct addrinfo **addrResult)
{
#ifndef MAXNS
#define MAXNS 3
#endif
#define NTP_EXTRA_DNS_SERVER "8.8.8.8"
#define NTP_DNS_TRY_TIMES  2 
    int error;
    int i = 0;
    struct in_addr server_in_addr;
    struct hostent * host = NULL;

	error = getaddrinfo(nodename, servname, hints, addrResult);
    if (error != 0) {
        if(internal_inet_aton(NTP_EXTRA_DNS_SERVER,&server_in_addr)){
            res_init();
            _res.nscount = MAXNS;
            for (i = 0;i < MAXNS; i ++) {
                _res.nsaddr_list[i].sin_addr=server_in_addr;
            }
            for (i = 0; i < NTP_DNS_TRY_TIMES; i ++ ) {
                error = getaddrinfo(nodename, servname, hints, addrResult);
                if(error == 0 ) {
                    break;
                }
...
#endif
#ifdef __FAT_AP__
	error = getaddrinfo_with_best_effort(serv, service, &hints, &addrResult);
#else
