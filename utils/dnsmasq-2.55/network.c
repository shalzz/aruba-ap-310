...
#else
  return iface_enumerate(&daemon->interfaces, iface_allowed_v4);
#endif
}
...
#if defined(HAVE_IPV6) && !defined(FORWARD_RAW)
...
struct listener *create_wildcard_listeners(void)
{
  ...
  if (daemon->port != 0)
    {
#ifdef FORWARD_RAW
  if ((fd = raw_socket(SOS_MSG_OPCODE_DNSPROXY)) < 0) {
...
#ifdef __FAT_AP__
      /*if enable OpenDNS, we use 208.67.222.222 and 208.67.220.220 as open dns server*/
      if(strcmp(daemon->namebuff, "208.67.222.222") == 0 || strcmp(daemon->namebuff, "208.67.220.220")== 0)
          new->opendns_flag = 1;
#endif
...
#if 0 /* do not need for FAT_AP */
#ifdef __FAT_AP__
      if (!(new->flags & SERV_FROM_RESOLV))
           new->source_addr.in.sin_port = sport++;
#endif
#endif
	  if (!new->sfd && 
...
#ifdef FORWARD_RAW
#define SOURCE_PORT_START_VALUE  3000
#define SOURCE_PORT_RANGE_VALUE  20
  static unsigned int source_port = SOURCE_PORT_START_VALUE;
#endif
...
int reload_servers(char *fname)
{
  ...
  int gotone = 0;
#ifdef FORWARD_RAW
  if (source_port >= (SOURCE_PORT_START_VALUE + SOURCE_PORT_RANGE_VALUE))
      source_port = SOURCE_PORT_START_VALUE;
#endif
...
#ifdef FORWARD_RAW
    {
     short port = source_port++;
      source_addr.in.sin_port = htons(port);
    }
#else
...
#ifdef __FAT_AP__
      serv->temp_failed_queries = 0;
      serv->last_check_time = 0;
#endif
      gotone = 1;
    }
