...
#ifdef FORWARD_RAW
extern int sysctl_write_param(char *nodename, char *value);
#endif

#ifdef __FAT_AP__
extern int native_vlan;
#endif
...
static unsigned short get_vlan_id(struct dhcp_packet* mess)
{
    ...
    while (*opt != OPT_END && opt < end)
    {
        ...
        if (*opt == OPT_ARUBA_VLAN_ID) 
        {
            ...
            vlan = ntohs(vlan);
            my_syslog(MS_DHCP | LOG_WARNING, "Vlan id: %d\n", vlan); 
            return vlan;
        }
        len = opt[1];
    ...
    return SWARM_MAGIC_VLAN_ID_FOR_DNSMASQ;
}

static struct dhcp_context* get_context_by_vlan(struct dhcp_packet* mess)
{
    struct dhcp_context* context;
    unsigned short vlan_id = get_vlan_id(mess);

    for(context = daemon->dhcp; context; context = context->next)
    {
        if(context->vlan_id == vlan_id)
        {
          ...
          size_t sz, time_t now, int unicast_dest, int *is_inform, int pxe)
{
  ...
  unsigned short vlan = get_vlan_id(mess);
#ifdef __FAT_AP__
  struct dhcp_opt *opt_tmp;
  struct dhcp_opt *config_opts = NULL;
  if (context) {
      config_opts = context->context_opts;
  }
#endif
  ...
  context = get_context_by_vlan(mess);
      ...
      if (!message)
        ...
        lease_set_hostname(lease, hostname, 1); 
          ...
          do_options(context, mess, end, NULL, hostname, get_domain(mess->yiaddr), 
             domain, tagif_netid, subnet_addr, vlan, 0, 0, 0, NULL, 0);
        }
    }
  ...
  */
  ...
  if (daemon->enable_pxe && 
      ...
      (opt = option_find1(option_ptr(opt, 0), option_ptr(opt, option_len(opt)), SUBOPT_PXE_BOOT_ITEM, 4)))
    {
      ...
      option_put(mess, end, OPT_ARUBA_VLAN_ID, 2, vlan);
  ...
  if ((opt = option_find(mess, sz, OPTION_REQUESTED_OPTIONS, 0)))
    {
      ...
      {
          context->local = raw_src_addr(vlan);
      }
#endif
  ...
  switch (mess_type)
...
#ifdef FORWARD_RAW
       {
           struct dhcp_lease *lease = lease_find_by_addr(option_addr(opt));

           if (lease == NULL || !memcmp(lease->hwaddr, mess->chaddr, lease->hwaddr_len)) {
                /*next is added for dhcp route cache*/
                char buf[200] = {0};      
                char ips[16] = "0.0.0.0";
                char mask[16] = "0.0.0.0";
#ifdef __FAT_AP__
                int vlan = (native_vlan ? native_vlan : DNSMASQ_ROUTE_CACHE_VLAN);
#else
                int vlan = DNSMASQ_ROUTE_CACHE_VLAN;
#endif
                int flags = ROUTE_CACHE_FLAG_ARP | ROUTE_CACHE_FLAG_PERMANENT;
        
                int action = DNSMASQ_ROUTE_CACHE_REM;
                memcpy(ips, inet_ntoa(option_addr(opt)), 16);
                if (context && context->vlan_id != SWARM_MAGIC_VLAN_ID_FOR_DNSMASQ) {
                    vlan = context->vlan_id;
                    memcpy(mask, inet_ntoa(context->netmask), 16);
                }
                print_mac(daemon->namebuff, emac, emac_len);
                sprintf(buf, "%d %s %s %d %d 0.0.0.0 0.0.0.0 %s", action, ips, daemon->namebuff, 
                        vlan, flags, mask);
                sysctl_write_param("/proc/sys/net/fw/route_table", buf);
                my_syslog(LOG_INFO, "%s %d: magic vlan user rem (%s)\n", __FUNCTION__, __LINE__, buf);
           }
       }
#endif
...
#ifdef FORWARD_RAW
    {
        struct dhcp_lease *lease = lease_find_by_addr(mess->ciaddr);
        ...
        if (lease == NULL || !memcmp(lease->hwaddr, mess->chaddr, lease->hwaddr_len)) {
            ...
            int action = DNSMASQ_ROUTE_CACHE_REM;
            memcpy(ips, inet_ntoa(mess->ciaddr), 16);
            if (context && context->vlan_id != SWARM_MAGIC_VLAN_ID_FOR_DNSMASQ) {
                vlan = context->vlan_id;
                memcpy(mask, inet_ntoa(context->netmask), 16);
            }
            print_mac(daemon->namebuff, emac, emac_len);
            sprintf(buf, "%d %s %s %d %d 0.0.0.0 0.0.0.0 %s", action, ips, daemon->namebuff, vlan, flags, mask);
            sysctl_write_param("/proc/sys/net/fw/route_table", buf);
            my_syslog(LOG_INFO, "%s %d: magic vlan user rem (%s)\n", __FUNCTION__, __LINE__, buf);
        }
    }
      ...
      if (context->netid.net)
    {
      ...
      if ((opt2 = option_find2(netid, daemon->dhcp_opts, OPTION_SERVER_IDENTIFIER))) {
          context->local.s_addr = ((struct in_addr *)opt2->val)->s_addr;
      }
#ifdef __FAT_AP__
      for (opt_tmp = config_opts; opt_tmp; opt_tmp = opt_tmp->next) {
        if ( opt_tmp->opt == OPTION_SERVER_IDENTIFIER) {
            context->local.s_addr = ((struct in_addr *)opt_tmp->val)->s_addr;
        }
      }
#endif
      ...
      if (time != 0xffffffff)
    {
      ...
      do_options(context, mess, end, req_options, offer_hostname, get_domain(mess->yiaddr), 
         domain, tagif_netid, subnet_addr, vlan, fqdn_flags, borken_opt, pxearch, uuid, vendor_class_len);
...
//#ifdef FORWARD_RAW
#if 1
      {
        ...
        int action = DNSMASQ_ROUTE_CACHE_ADD;
        if (context && context->vlan_id != SWARM_MAGIC_VLAN_ID_FOR_DNSMASQ) {
            vlan = context->vlan_id;
            memcpy(mask, inet_ntoa(context->netmask), 16);
        }
        memcpy(ips, inet_ntoa(mess->yiaddr), 16);
        memcpy(ips3, inet_ntoa(mess->siaddr), 16);
        print_mac(daemon->namebuff, emac, emac_len);
        sprintf(buf, "%d %s %s %d %d 0.0.0.0 0.0.0.0 %s", action, ips, daemon->namebuff, 
                  vlan, flags, mask);
        sysctl_write_param("/proc/sys/net/fw/route_table", buf);
        my_syslog(LOG_INFO, "%s %d: magic vlan user add (%s)\n", __FUNCTION__, __LINE__, buf);
      }
#endif
    }
    return dhcp_packet_size(mess, tagif_netid, agent_id, real_end); 
    ...
    return 0;
      ...
      if (!context) {
        my_syslog(MS_DHCP | LOG_INFO, "No context for informing client %s", inet_ntoa(mess->ciaddr));
        return 0;
      }
      ...
      if (lease)
    {
      if (lease->expires == 0)
        ...
        time = (unsigned int)difftime(lease->expires, now);
          /*in rfc2131, it said that server must not send 
            DHCPINFORM response with lease time, so we don't use it*/
      //option_put(mess, end, OPTION_LEASE_TIME, 4, time);
      lease_set_interface(lease, int_index);
    }
...
static void do_options(struct dhcp_context *context,
               ...
               int vendor_class_len)
{
    struct dhcp_opt *opt, *config_opts = context->context_opts;
  struct dhcp_boot *boot;
  unsigned char *p;
  ...
  if (context)
    ...
    option_put(mess, end, OPTION_NETMASK, INADDRSZ, ntohl(context->netmask.s_addr));
      ...
      {
          struct in_addr gateway_ip, netmask;
          void *ret = dhcp_get_config_option_value(context, 3);
          if (ret)
          {
              gateway_ip = *(struct in_addr *)ret;
              netmask = *(struct in_addr *)dhcp_get_config_option_value(context, 1);
              context->broadcast.s_addr = (context->start.s_addr & netmask.s_addr) | (~netmask.s_addr);
          } 
      }
#endif
      ...
      !option_find2(netid, config_opts, OPTION_DNSSERVER)){
         ...
         if (daemon->servers) {
             option_put(mess, end, OPTION_DNSSERVER, INADDRSZ,
                 ntohl(daemon->servers->addr.in.sin_addr.s_addr));
         } else
#endif
  ...
  for (opt = config_opts; opt; opt = opt->next)
    ...
    continue;
      ...
      if (optno == OPTION_SERVER_IDENTIFIER) { 
         continue;
      }
