...
void dhcp_packet(time_t now, int pxe_fd)
{
  ...
  struct in_addr iface_addr = { 0 }, *addrp = NULL;
...
#ifdef __FAT_AP__
#ifdef FORWARD_RAW 
  if (raw_is_uplink_ppp())
  {
      (void)memset(&ifr, 0, sizeof(ifr));
      (void)strncpy(ifr.ifr_name, "ppp0", sizeof(ifr.ifr_name));
  }
#endif
#endif
  ...
  if (pxe_fd)
...
#ifdef __FAT_AP__
      raw_udp_send(iov.iov_base, iov.iov_len, daemon->dhcp_src, dest.sin_addr,
               htons(DHCP_SERVER_PORT), dest.sin_port, mess->chaddr);
      return;
#endif
    } 
#if defined(HAVE_LINUX_NETWORK)
	   ...
	   mess->hlen > sizeof(ifr.ifr_addr.sa_data) || mess->htype == 0)
    {
      ...
      dest.sin_port = htons(daemon->dhcp_client_port);
      raw_udp_send(iov.iov_base, iov.iov_len, daemon->dhcp_src, dest.sin_addr,
               htons(DHCP_SERVER_PORT), dest.sin_port, bcast_mac);
      return;
    }
  else
#ifdef FORWARD_RAW 
...
{  
     ...
     rc = write(fd, value, strlen(value)+1); // must also write NUL
     if (rc < 0 && errno != 0) {
          printf("Error writing %s : %s", nodename, strerror(errno));
          rc = -1 ;
     }
     close(fd) ;
     return rc ;
...
int sysctl_read_param(char *nodename, char *value, int size)
{
     ...
     if ((fd = open(nodename, O_RDONLY, 0)) < 0) {
          printf("Error opening %s : %s", nodename, strerror(errno));
          return -1 ;
     }
     rc = read(fd, value, size);
     if (rc <= 0) {
          printf("Error reading %s : %s", nodename, strerror(errno));
          rc = -1 ;
     }
     close(fd) ;
     return rc ;
...
int get_native_vlan(void)
{
    char value[32];
    int ret;
    int vlan;

    ret = sysctl_read_param("/proc/sys/net/aruba_asap/native_vlan", value, sizeof(value) - 1);
    if (ret == -1) {
       return 0; /* def val */
    }
    sscanf(value, "%d", &vlan);
...
void *dhcp_get_config_option_value(struct dhcp_context *context, int opt_index)
{
    struct dhcp_opt* opt_tmp = context->context_opts;
    while (opt_tmp)
    {
        if (opt_index == opt_tmp->opt)
        {
            return opt_tmp->val;
        }
