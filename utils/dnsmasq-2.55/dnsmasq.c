...
int is_master = 2;
int native_vlan = 0;
extern char filtered_domain[MAX_FILTERED_DOMAIN][MAX_FILTERED_DOMAIN_LEN];
extern char instant_fqdn[256];
extern int get_native_vlan();
...
void get_dhcp_relay_config(void)
{
    ...
    is_master = atoi(++str);
    my_syslog(LOG_INFO, _("get config, role is %d"), is_master);
    fclose(fp);
    return;
}

void read_filtered_domain(void)
{
    FILE *fp;
    int i = 0;

    fp = fopen(SWARM_DNS_FILTERED_DOMAIN, "r");

    if(fp == NULL){
        return;
    } 
    
    memset(filtered_domain, 0, sizeof(filtered_domain));
    while (fgets(filtered_domain[i], MAX_FILTERED_DOMAIN_LEN, fp) != NULL){
        filtered_domain[i][strlen(filtered_domain[i]) - 1] = 0;
        i ++;    
    }
    fclose(fp);
    return;
...
#ifdef __OEM_hp__
    snprintf(instant_fqdn, sizeof(instant_fqdn), "local.%s", domain);
#else
#ifdef __OEM_siemens__
    snprintf(instant_fqdn, sizeof(instant_fqdn), "direct.%s", domain);
#else
    snprintf(instant_fqdn, sizeof(instant_fqdn), "instant.%s", domain);
#endif
#endif
    return;
}
