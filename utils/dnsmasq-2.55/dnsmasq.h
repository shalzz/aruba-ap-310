...
#  include <net/if_dl.h>
#endif
#include "cmn/util/sos_opcodes.h"
#ifndef __OEM_siemens__
#include "cmn/util/sos_l3.h"
#endif
...
#define FORWARD_RAW
#define DNSMASQ_ROUTE_CACHE_ADD       0
#define DNSMASQ_ROUTE_CACHE_REM       1

#define ROUTE_CACHE_FLAG_ARP          SOS_ROUTE_FLAG_ARP
#define ROUTE_CACHE_FLAG_PERMANENT    SOS_ROUTE_FLAG_PERMANENT

#define DNSMASQ_ROUTE_CACHE_VLAN      1
#endif
...
#ifdef __FAT_AP__
#define CONTENT_DISABLE  0
#define CONTENT_ENABLE   1
#define CONTENT_OTHER    2

#define MAX_FILTERED_DOMAIN        10
#define MAX_FILTERED_DOMAIN_LEN    128

#ifdef __OEM_siemens__
/* These definitins copied from cmn/wifi/swarm_ioctl_acl.h, cmn/wifi/atheros_sysctl.h, and sos_l3.h,
 * After these header files have been refactored, these duplicate definitions should be removed
 */
#define SWARM_DNS_FILTERED_DOMAIN         "/proc/sys/net/fw/filtered_domain"
#define SOS_ROUTE_FLAG_ARP            0x00000010
#define SOS_ROUTE_FLAG_PERMANENT      0x00000002
#define SOS_MSG_PROTO(Opcode)       ((Opcode) | SOS_MSG_PROTO_BASE)
#define SOS_MSG_PROTO_BASE          0x8000
...
struct frec {
  ...
  unsigned int crc;
  time_t time;
  char header[sizeof(struct ether_header) + 60 + sizeof(struct udphdr)];
  int header_len;
  struct frec *next;
};
...
#define SWARM_MAGIC_VLAN_ID_FOR_DNSMASQ  3333
...
#else
int iface_enumerate(void *parm, int (*ipv4_callback)());
#endif
/* dbus.c */
#ifdef HAVE_DBUS
char *dbus_init(void);
...
int raw_send(int fd, char *header, size_t hlen, char *packet, size_t len);
#ifdef FORWARD_RAW
extern char mac_addr[ETH_ALEN];
int raw_get_hwaddr(void);
int raw_socket(unsigned short Opcode);
struct in_addr raw_src_addr(unsigned short vlan);
int raw_udp_send(char *header, int plen, struct in_addr, struct in_addr,
                 u_int16_t, u_int16_t, char *mac);
int raw_is_uplink_ppp(void);
#endif /* FORWARD_RAW */

extern void *dhcp_get_config_option_value(struct dhcp_context *context, int opt_index);
