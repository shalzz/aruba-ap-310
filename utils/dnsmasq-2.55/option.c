...
static int is_new_context;
static struct dhcp_context* current_vlan_context;
#ifdef __FAT_AP__
int opendns_enable = 0;
#endif
...
#define LOPT_ARUBA_VLAN_ID   300
#ifdef __FAT_AP__
#define LOPT_EDNS_OPTION 302
#endif
...
static const struct myoption opts[] = 
  ...
  { 
    ...
    { "vlan-id", 1, 0, LOPT_ARUBA_VLAN_ID },
#ifdef __FAT_AP__
...
static struct {
  ...
  char * const arg;
    ...
    { LOPT_ARUBA_VLAN_ID, ARG_DUP, "vlan-id",  gettext_noop("Specify vlan to afford dhcp service."), NULL },
...
#ifdef FORWARD_RAW
  { 'V', ARG_DUP, "ipaddr", gettext_noop("Forward DCHP responses from address"), NULL },
#else /* FORWARD_RAW */
...
static char *parse_dhcp_opt(char *arg, int flags)
{
  ...
  if (!problem)
    ...
    {
            struct dhcp_context* tmp = daemon->dhcp;

            new->next = current_vlan_context->context_opts;
            current_vlan_context->context_opts = new;
            
            while(tmp){
                if((tmp != current_vlan_context) && (tmp->vlan_id == current_vlan_context->vlan_id)){
                    tmp->context_opts = current_vlan_context->context_opts;
                }
                tmp = tmp->next;
            }
    }
...
#  endif
    ...
    char *cp, *a[5] = { NULL, NULL, NULL, NULL, NULL };
        struct dhcp_context *new;
        
        if (!current_vlan_context){
            problem = _("No context for current config");
            break;
        }    

        if(current_vlan_context->start.s_addr){
            // a new dhcp-range, this maybe seperate config 
            new = opt_malloc(sizeof(struct dhcp_context));
            new->vlan_id = current_vlan_context->vlan_id;
        }else{
            new = current_vlan_context;
        }   
    ...
    case 'V':  /* --alias */
#ifdef FORWARD_RAW
      daemon->dhcp_src.s_addr = inet_addr(arg);
      break;
#else /* FORWARD_RAW */
      {
    ...
    *up = new;
    ...
    case LOPT_ARUBA_VLAN_ID:
        {
            if (!is_new_context){
                return _("Wrong config file format, missing config start brace.");
            }

            current_vlan_context = opt_malloc(sizeof(*current_vlan_context));
            bzero(current_vlan_context, sizeof(*current_vlan_context));
            current_vlan_context->vlan_id = atoi(arg);
        } 
        break;
    ...
    case LOPT_EDNS_OPTION:
      {
          ...
          {
              option = '?';
              problem = _("invalid EDNS option code");
              break;
         }
         if (comma &&
         ...
         {
             option = '?';
             problem = _("invalid EDNS option data");
             break;
         }

         tmp = safe_malloc(daemon->edns_options_len + 4 + option_len);
