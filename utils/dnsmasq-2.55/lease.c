...
void lease_init(time_t now)
{
  ...
  if (leasestream)
#ifdef __FAT_AP__
    while (fscanf(leasestream, "%lu %255s %16s %d %255s %764s",
		  &ei, daemon->dhcp_buff2, daemon->namebuff, &vlan_id,
		  daemon->dhcp_buff, daemon->packet) == 6)
#else
...
#ifdef FORWARD_RAW
	     /* add routing cache for ASAP mod*/
		{
        char buf[100] = {0};
	    char mask[16] = "255.255.255.0";
	    int flags = ROUTE_CACHE_FLAG_ARP | ROUTE_CACHE_FLAG_PERMANENT; 
	    int action = DNSMASQ_ROUTE_CACHE_ADD; 
	    int vlan = lease->vlan_id;

        if(vlan == SWARM_MAGIC_VLAN_ID_FOR_DNSMASQ)
#ifdef __FAT_AP__
            vlan = (native_vlan ? native_vlan : DNSMASQ_ROUTE_CACHE_VLAN);
#else
            vlan = DNSMASQ_ROUTE_CACHE_VLAN;
#endif

	    sprintf(buf, "%d %s %.2x:%.2x:%.2x:%.2x:%.2x:%.2x %d %d 0.0.0.0 0.0.0.0 %s", action, daemon->namebuff, lease->hwaddr[0], 
				lease->hwaddr[1], lease->hwaddr[2], lease->hwaddr[3], lease->hwaddr[4], lease->hwaddr[5], vlan, flags, mask);
		my_syslog(LOG_INFO, _("%s %d: magic vlan user add (%s)"), __FUNCTION__, __LINE__, buf);
      ...
      if ((lease->expires != 0 && difftime(now, lease->expires) > 0) || lease == target)
	{
	  ...
	  *up = lease->next; /* unlink */
	     ...
	     /* add routing cache for ASAP mod*/
	  {
	    ...
	    sprintf(buf, "%d %s %.2x:%.2x:%.2x:%.2x:%.2x:%.2x %d %d 0.0.0.0 0.0.0.0 %s", action, inet_ntoa(lease->addr), lease->hwaddr[0], 
				lease->hwaddr[1], lease->hwaddr[2], lease->hwaddr[3], lease->hwaddr[4], lease->hwaddr[5], vlan, flags, mask);
	    sysctl_write_param("/proc/sys/net/fw/route_table", buf);
