...
{
        Aes *aesCtx  = malloc( sizeof(Aes) ) ;;
        AesSetKeyDirect( aesCtx, key, len, NULL, AES_ENCRYPTION ) ;
								return aesCtx;
}

void aes_encrypt(void *ctx, const uint8_t *plain, uint8_t *crypt)
{
        AesEncryptDirect( ctx, crypt, plain ) ;
}
...
{
        Aes *aesCtx  = malloc( sizeof(Aes) ) ;;
        AesSetKeyDirect( aesCtx, key, len, NULL, AES_DECRYPTION ) ;
								return aesCtx;
}

void aes_decrypt(void *ctx, const uint8_t *crypt, uint8_t *plain)
{
        AesDecryptDirect( ctx, plain, crypt ) ;
}
...
{
								Aes aesCtx ;
        AesSetKey( &aesCtx, key, 128, iv, AES_ENCRYPTION ) ;
        return AesCbcEncrypt( &aesCtx, data, data, data_len ) ;
}
...
{
								Aes aesCtx ;
        AesSetKey( &aesCtx, key, 128, iv, AES_DECRYPTION ) ;
        return AesCbcDecrypt( &aesCtx, ciphertext, ciphertext, cipher_len ) ;
}
