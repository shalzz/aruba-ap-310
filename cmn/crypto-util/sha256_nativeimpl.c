...
//#ifdef INTERNAL_SHA256                 just do it always
...
/*
 * how about a more rational interface to sha256 that doesn't require
 * the caller to maintain a data structure? Sounds like great idea!
 */
void
sha256_vectorized (u8 *mac, int nparams, ...)
{
    struct sha256_state ctx;
    va_list val;
    u8 *param;
    ...
    if (nparams) {
        sha256_init(&ctx);
        va_start(val, nparams);
        while (nparams) {
            param = va_arg(val, u8 *);
            plen = va_arg(val, int);
            sha256_process(&ctx, param, plen);
            nparams--;
        }
        va_end(val);
        sha256_done(&ctx, mac);
