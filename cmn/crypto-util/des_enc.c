...
ap_prov_decrypt(__u8 * in_cipher, int ilen, __u8 * clear, int *olen)
{
...
#else
    des_key_sched(&my_keys[0], ks1);
    des_key_sched(&my_keys[1], ks2);
    des_key_sched(&my_keys[2], ks3);
#endif
    cipher = (__u8 *) 0;
    ...
    memcpy((void *) &ivec, (void *) cipher, sizeof(ivec));
    memcpy(ivec_bytes, (void *) &ivec, 8);
#ifdef USE_XYSSL
    des3_crypt_cbc(&des3_ctx, DES_DECRYPT, ilen - 8, ivec,
            cipher + 8, clear);
#else
    des_ede3_cbc_encrypt((unsigned char *) (cipher + 8),
            (unsigned char *) clear,
            ilen - 8, ks1, ks2, ks3, &ivec, 0);
#endif
...
ap_prov_encrypt(__u8 * clear, int ilen, __u8 * cipher, int *olen)
{
    ...
    *(__u16 *) (ivec_bytes + 6) = (__u16) (rand() & 0xFFFF);
    ...
    memcpy((void *) &ivec, (void *) ivec_bytes, sizeof(ivec_bytes));
#ifdef USE_XYSSL
    des3_crypt_cbc(&des3_ctx, DES_ENCRYPT, ilen, ivec,
            input, ci_tmp);
#else
    des_ede3_cbc_encrypt((unsigned char *) input,
            (unsigned char *) ci_tmp,
            ilen, ks1, ks2, ks3, &ivec, 1);
#endif
    free(input);
