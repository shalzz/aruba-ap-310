...
static int get_drv_info(char *master_ifname)
{
	...
	strlcpy(ifr.ifr_name, master_ifname, IFNAMSIZ);
	ifr.ifr_data = (caddr_t)&info;

	info.cmd = ETHTOOL_GDRVINFO;
	strlcpy(info.driver, "ifenslave", 32);
...
static int change_active(char *master_ifname, char *slave_ifname)
{
	...
	strlcpy(ifr.ifr_name, master_ifname, IFNAMSIZ);
	strlcpy(ifr.ifr_slave, slave_ifname, IFNAMSIZ);
...
static int get_if_settings(char *ifname, struct dev_ifr ifra[])
{
	...
	for (i = 0; ifra[i].req_ifr; i++) {
		strlcpy(ifra[i].req_ifr->ifr_name, ifname, IFNAMSIZ);
...
static int get_slave_flags(char *slave_ifname)
{
	...
	strlcpy(slave_flags.ifr_name, slave_ifname, IFNAMSIZ);
...
static int set_slave_hwaddr(char *slave_ifname, struct sockaddr *hwaddr)
{
	...
	strlcpy(ifr.ifr_name, slave_ifname, IFNAMSIZ);
	memcpy(&(ifr.ifr_hwaddr), hwaddr, sizeof(struct sockaddr));
	res = ioctl(skfd, SIOCSIFHWADDR, &ifr);
	if (res < 0) {
		saved_errno = errno;
