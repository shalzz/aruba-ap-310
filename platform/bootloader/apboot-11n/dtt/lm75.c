...
static int
lm75_read(uchar chip, uint addr, int alen, uchar *buffer, int len)
{
#if defined(LM75_I2C_CHANNEL)
    return i2c_read(LM75_I2C_CHANNEL, chip, addr, alen, buffer, len);
#else
...
static int
lm75_write(uchar chip, uint addr, int alen, uchar *buffer, int len)
{
#if defined(LM75_I2C_CHANNEL)
    return i2c_write(LM75_I2C_CHANNEL, chip, addr, alen, buffer, len);
#else
...
int dtt_read(int sensor, int reg)
{
     ...
     */
    if (lm75_read(sensor, reg, 1, data, dlen) != 0)
	return -1;
...
int dtt_write(int sensor, int reg, int val)
{
     ...
     */
    if (lm75_write(sensor, reg, 1, data, dlen) != 0)
	return 1;
