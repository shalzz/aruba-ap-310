...
// pass in a bitmask - delay is a power of 2
void spin2(int wtf)
{
    static int   spnr2, i = 1;
    static char  disp[4] = {'-','\\','|','/'};
    if(spnr2 > 4) spnr2 = 0;
    
    if(i++ & wtf) {
        printf("%c\b",disp[spnr2++]);
        i = 1;
    }
}
...
//	struct jffs2_raw_inode *jNode;
	...
	if ((list->listTail != NULL) && (compare_inodes(new, list->listTail)))
		prev = list->listTail;
	else if ((list->listLast != NULL) && (compare_inodes(new, list->listLast)))
		prev = list->listLast;
	else
		prev = NULL;

	for (b = (prev ? prev->next : list->listHead);
	     b != NULL && compare_inodes(new, b);
	     prev = b, b = b->next) {
		list->listLoops++;
	}
	if (b != NULL)
...
static struct b_node *
insert_Dnode(struct b_list *list, struct b_node *new)
{
#ifdef CFG_JFFS2_SORT_FRAGMENTS
	struct b_node *b, *prev;
//	struct jffs2_raw_inode *jNode;

	if ((list->listTail != NULL) && (compare_dirents(new, list->listTail)))
		prev = list->listTail;
	else if ((list->listLast) != NULL && (compare_dirents(new, list->listLast)))
		prev = list->listLast;
	else
		prev = NULL;

	for (b = (prev ? prev->next : list->listHead);
	     b != NULL && compare_dirents(new, b);
	     prev = b, b = b->next) {
		list->listLoops++;
	}
	if (b != NULL)
...
insert_inode(struct b_list *list, struct jffs2_raw_inode *node, u32 offset)
{
	...
	new->offset = offset;
#if 0
    if(node->version < old_vers) {
        printf("old_vers=%x node_ver=%x\n",old_vers, node->version);
        print_nxt = 1;
    } else {
        if(print_nxt) {
            printf("nxt_node=%x\n",node->version);
...
insert_dirent(struct b_list *list, struct jffs2_raw_dirent *node, u32 offset)
{
	...
	return insert_Dnode(list, (struct b_node *)new);
}
...
{
#if 0
	struct b_inode ojNew;
	struct b_inode ojOld;
    
	struct b_inode *jNew =
		(struct b_inode *)get_fl_mem(new->offset, sizeof(ojNew), &ojNew);
	struct b_inode *jOld =
		(struct b_inode *)get_fl_mem(old->offset, sizeof(ojOld), &ojOld);
...
static int compare_dirents(struct b_node *new, struct b_node *old)
{
	struct b_dirent ojNew;
	struct b_dirent ojOld;
	struct b_dirent *jNew =
		(struct b_dirent *)new;
//		(struct jffs2_raw_dirent *)get_fl_mem(new->offset, sizeof(ojNew), &ojNew);
	struct b_dirent *jOld =
		(struct b_dirent *)old;
		       ...
		       struct stat *stat)
{
	...
	for (jNode = (struct b_inode *)pL->frag.listHead; jNode; jNode = jNode->next) {
		...
		if (dest || stat) {
			...
			if (dest)
				len += jNode->csize;
			nand_read(nand, jNode->offset, (ulong*) &len,(u_char*) inode);
...
/*					version); */
	...
	for (jNode = (struct b_inode *)pL->frag.listHead; jNode; jNode = jNode->next) {
		if (jNode->ino == jDirFoundIno) {
			...
			nand_read(nand, jNode->offset + sizeof(struct jffs2_raw_inode), (ulong*) &len, (uchar*) &tmp);
			tmp[jNode->csize] = '\0';
			break;
		}
	}
...
jffs2_1pass_search_list_inodes(struct b_lists * pL, const char *fname, u32 pino)
{
	...
	strcpy(tmp, &fname[i]);
	working_tmp[0] = '\0';
    printf("%s searching for: %s and: %s\n", __FUNCTION__, fname, tmp);
	while ((c = (char *) strchr(tmp, '/')))	/* we are still dired searching */
	{
		strncpy(working_tmp, tmp, c - tmp);
		working_tmp[c - tmp] = '\0';
...
//	struct jffs2_unknown_node *node;
	...
	if (((u32)pL->partOffset) != part->offset) {
		printf ("rescan: different partition\n");
		return 1;
	}
...
dump_fragments(struct b_lists *pL)
{
	...
	while (b) {
		...
		putLabeledWord("\r\n\tbuild_list: FLASH_OFFSET = ", ((struct jffs2_raw_inode *)b)->offset);
		...
		putLabeledWord("\tbuild_list: offset = ", ((struct jffs2_raw_inode *)b)->offset);	/* FIXME: ? [RS] */
		b = b->next;
	}
}
#endif
...
dump_dirents(struct b_lists *pL)
{
	...
	b = pL->dir.listHead;
	while (b) {
		jDir = (struct jffs2_raw_dirent *) get_node_mem(((struct jffs2_raw_dirent *)b)->offset);
		putstr("\r\n");
		putnstr(jDir->name, jDir->nsize);
		    ...
		    unsigned ofs, unsigned len)
{
	...
	olen = len;
	ret = nand_read(nand, ofs, (ulong*) &olen, buf);
	if (ret) {
...
jffs2_1pass_build_lists(struct part_info * part)
{
	...
	int i;
//	u32 counter = 0;
	u32 counter4 = 0;  // +4 increments...?
	u32 counterF = 0;
	u32 counterN = 0;  // dirents?
    u32 counterS = 0;
    u32 counterCl = 0;
    //static int  init = 0;
    ...
    //printf("%s sectorsize=%d, part->size=%d, nr_blocks=%d \n",__FUNCTION__, sectorsize, part->size, nr_blocks);
	if (!buf)
		return 0;
	...
	for (i = 0; i < nr_blocks; i++) {
		...
		while (ofs < sectorsize - sizeof(struct jffs2_unknown_node)) {
			...
			if (node->nodetype == JFFS2_NODETYPE_INODE &&
                ...
                sumnode_crc((struct jffs2_raw_summary *) node)) {
                //printf("+S+");
				if (insert_inode(&pL->sum, (struct jffs2_raw_summary *) node,
						 offset) == NULL) {
					return 0;
				}
                counterS++;
	...
	putLabeledWord("Summary nodes           = ", counterS);
	putLabeledWord("CleanMarker nodes       = ", counterCl);
#endif
