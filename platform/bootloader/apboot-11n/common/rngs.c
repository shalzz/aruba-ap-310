 ...
 */
{
  ...
  return ((uint32_t) seed[stream]);
//  return ((uint32_t) seed[stream] / MODULUS);
#endif
    return(*((uint32_t*)0xbef180ac));
}
 ...
 * the state is obtained from the system clock
 * ---------------------------------------------------------------
 */
{
  unsigned long x = 0;

    x = ((unsigned long) get_ticks()) % MODULUS;              
 ...
 */
{
  ...
  if ((initialized == 0) && (stream != 0))   /* protect against        */
    PlantSeeds();                     /* un-initialized streams */
}
 ...
 */
{
  ...
  PutSeed();                       /* and set the state to 1    */
  for(i = 0; i < 10000; i++)
    u = rand();
  ...
  PlantSeeds();                    /* set the state of all streams    */
