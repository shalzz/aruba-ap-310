...
//	printf("in input_swap_data base for read is %lx\n", (unsigned long) pbuf);

	while (words--) {
		*dbuf++ = swap16(*(unsigned short *)pbuf);
		*dbuf++ = swap16(*(unsigned short *)pbuf);
//		*dbuf++ = ld_le16(pbuf);
//		*dbuf++ = ld_le16(pbuf);
	}
...
static void ide_reset (void)
{
    ...
    //ide_set_reset (1); /* assert reset */ FIXME??
	WATCHDOG_RESET();
