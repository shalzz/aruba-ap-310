...
extern void memcpyFlash(env_t *env_ptr, void * buffer, unsigned size);
#endif
...
#if 0//defined(CONFIG_MARVELL) && defined(MV_SPI_BOOT)
...
// this is the one Nebbiolo's using
int saveenv(void)
{
	int	len, rc;
	ulong	end_addr;
	...
	/* copy old contents to temporary buffer */
        ...
        memcpyFlash((env_t *)flash_sect_addr, (void*)env_buffer, CFG_ENV_SECT_SIZE);
#else 
	...
	//puts ("Writing to Flash... ");
    //printf("env_buffer: %x, flash_sect_addr: %x, len: %x\n",env_buffer, flash_sect_addr, len);
	debug ("Writing %08lX ... %08lX, %X\n", env_buffer, 
		(ulong)flash_sect_addr, len);
