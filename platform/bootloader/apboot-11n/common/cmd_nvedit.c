...
# error Define one of CFG_ENV_IS_IN_{NVRAM|EEPROM|FLASH|DATAFLASH|SPI_FLASH|NOWHERE}
#endif
...
int _do_setenv (int flag, int argc, char *argv[])
{
	...
	name = argv[1];
    if (!strcmp(name, "boardname") || !strcmp(name, "baudrate")) {
           printf("This variable cannot be modified.\n");
           return 1;
    }
	/*
...
int do_purgeenv (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	extern uchar default_environment[];
#if defined(CONFIG_NEBBIOLO) | defined(CONFIG_GRENACHE)
	int default_environment_size = ENV_SIZE;
#else
	extern int default_environment_size;
#endif
	extern env_t *env_ptr;
#ifdef CONFIG_APBOOT
	int saved_os_partition = 0;
	char *p_os_partition = NULL;
	char os_partition[5];
#ifdef CONFIG_X4
    char *fman_ucode = NULL;
    char saved_fman_ucode[64];
#endif
#endif
#if defined(CONFIG_MSR_SUBTYPE)
	extern void msr_export_default_env(void);
	extern void msr_export_default_bakimg_env(void);
#endif
        if (argc != 1) {
		printf ("Usage:\n%s\n", cmdtp->usage);
		return 1;
...
#ifdef CONFIG_APBOOT
	// preserve os_partition for IAPs
	if (manuf_is_iap())  {
		if ((p_os_partition = getenv("os_partition")))  {
			saved_os_partition = simple_strtoul(p_os_partition, NULL, 10);
			if (saved_os_partition < 0 || saved_os_partition > 1)  {
			    p_os_partition = NULL;  /* don't restore invalid os_partition */
			}  else  {
				printf ("preserving os_partition (%d)\n", saved_os_partition);
			}
		}
	}
#ifdef CONFIG_X4
        fman_ucode = getenv("fman_ucode");
        strncpy(saved_fman_ucode, fman_ucode, strlen(fman_ucode)+1);
#endif
#endif
...
#if defined(CONFIG_MSR_SUBTYPE)
	/* Update AOS info to APBoot ENV variable */
	msr_export_default_env();
	msr_export_default_bakimg_env();
	...
	manuf_recover_ethaddr(1);
	// preserve os_partition for IAPs
	if (p_os_partition)  {
		sprintf(os_partition, "%d", saved_os_partition);
		setenv("os_partition", os_partition);
		saveenv();
	}
#ifdef CONFIG_X4
#ifndef CONFIG_SPL_BUILD
    if (fman_ucode) {
        setenv("fman_ucode", saved_fman_ucode); 
    }
#endif
#endif
#endif
