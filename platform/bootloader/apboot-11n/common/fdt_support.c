...
/*
 * Provide a weak default function to return the flash bank base address.
 * There might be multiple non-identical flash chips connected to one
 * chip-select, so we need to pass an index as well.
 */
u32 __flash_get_bank_base(int cs, int idx)
{
	extern flash_info_t flash_info[];
	 ...
	 * routine.
	 */
	return flash_info[cs].start[0];
}
u32 flash_get_bank_base(int cs, int idx)
	__attribute__((weak, alias("__flash_get_bank_base")));
...
/*
 * This function can be used to update the base address in the "range" property
 * of all NOR FLASH device nodes. This is necessary for boards with
 * non-fixed NOR FLASH sizes.
 */
int fdt_fixup_nor_flash_base(void *blob)
{
    char compat[16] = "fsl,ifc";
    int off;
    int len;
    struct fdt_property *prop;
    u32 *reg, *reg2;
    ...
    if (off != -FDT_ERR_NOTFOUND) {
        ...
        /*
         * Found one compatible node, so fixup the base
         * in its ranges properties
         */
        prop = fdt_get_property_w(blob, off, "ranges", &len);
        if (prop) {
            int tuple_size = 5 * sizeof(reg);
            ...
            for (idx = 0; idx < (len / tuple_size); idx++) {
                /*
                 * Update size in ranges property
                 */
                reg[4] = flash_get_bank_size(reg[0],
                        idx);
                /*
                 * Update base in ranges property
                 */
                reg[3] = flash_get_bank_base(reg[0],
                        idx);

                /*
                 * Point to next ranges tuple
                 */
                reg += 5;
            }
