...
#if defined(CONFIG_CARDHU) || defined(CONFIG_DALMORE) || defined(CONFIG_HAZELBURN)// (CONFIG_COMMANDS & CFG_CMD_ETHREG)
...
extern int ag7240_miiphy_write(char *devname, unsigned char phy_addr, unsigned char reg, unsigned short data);
extern int ag7240_miiphy_read(char *devname, unsigned char phy_addr, unsigned char reg, unsigned short *value);
#endif
...
#if defined(CONFIG_ATHRS17_PHY) && defined(CFG_ATHRS27_PHY)
int do_s17reg (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	int   rcode = 0;
	...
	if(rcode == READ_PHY) {
		unsigned short rval;
		ag7240_miiphy_read("eth0",portnum,reg,&rval);
		rd_value = rval;
...
#if defined(CONFIG_ATHRS17_PHY) && defined(CFG_ATHRS27_PHY)
/* Dalmore uses s17reg and s27reg, so a single "ethreg" won't work */
int do_s27reg (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
#else
...
#if defined(CONFIG_ATHRS17_PHY) && defined(CFG_ATHRS27_PHY)
U_BOOT_CMD(
	s17reg,	6,	1,	do_s17reg,
	"s17reg    - s17 MAC/PHY reg rd/wr utility\n",
	"[p <phyaddr>] <reg> [<value>]\n"
	"p - indicates operation is to phy registers; by default rd/wr is to s17 mac registers\n"
	"<phyaddr> - address of the phy\n"
	"<reg>     - register offset\n"
	"<value>   - value to write\n"
);
U_BOOT_CMD(
	s27reg,	6,	1,	do_s27reg,
	"s27reg    - s27 MAC/PHY reg rd/wr utility\n",
	"[p <phyaddr>] <reg> [<value>]\n"
	"p - indicates operation is to phy registers; by default rd/wr is to s27 mac registers\n"
	"<phyaddr> - address of the phy\n"
	"<reg>     - register offset\n"
	"<value>   - value to write\n"
);
...
U_BOOT_CMD(
	...
	"ethreg    - s16/s26/s27 MAC/PHY reg rd/wr utility\n",
	"[p <phyaddr>] <reg> [<value>]\n"
	"p - indicates operation is to phy registers; by default rd/wr is to s16/s26/s27 mac registers\n"
	"<phyaddr> - address of the phy\n"
	"<reg>     - register offset\n"
	"<value>   - value to write\n"
);
#endif  // defined(CONFIG_ATHRS17_PHY) && defined(CFG_ATHRS27_PHY)

#endif  // defined(CONFIG_CARDHU) || defined(CONFIG_DALMORE) || defined(CONFIG_HAZELBURN)
#endif  // CONFIG_APBOOT
