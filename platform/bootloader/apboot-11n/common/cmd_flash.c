 ...
 */
#include <common.h>
#if !defined(CONFIG_OCTEON) && !defined(AR7100) && !defined(CONFIG_MARVELL) && !defined(powerpc) \
    && !defined(CONFIG_IPROC) && !defined(CONFIG_MACH_IPQ806x)
#ifdef CONFIG_RMI_CPU
#include <nebbiolo/boot_flash.h>
#endif
#include <command.h>
...
addr_spec(char *arg1, char *arg2, ulong *addr_first, ulong *addr_last)
{
	...
	int erase_for_nand = (*addr_first == 0x6e616e64);
#else
#	define erase_for_nand	0
#endif
	...
	*s_count = 0;
	...
	for (bank=0,info=&flash_info[0];
		...
		if (s_first[bank] >= 0) {
            ...
            printf("addr_first:%x info->start[0]:%x b_end:%x\n",
...
int do_flerase (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	...
	if (strcmp(argv[1], "all") == 0) {
		...
		for (bank=1; bank<=CFG_MAX_FLASH_BANKS; ++bank) {
			...
			rcode = flash_erase (info, 0, info->sector_count-1, 1);
		}
		return rcode;
	}
	...
	if ((n = abbrev_spec(argv[1], &info, &sect_first, &sect_last)) != 0) {
		...
		rcode = flash_erase(info, sect_first, sect_last, 1);
		return rcode;
	}
	...
	rcode = flash_erase(NULL, addr_first, addr_last);
#else
	rcode = flash_sect_erase(addr_first, addr_last);
#endif
	return rcode;
...
#ifdef ATH_DUAL_FLASH
int do_nferase (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	flash_info_t *info;
	ulong bank, addr_first, addr_last;
	int n, sect_first, sect_last;
	...
	rcode = ath_nand_flash_erase(NULL, addr_first, addr_last);
	return rcode;
}
...
{
	flash_info_t *info;
	ulong addr_first = 0xbc400000, addr_last = 0xbc420000 - 1;
	int rcode = 0, errors = 0;
    uint32_t    t_buf[0x20000/4], i, val;
	...
	rcode = flash_sect_erase(addr_first, addr_last);
    flash_write((char*) t_buf, addr_first, 0x20000);
    
    //verify sector against test data
    for(i = 0; i < 0x8000; i++)
    {
        val = ((uint32_t*)addr_first)[i];
        if(val != t_buf[i]) {
...
int flash_sect_erase (uint32_t addr_first, uint32_t addr_last)
{
	flash_info_t *info;
	ulong bank;
		...
		for (bank=0,info=&flash_info[0];
			...
			if (s_first[bank]>=0) {
				...
				debug ("Erase Flash from 0x%08lx to 0x%08lx "
					...
					bank+1);
				rcode = flash_erase (info, s_first[bank], s_last[bank], 0);
			}
		}
...
int do_protect (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
    ...
    printf("somehow this runs... addr_first: %x addr_last:%x\n",addr_first,addr_last);
	rcode = flash_sect_protect (p, addr_first, addr_last);
	return rcode;
}


int flash_sect_protect (int p, uint32_t addr_first, uint32_t addr_last)
{
	flash_info_t *info;
	ulong bank;
#ifdef CFG_MAX_FLASH_BANKS_DETECT
...
U_BOOT_CMD(
	fltest,    2,    0,    do_fltest,
	"fltest  - test u-boot FLASH driver\n",
	"\n    - perform simple address test on hard coded flash sector\n"
...
#ifdef ATH_DUAL_FLASH
U_BOOT_CMD(
	nferase,   3,   1,  do_nferase,
	"nferase   - erase NAND FLASH memory\n",
	"start end\n"
	"    - erase NAND FLASH from addr 'start' to addr 'end'\n"
	"nferase start +len\n"
	"    - erase NAND FLASH from addr 'start' to the end of sect "
	"w/addr 'start'+'len'-1\n"
);
#endif /* ATH_DUAL_FLASH */
