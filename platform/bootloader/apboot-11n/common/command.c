...
extern int xlr_board_dolcetto(void);
extern int xlr_board_nebbiolo(void);
#endif
#ifdef CONFIG_APBOOT
...
int do_help (cmd_tbl_t * cmdtp, int flag, int argc, char *argv[])
{
	...
	if (argc == 1) {	/*show list of commands */
        ...
        //printf("do_help: cmd_items=%d\n",cmd_items);
		cmd_tbl_t *cmd_array[cmd_items];
		int i, j, swaps;

		/* Make array of commands from .uboot_cmd section */
		cmdtp = (cmd_tbl_t*) (void *)&__u_boot_cmd_start;
//        printf("do_help: argv_0: %s\n",argv[0]);
//        printf("do_help: cmdtp: %x\n",cmdtp);
        i = 0;
     	while (cmdtp != ((cmd_tbl_t*) (void *)&__u_boot_cmd_end)) {
        ...
        //printf("commands sorted - now show usage(s)...\n");

		/* print short help (usage) */
		for (i = 0; i < cmd_items; i++) {
#if defined(CONFIG_NEBBIOLO) || defined(CONFIG_APBOOT) || defined(CONFIG_GRENACHE)
			int repeat = cmd_array[i]->repeatable;
            // See if command is for both platforms.
            if(repeat != 0) {
#if !defined(CONFIG_APBOOT) && !defined(CONFIG_GRENACHE)
                // Don't display Dolcetto commands on Nebbiolo.
                if((xlr_board_nebbiolo() != 0) && (repeat == CMD_DOLCETTO)) {
                    continue;
                }
                // Don't display Nebbiolo commands on Dolcetto.
                if((xlr_board_dolcetto() != 0) && (repeat == CMD_NEBBIOLO)) {
                    continue;
                }
			...
			if (usage == NULL)
				continue;
            // buffer the string to keep strtok from f'king it up
            strncpy(usage_buf, usage, 254);
            // let's pretty-print the cmd listing
            cmd_name = strtok( usage_buf, " -" );
            cmd_desc = strtok( NULL, "\n" );
            cmd_desc = cmd_desc + strspn( cmd_desc, " -" );
         
            if( strlen(cmd_name) < 14 )
                printf( "%-14s - %s\n", cmd_name, cmd_desc );
            else
                printf( "%s  - %s\n", cmd_name, cmd_desc );                

            //printf( "DBG:cmd=#%s# desc=#%s# cmd len=%d\n\n",
            //        cmd_name, cmd_desc, strlen(cmd_name) );
		}
		return 0;
	}
...
#ifdef CONFIG_APBOOT
			/* hide diag commands if not in diag mode */
			if (!in_diag_mode && cmdtp->repeatable) {
				goto nohelp;
			}
#endif
#ifdef	CFG_LONGHELP
...
cmd_tbl_t *find_cmd_tbl (const char *cmd, cmd_tbl_t *table, int table_len)
{
	...
	for (cmdtp = table;
		...
		if (cmdtp->repeatable) {
			// no diag commands in 2-stage boot
			continue;
		}
#endif
...
static int complete_cmdv(int argc, char *argv[], char last_char, int maxv, char *cmdv[])
{
	...
	if (argc == 0) {
		...
		for (cmdtp = (cmd_tbl_t *)(void *)&__u_boot_cmd_start; 
		       cmdtp != (cmd_tbl_t *)(void *)&__u_boot_cmd_end; cmdtp++) {
#ifdef CONFIG_APBOOT
			/* hide diag commands if not in diag mode */
			if (!in_diag_mode && cmdtp->repeatable) {
			    continue;
