 ...
 * we have a match
 */
static ulong find_sector_end_addr (flash_info_t * info, ulong addr)
{
	ulong sector;
    ...
    {
        return(info->start[0] + info->size - 1);
    }
    else
        return(info->start[sector + 1] - 1);
}
...
int do_flerase (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
    ...
    /* allow erase length to be specified with + at start of second argument */
	addr_first = simple_strtoul(argv[1], NULL, 16);
    if (argv[2][0] == '+')
    {
        addr_last  = find_sector_end_addr(&flash_info[0], addr_first + simple_strtoul(argv[2] + 1, NULL, 16));
    }
    else
    {
...
U_BOOT_CMD(
	...
	"start +size"
	"    - erase FLASH from addr 'start' to addr 'start + size', rounded up to sector size\n"
