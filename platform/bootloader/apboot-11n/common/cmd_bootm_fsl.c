...
#endif
#if defined(CONFIG_CMD_ELF) && !defined(CONFIG_APBOOT)
...
#if defined(CONFIG_PCI) && defined(CONFIG_BOARD_PCI_DEINIT)
    board_pci_deinit();
#endif
		...
		bootm_headers_t *images, ulong *os_data, ulong *os_len)
{
	...
	*os_data = *os_len = 0;
	...
	case IMAGE_FORMAT_FIT:
		...
		debug ("   Trying '%s' kernel subimage\n", fit_uname_kernel);
...
#ifdef CONFIG_NEEDS_MANUAL_RELOC_notyet
void bootm_reloc(void) {
	fixup_cmdtable(cmd_bootm_sub, ARRAY_SIZE(cmd_bootm_sub));
}
#endif
#endif /* powerpc */
