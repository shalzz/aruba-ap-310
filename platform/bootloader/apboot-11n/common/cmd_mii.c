...
int do_mii (cmd_tbl_t * cmdtp, int flag, int argc, char *argv[])
{
	...
	if (op[0] == 'i') {
		...
		if (argc == 2)
			...
			miiphy_set_current_dev (argv[2]);
	} else if (strncmp(op, "pr", 2) == 0) {
            for (j1 = 0; j1 < 5; j1++) {
                /* set the page */
                miiphy_write(devname, 0, 0x16, phy_addr[j1]);
//                printf("setting the page %d\n", phy_addr[j1]);
                for (k = 0; k < 29; k++) {
                    miiphy_read (devname, 0, k, &data);
			printf("page=%02x reg=%02x data=",
                                phy_addr[j1], k);
                        printf("%04X\n", data & 0x0000FFFF);
                }
                printf("\n");
