...
#if defined(CONFIG_APBOOT) && defined(CONFIG_USB_STORAGE) && defined(powerpc)
int
do_dir(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	...
	return do_fat_ls(cmdtp, flag, ac, av);
}

U_BOOT_CMD(
	dir,	2,	0,	do_dir,
	"dir  - list files in a directory (default /)",
	"[directory]\n"
	"    - list files"
);
#endif
