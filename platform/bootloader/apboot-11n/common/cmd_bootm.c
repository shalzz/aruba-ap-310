 ...
 */
#include <common.h>
#if !defined(powerpc) && !defined(CONFIG_IPROC) && !defined(CONFIG_MACH_IPQ806x)
...
static boot_os_Fcn do_bootm_rtems;
    ...
    (CONFIG_COMMANDS & CFG_CMD_BOOTVX)
...
int do_bootm (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	...
	s = getenv ("verify");
	verify = (s && (*s == 'n')) ? 0 : 1;
#if defined(ARUBA_ARM) || defined(CONFIG_IPROC)
	verify = 0;
#endif
...
#if !defined(ARUBA_ARM) && !defined(CONFIG_IPROC) && !defined(CONFIG_MACH_IPQ806x)
#ifdef CONFIG_ATH_NAND_FL
...
#if defined(__PPC__)
	if (hdr->ih_arch != IH_ARCH_PPC)
#elif defined(__ARM__)
	if (hdr->ih_arch != IH_ARCH_ARM)
#elif defined(__I386__)
	if (hdr->ih_arch != IH_ARCH_I386)
#elif defined(__mips__)
	if (hdr->ih_arch != IH_ARCH_MIPS)
#elif defined(__nios__)
	if (hdr->ih_arch != IH_ARCH_NIOS)
#elif defined(__M68K__)
	if (hdr->ih_arch != IH_ARCH_M68K)
#elif defined(__microblaze__)
	if (hdr->ih_arch != IH_ARCH_MICROBLAZE)
#elif defined(__nios2__)
	if (hdr->ih_arch != IH_ARCH_NIOS2)
#elif defined(__blackfin__)
	if (hdr->ih_arch != IH_ARCH_BLACKFIN)
#else
