...
#if defined(CONFIG_MARVELL) || defined(powerpc) || defined(CONFIG_IPROC) ||defined(CONFIG_MACH_IPQ806x)
...
static void index_partitions(void)
{
	...
	if (current_dev) {
        ...
        if(getenv("mtddevnum") != NULL)
		  setenv("mtddevnum", NULL);
        if(getenv("mtddevname") != NULL)
...
static int part_validate(struct mtdids *id, struct part_info *part)
{
	...
	if (id->type == MTD_DEV_TYPE_NAND) {
		...
		printf("part_validate: invalid dev type\n");
    }
...
static int device_parse(const char *const mtd_dev, const char **ret, struct mtd_device **retdev)
{
	...
	*ret = NULL;
    ...
    DEBUGF("%s: id->size = %x\n",id->mtd_id, id->size);
    if(id->size == 0) {
        printf("partition id: %s id->size == 0 ; fixed it...\n",id->mtd_id);
        id->size = 0x10000000;    // double FUCKME i jammed in a valid size to see what happens...
    }
	...
	while (p && (*p != '\0') && (*p != ';')) {
		...
		if (part->offset == OFFSET_NOT_SPECIFIED)
			...
			offset = part->offset;
        printf("part offset: %x part size: %x\n",part->offset,part->size);
		/* verify alignment and size */
		if (part_validate(id, part) != 0) // FUCKME second part device size == 0
			break;
...
int mtdparts_init(void)
{
	...
	if (parts && ((last_parts[0] == '\0') || ((strstr(last_parts, parts) != 0)) || ids_changed)) {
	...
	if ((tmp_ep[0] != '\0') && (strcmp(tmp_ep, last_partition) != 0)) {
		...
		printf("--- getting current partition: %s\n", tmp_ep);
...
int mtdparts_init(void)
{
	...
	if (!initialized) {
        ...
        printf("Partition list already initialized\n");
    }
...
#if defined(CONFIG_GRENACHE)    // so bootfile set for tftp doesn't override here
    		filename = CFG_IMG_FILENAME;
#else
