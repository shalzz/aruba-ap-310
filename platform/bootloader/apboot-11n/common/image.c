 ...
 * MA 02111-1307 USA
 */
#if defined(powerpc) || defined(USE_HOSTCC) || defined (__ARM_ARCH_7A__)  || defined(CONFIG_MACH_IPQ806x)
#ifndef USE_HOSTCC
#include <common.h>
#include <watchdog.h>
	...
	*rd_end = 0;
	...
	if ((argc >= 3) && (strcmp(argv[2], "-") ==  0)) {
		...
		case IMAGE_FORMAT_FIT:
			...
			debug ("   Trying '%s' ramdisk subimage\n", fit_uname_ramdisk);
	...
	if (argc > 3 || genimg_has_config (images)) {
		...
		case IMAGE_FORMAT_FIT:
			...
			if (fit_check_format ((const void *)fdt_addr)) {
				 ...
				 */
				fit_hdr = (void *)fdt_addr;
				debug ("## Flattened Device Tree from FIT Image at %08lx\n",
						fdt_addr);
				...
				debug ("   Trying '%s' FDT blob subimage\n",
						fit_uname_fdt);
		...
		debug ("   Booting using the fdt blob at 0x%x\n", (int)fdt_blob);
		...
		if (fdt_len) {
			...
			debug ("   Booting using the fdt at 0x%x\n", (int)fdt_blob);
...
void fit_image_print (const void *fit, int image_noffset, const char *p)
{
	...
	debug ("%s  Compression:  %s\n", p, genimg_get_comp_name (comp));

	ret = fit_image_get_data (fit, image_noffset, &data, &size);

#ifndef USE_HOSTCC
	debug ("%s  Data Start:   ", p);
	if (ret)
		debug ("unavailable\n");
	else
		debug ("0x%08lx\n", (ulong)data);
