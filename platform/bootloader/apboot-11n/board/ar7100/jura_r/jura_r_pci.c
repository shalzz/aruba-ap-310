...
#ifdef JURA_PCI_SCAN_WORKAROUND
int  ar7100_local_write_config(int where, int size, uint32_t value);
#else
static int  ar7100_local_write_config(int where, int size, uint32_t value);
#endif
int ar7100_pci_write_config(struct pci_controller *hose, pci_dev_t dev, int offset, u32 value);
...
{
    u32     BaseAddr[3] = {0, 0x10000000, 0x10010000};
#if 0
    ...
    /*
    ** For Jura, we have two Merlins, but bus IDs are 1 and 2, not 0 and 1
    */
