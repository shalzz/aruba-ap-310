...
#ifdef CONFIG_JURA_R
  // select extended LED mode operation
  ag7100_mii_write(unit, phy_addr, 0x1f,  0x1);	// extended register page

  // the duplicate write below is intentional
  ag7100_mii_write(unit, phy_addr, 0x11,  0x0);
  ag7100_mii_write(unit, phy_addr, 0x11,  0x0);

  // the duplicate write below is intentional
  ag7100_mii_write(unit, phy_addr, 0x11,  0x10);
  ag7100_mii_write(unit, phy_addr, 0x11,  0x10);

  ag7100_mii_write(unit, phy_addr, 0x10,  0x3e16);

  ag7100_mii_write(unit, phy_addr, 0x1f,  0x0);	// normal register page
#endif
}
