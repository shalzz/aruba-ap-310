...
//    gpio_mask &= ~((1<<GPIO_TPM_CLK) | (1 << GPIO_TPM_DATA));
    ...
    gpio_set_as_output(GPIO_PSE_CONTROL);
    gpio_set_as_output(GPIO_GREEN_STATUS_LED);
    gpio_set_as_output(GPIO_AMBER_STATUS_LED);
    gpio_set_as_output(GPIO_GREEN_PSE_LED);
    gpio_set_as_output(GPIO_AMBER_PSE_LED);


    /* disable USB power */
    gpio_out(GPIO_USB_PWR, 0);
    /* turn of PSE led green */
    gpio_out(GPIO_GREEN_PSE_LED, 0);
    /* turn of PSE led amber */
    gpio_out(GPIO_AMBER_PSE_LED, 0);
...
late_board_init(void)
{
    ...
    ar5416_install_callback(balvenie_radio_callback);
    ar5416_initialize();

#ifdef CONFIG_RESET_BUTTON
    execute_config_clear();
#endif
    enable_ethernet_ports(0);
...
{
    int dc = !gpio_value(GPIO_POWER_DC);                 /* active low */
...
//    printf("POE+:            %s\n", gpio_value(GPIO_POWER_POE_PLUS) ? "high" : "low");
    ...
    printf("Setting leds as green\n");
    gpio_out(GPIO_GREEN_STATUS_LED, 1);
    gpio_out(GPIO_GREEN_PSE_LED, 1);
    gpio_out(GPIO_AMBER_STATUS_LED, 0);
    gpio_out(GPIO_AMBER_PSE_LED, 0);
 
    udelay(8000000);
    printf("Setting leds as amber\n");
    gpio_out(GPIO_AMBER_STATUS_LED, 1);
    gpio_out(GPIO_AMBER_PSE_LED, 1);
    gpio_out(GPIO_GREEN_STATUS_LED, 0);
    gpio_out(GPIO_GREEN_PSE_LED, 0);

    udelay(8000000);
    printf("Setting leds as red\n");
    gpio_out(GPIO_GREEN_PSE_LED, 0);
    gpio_out(GPIO_GREEN_STATUS_LED, 0);
    gpio_out(GPIO_AMBER_STATUS_LED, 0);
    gpio_out(GPIO_AMBER_PSE_LED, 0);
