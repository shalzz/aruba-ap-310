...
/* scratch-pad address used for Dalmore-LTE */
/* (aka, last word of 32k On-Chip SRAM) */
#define DALMORE_SCRATCH_PAD 0xbd007ffc
...
#endif
{
    ...
    ar7240_reg_wr(AR7240_GPIO_SET, 0x007cf83c);  /* bits 2, 3, 4, 5, 11, 12, 13, 14, 15, 18, 19, 20, 21, & 22 high */
    ar7240_reg_wr(AR7240_GPIO_OE, 0x00021b0b);  /* set appropriate gpio direction (retain initial jtag signal direction) */
    ...
    ar7240_reg_wr(AR934X_GPIO_OUT_FUNCTION3, 0x00000700);  /* (15-12) = 0, 0, SPI_CS_1 (0x07), 0 */
    ar7240_reg_wr(AR934X_GPIO_OUT_FUNCTION4, 0x0000002b);  /* (19-16) = 0, GPIO18_Ch0 (0x2e), 0, LINK[2] (0x2b) */
    ...
    ar7240_reg_wr(AR7240_GPIO_OE, 0x00021b03);          /* set pin 3 as an output for LED_5GHZ_RED */
#else
    ...
    ar7240_gpio_config_output(GPIO_LTE_AMBER_LED);
    ar7240_gpio_config_output(GPIO_LTE_GREEN_LED);
    ...
    ar7240_gpio_out_val(GPIO_LTE_AMBER_LED, 1);
    ar7240_gpio_out_val(GPIO_LTE_GREEN_LED, 1);

    // turn off red status led
    /* On Rev C, the contorl logic for red status led is changed 
     * 1 - on
     * 0 -off
     */
...
{
    ar7240_reg_wr(DALMORE_SCRATCH_PAD, byte);
}

unsigned char
scratch_read(void)
{
    return (ar7240_reg_rd(DALMORE_SCRATCH_PAD) & 0xff);
}
...
static void
dalmore_lte_radio_callback(int n)
{
   n_radios += n;
}
#endif
...
#ifdef CONFIG_AR5416
    ar5416_install_callback(dalmore_lte_radio_callback);
    ar5416_initialize();
#endif
#ifdef CONFIG_ATH_WMAC
    ath_wmac_install_callback(dalmore_lte_radio_callback);
