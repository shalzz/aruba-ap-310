 ...
 * Bit7:4 could adjust the reference voltage, 25mv per one step
 */
#define AR934X_CORE_VOLTAGE 0x18116c40
#define AR934X_CORE_VOLTAGE_SET(x) (((x) << 4) & 0xf0)
...
#endif
{
    ...
    ar7240_reg_wr(AR7240_GPIO_SET, 0x0025b838);  /* bits 3, 4, 5, 11, 12, 13, 15, 16, 18, 21 high */
    ...
    ar7240_gpio_config_output(GPIO_AMBER_STATUS_LED);
#endif
...
late_board_init(void)
{
    ...
    /* Per HW, increase core voltage by 50mV, the default value is 7, 
     * 25mV per one step, so set it to 9
     */
    unsigned int val;

    val = ar7240_reg_rd(AR934X_CORE_VOLTAGE);
    val &= 0xffffff0f;
    ar7240_reg_wr(AR934X_CORE_VOLTAGE, val | AR934X_CORE_VOLTAGE_SET(9));
