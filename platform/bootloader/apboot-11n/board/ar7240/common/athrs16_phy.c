...
void athrs16_reg_init()
{
    ...
    /* turn 1000 green LED on at all rates to get amber for 10/100 */
    athrs16_reg_write(0xb0, 0xcf35cf35);
#endif
...
athrs16_phy_setup(int ethUnit)
{
    ...
    for (phyUnit=0; phyUnit < ATHR_PHY_MAX; phyUnit++) {
        if ((phyUnit == 1) || (!ATHR_IS_ETHUNIT(phyUnit, ethUnit)))
            continue;
        phyBase = ATHR_PHYBASE(phyUnit);
        phyAddr = ATHR_PHYADDR(phyUnit);
        phy_reg_write(phyBase, phyAddr, ATHR_PHY_CONTROL, ATHR_CTRL_POWER_DOWN);
        ATHR_ETHUNIT(phyUnit) = -1;  // set invalid ethUnit for this port
    }
#endif
...
#ifdef CONFIG_SCAPA_H
int
do_s16write(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
    uint32_t reg, val;
    ...
    reg = simple_strtoul(argv[1], (char **)0, 10);
...
int
do_s16read(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
    uint32_t reg, val;
    ...
    val = athrs16_reg_read(reg);
    printf("Read 0x%x from 0x%x\n", val, reg);
    return 0;
}

int
do_s16init(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
    if (argc != 1) {
        printf("Usage: \n%s\n", cmdtp->usage);
        return 1;
...
U_BOOT_CMD(
	s16write,    3,    1,     do_s16write,
	"s16write    - write to 8316 switch\n",
	" Usage: s16write \n"
);

U_BOOT_CMD(
	s16read,    2,    1,     do_s16read,
	"s16read    - read to 8316 switch\n",
	" Usage: s16read \n"
);

U_BOOT_CMD(
	s16init,    1,    1,     do_s16init,
	"s16init    - init 8316 switch\n",
	" Usage: s16init \n"
);
#endif
