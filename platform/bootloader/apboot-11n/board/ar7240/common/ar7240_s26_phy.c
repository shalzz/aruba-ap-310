...
int
do_s26write(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
    unsigned reg, val;
...
int
do_s26read(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
    unsigned reg, val;
    ...
    val = athrs26_reg_read(reg);
    printf("Read 0x%x from 0x%x\n", val, reg);
    return 0;
}

int
do_s26init(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
    if (argc != 1) {
        printf("Usage: \n%s\n", cmdtp->usage);
        return 1;
...
U_BOOT_CMD(
	s26write,    3,    1,     do_s26write,
	"s26write    - write to 8226 switch\n",                   
	" Usage: s26write \n"
);

U_BOOT_CMD(
	s26read,    2,    1,     do_s26read,
	"s26read    - read to 8226 switch\n",                   
	" Usage: s26read \n"
);

U_BOOT_CMD(
	s26init,    1,    1,     do_s26init,
	"s26init    - init 8226 switch\n",                   
	" Usage: s26init \n"
);
#endif
