...
/* The phy address of GE0 for Hazelburn is 0x4 */
#define ATHR_PHY0_ADDR   0x4
#else
#define ATHR_PHY0_ADDR   0x0
#endif
#define ATHR_PHY1_ADDR   0x0
#define ATHR_PHY2_ADDR   0x0
#define ATHR_PHY3_ADDR   0x0
#define ATHR_PHY4_ADDR   0x0
...
/*********************************************************************/
void marvell_reg_init(void)
{
    /* PHY initialization per Release Notes
       (MV-S302033-00_88E1510_88E1518_88E1512_RevA0_RN_04292011.pdf) */
	phy_reg_write(0, 0x00, 0x16, 0x00FF);
	phy_reg_write(0, 0x00, 0x11, 0x214B);
	phy_reg_write(0, 0x00, 0x10, 0x2144);
	phy_reg_write(0, 0x00, 0x11, 0x0C28);
	phy_reg_write(0, 0x00, 0x10, 0x2146);
	phy_reg_write(0, 0x00, 0x11, 0xB233);
	phy_reg_write(0, 0x00, 0x10, 0x214D);
	phy_reg_write(0, 0x00, 0x11, 0xCC0C);
	phy_reg_write(0, 0x00, 0x10, 0x2159);
	phy_reg_write(0, 0x00, 0x16, 0x00FB);
	phy_reg_write(0, 0x00, 0x07, 0xC00D);
	phy_reg_write(0, 0x00, 0x16, 0x0000);

	/* see description of 21_2.3:4 in MV-S107146-00 */
	/* SCA; assume we want a gig link and set the PHY delay accordingly */
	phy_reg_write(0, 0x00, 0x16, 0x02);	// set page 2
	phy_reg_write(0, 0x00, 0x15, (phy_reg_read(0, 0x00, 0x15) & 0xFFEF));

	/* Disable CLK125 pin output due to EMI issue */
	phy_reg_write(0, 0x00, 0x10, (phy_reg_read(0, 0x00, 0x10) | 0x2));

	phy_reg_write(0, 0x00, 0x16, 0x00);	// set page 0

	phy_reg_write(0, 0x00, 0x00, 0x9000);

	ar7240_reg_wr(AG7240_ETH_CFG, 0x3c001);

	/* Setup giga ethernet port LED function */
	phy_reg_write(0, 0x00, 0x16, 0x03);	// set page 3
	phy_reg_write(0, 0x00, 0x11, 0x4410);
	phy_reg_write(0, 0x00, 0x10, 0x1187);
        phy_reg_write(0, 0x00, 0x16, 0x00);	// set page 0
	
	if((phy_reg_read(0, 0x00, 0x00) & 0x1000))
	    athr17_init_flag = 1;
