...
static uint16_t
ag7240_mii_read(uint32_t phybase, uint16_t phyaddr, uint16_t reg)
{
    uint16_t val;

    val = phy_reg_read(phybase, phyaddr, reg);
    return val;
}

static void
ag7240_mii_write(uint32_t phybase, uint16_t phyaddr, uint16_t reg, uint32_t val)
{
...
/* Base read/write routines built on ag7240_mii_ macros */

static inline void
ar8021_phy_mii_write_main(int unit, uint32_t phy_addr, uint8_t reg, uint16_t data)
{
  ag7240_mii_write(unit, phy_addr, 31,  0);
...
{
  ag7240_mii_write(unit, phy_addr, 31,  1);
  ag7240_mii_write(unit, phy_addr, reg, data);
}
...
{
  ag7240_mii_write(unit, phy_addr, 31,  0);
  return ag7240_mii_read(unit, phy_addr, reg);
}
...
ar8021_phy_mii_read_ex(int unit, uint32_t phy_addr, uint8_t reg)
{
  uint16_t val;
  ag7240_mii_write(unit, phy_addr, 31,  1);
  val = ag7240_mii_read(unit, phy_addr, reg);
  ag7240_mii_write(unit, phy_addr, 31,  0);
  return val;
}
...
#ifdef CONFIG_AR9100
  ag7240_mii_write(unit, phy_addr, 0x1f,  0x1);
  ag7240_mii_write(unit, phy_addr, 0x1c,  0x3000);
  ag7240_mii_write(unit, phy_addr, 0x1f,  0x0);
#endif
}

#ifdef CONFIG_SCAPA
void 
ar8021_skew_clock(uint16_t unit)
{
  // magic from Atheros F1E code
  int phy_addr = 0; // XXX
  ag7240_mii_write(unit, phy_addr, 0x1d, 0x0);	// select debug reg 0
  ag7240_mii_write(unit, phy_addr, 0x1e, 0x34e);	// delay RX clock
  ag7240_mii_write(unit, phy_addr, 0x1d, 0x5);	// select debug reg 5
  ag7240_mii_write(unit, phy_addr, 0x1e, 0x3d47);	// delay TX clock
}

void 
ar8021_unskew_clock(uint16_t unit)
{
  int phy_addr = 0; // XXX
  // magic from Atheros F1E code
  ag7240_mii_write(unit, phy_addr, 0x1d, 0x0);	// select debug reg 0
  ag7240_mii_write(unit, phy_addr, 0x1e, 0x24e);	// delay RX clock
  ag7240_mii_write(unit, phy_addr, 0x1d, 0x5);	// select debug reg 5
  ag7240_mii_write(unit, phy_addr, 0x1e, 0x3c47);	// delay TX clock
}
#endif
...
ar8021_phy_setup_ar8021(uint16_t unit, uint16_t phy_addr)
{
  ...
  ag7240_mii_write(unit, phy_addr, 0x1e, 0x34e);	// delay RX clock
  ag7240_mii_write(unit, phy_addr, 0x1d, 0x5);	// select debug reg 5
  ag7240_mii_write(unit, phy_addr, 0x1e, 0x100);	// delay TX clock
#endif
  ar8021_phy_mii_set_duplex(unit, phy_addr, 1);
...
/* ***********************************************************
 *
 * These are exported for use by ag7240_phy.h
 *
 * *********************************************************** */
