...
void ardmore_power_down_usb(void)
{
...
void board_gpio_init(void)
{
    ...
    gpio_set_as_output(GPIO_PCI_RST);
    gpio_set_as_output(GPIO_TPM_RST_L);
...
static void
ardmore_disable_unused_blocks(void)
{
    ccsr_gur_t *gur = (void *)(CONFIG_SYS_MPC85xx_GUTS_ADDR);
    unsigned val;
...
ardmore_phy_init(void)
{
    ...
    for (idx = 0; idx < 2; idx++) {
        ...
        miiphy_write("eth0", idx, 0x10, 0x1777);
        /* reverse polarity on LED3 and set LEDs 0-2 to tri-state when off */
        miiphy_write("eth0", idx, 0x11, 0x88ea);
...
int board_early_init_r(void)
{
    ...
    /* reset radios */
    gpio_out(GPIO_PCI_RST, 0);
    udelay(80 * 1000);
    gpio_out(GPIO_PCI_RST, 1);
    
    /* reset TPM */
    gpio_out(GPIO_TPM_RST_L, 0);
    for (i = 0; i < 7; i++) {   /* 210 ms */
        udelay(30 * 1000);
    }
...
late_board_init(void)
{
    ...
    ardmore_phy_init();
#endif
    ar5416_install_callback(ardmore_radio_callback);
    ar5416_initialize();
