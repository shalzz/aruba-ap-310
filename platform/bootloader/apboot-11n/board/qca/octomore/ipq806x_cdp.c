...
static void octomore_gpio_init(void)
{
 ...
 */
extern int manuf_is_fips_ap(void);
...
int board_late_init(void)
{
    ...
    /* if FIPS AP, hold BLE in reset */
    if (manuf_is_fips_ap()) {
        gpio_tlmm_config(GPIO_BLE_RST_L, 0, GPIO_OUTPUT, GPIO_PULL_UP, GPIO_10MA, GPIO_DISABLE);
        gpio_out(GPIO_BLE_RST_L, 0);
    }
...
int get_eth_mac_address(uchar *enetaddr, uint no_of_macs)
{
    ...
    tmp = addr;
    mac = enetaddr;
    /* we assume octomore has two ethernet ports, and manufacturing
...
late_board_init(void)
{
    ...
    ar5416_install_callback(octomore_radio_callback);
    ar5416_initialize();
#endif
