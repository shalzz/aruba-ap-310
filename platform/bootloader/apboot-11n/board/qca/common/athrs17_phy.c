...
void m88e1514_reg_init(uint32_t phy_addr)
{
	uint8_t phy_reg;
	uint16_t phy_val;

    phy_val = phy_reg_read(0, phy_addr, 2);
    
	/* 1. 10/100/1000BASE-T Auto-Neg */
	/* Set register 0 as 0x9140 */
	phy_reg = 0x0; // 0
	phy_val = 0x9140;
	phy_reg_write(0, phy_addr, phy_reg, phy_val);
	...
	/* Set register 20 as 0x0201 */
	phy_reg = 0x14; // 20
	phy_val = 0x0201;
	phy_reg_write(0, phy_addr, phy_reg, phy_val);

	/* Set register 20 as 0x8201 */
	phy_reg = 0x14; // 20
	phy_val = 0x8201;
	phy_reg_write(0, phy_addr, phy_reg, phy_val);

	/* 3. Standard SGMII Auto-Neg */
	/* Set page (22) to 1 */
	phy_reg = 0x16; // 22
	phy_val = 0x1; // 1
	phy_reg_write(0, phy_addr, phy_reg, phy_val);
	...
	/* 4. Come back to page 0 */
	phy_reg = 0x16; // 22
	phy_val = 0x0; // 0
	phy_reg_write(0, phy_addr, phy_reg, phy_val);

	/* Set register 9 as 0xe00 */
	phy_reg = 0x9; // 9
	phy_val = 0xe00;
	phy_reg_write(0, phy_addr, phy_reg, phy_val);

	/* Set register 4 as 0x1e1 */
	phy_reg = 0x4; // 4
	phy_val = 0x1e1;
	phy_reg_write(0, phy_addr, phy_reg, phy_val);
...
void m88e1512_reg_init(uint32_t phy_addr, int sgmii, int re_init)
{
	uint16_t phy_val;
	uint16_t phy_id[2];
	uint8_t phy_reg;
	 ...
	 ***********************************/
	if (!re_init) {
	phy_reg_write(0, phy_addr, 0x16, 0);
	phy_id[0] = phy_reg_read(0, phy_addr, 2);
	phy_id[1] = phy_reg_read(0, phy_addr, 3);

	phy_reg_write(0, phy_addr, 0x16, 6);
	phy_val = phy_reg_read(0, phy_addr, 26);

	printf("PHY id 0x%x rev %d temp %dC/%dC, ", 
			(phy_id[0] << 16) | (phy_id[1] & 0xfff0), phy_id[1] & 0x0f,
			(phy_val & 0x1f) * 5 - 25,
			((phy_val >> 8) & 0x1f) * 5 - 25);
	}
	...
	/************************************
	 *     Select Copper/SGMII Auto     *
	 ***********************************/
	/* Slect page 18 */
	phy_reg_write(0, phy_addr, 0x16, 18);
	/* RGMII to Copper/SGMII (6) */
	phy_reg = 0x14; // 20
	phy_val = phy_reg_read(0, phy_addr, phy_reg);
	phy_val &= ~0x7f;
	if (sgmii) {
		phy_val |= 0x4;
	}
	...
	/************************************
	 *     PAGE 1 - SGMII settings      *
	 ***********************************/
	/* Select page 1 */
	phy_reg_write(0, phy_addr, 0x16, 1);

	/* Auto-NEG enabled and restart */
	phy_reg = 0x0;
	phy_val = phy_reg_read(0, phy_addr, phy_reg);
	phy_val = 0x9340;
	phy_reg_write(0, phy_addr, phy_reg, phy_val);
