...
extern void i2c_clock_config(unsigned int gsbi_port, unsigned int m,
...
static int i2c_hw_init(void)
{
	...
	if (ret) {
		//printf("config_i2c_state RESET failed %d\n", ret);
		return ret;
	}
	...
	if (ret) {
		//printf("config_i2c_state RUN failed %d\n", ret);
		return ret;
	}
...
int i2c_read(uchar chip, uint addr, int alen, uchar *buffer, int len)
{
	...
	if (ret != SUCCESS) {
		//printf("i2c_read: check_write_done\n");
		goto out;
	}

	ret = check_fifo_status(WRITE);
	if (ret != SUCCESS) {
		printf("i2c_read: check_fifo_status\n");
		goto out;
	}
	...
	while (len--) {
		...
		if (ret != SUCCESS) {
			//printf("i2c_read: check_fifo_status - 2\n");
			goto out;
		}
	...
	if (ret != SUCCESS) {
		//printf("i2c_read: QUP_STATE_PAUSE\n");
		goto out;
	}
...
#if 0
	if (ret != SUCCESS) {
		printf("***QUP REGISTERS DUMP***\n");
		printf("QUP_CONFIG=0x%x\n", readl(GSBI_QUP_BASE(gsbi_port) + QUP_CONFIG_OFFSET));
		printf("QUP_STATE=0x%x\n", readl(GSBI_QUP_BASE(gsbi_port) + QUP_STATE_OFFSET));
		printf("QUP_IO_MODES=0x%x\n", readl(GSBI_QUP_BASE(gsbi_port) + QUP_IO_MODES_OFFSET));
		printf("QUP_SW_RESET=0x%x\n", readl(GSBI_QUP_BASE(gsbi_port) + QUP_SW_RESET_OFFSET));
		printf("QUP_TIME_OUT=0x%x\n", readl(GSBI_QUP_BASE(gsbi_port) + QUP_TIME_OUT_OFFSET));
		printf("QUP_TIME_OUT_CURRENT=0x%x\n", readl(GSBI_QUP_BASE(gsbi_port) + QUP_TIME_OUT_CURRENT_OFFSET));
		printf("QUP_OPERATIONAL=0x%x\n", readl(GSBI_QUP_BASE(gsbi_port) + QUP_OPERATIONAL_OFFSET));
		printf("QUP_ERROR_FLAGS_MODES=0x%x\n", readl(GSBI_QUP_BASE(gsbi_port) + QUP_ERROR_FLAGS_OFFSET));
		printf("QUP_ERROR_FLAGS_EN=0x%x\n", readl(GSBI_QUP_BASE(gsbi_port) + QUP_ERROR_FLAGS_EN_OFFSET));
		printf("QUP_TEST_CTRL=0x%x\n", readl(GSBI_QUP_BASE(gsbi_port) + QUP_TEST_CTRL_OFFSET));
		printf("QUP_MX_OUTPUT_COUNT=0x%x\n", readl(GSBI_QUP_BASE(gsbi_port) + QUP_MX_OUTPUT_COUNT_OFFSET));
		printf("QUP_MX_OUTPUT_CNT_CURRENT=0x%x\n", readl(GSBI_QUP_BASE(gsbi_port) + QUP_MX_OUTPUT_CNT_CURRENT_OFFSET));
		printf("QUP_OUTPUT_DEBUG=0x%x\n", readl(GSBI_QUP_BASE(gsbi_port) + QUP_OUTPUT_DEBUG_OFFSET));
		printf("QUP_OUTPUT_FIFO_WORD_CNT=0x%x\n", readl(GSBI_QUP_BASE(gsbi_port) + QUP_OUTPUT_FIFO_WORD_CNT_OFFSET));
		printf("QUP_OUTPUT_FIFO=0x%x\n", readl(GSBI_QUP_BASE(gsbi_port) + QUP_OUTPUT_FIFO_OFFSET));
		printf("QUP_MX_WRITE_COUNT=0x%x\n", readl(GSBI_QUP_BASE(gsbi_port) + QUP_MX_WRITE_COUNT_OFFSET));
		printf("QUP_WRITE_CNT_CURRENT=0x%x\n", readl(GSBI_QUP_BASE(gsbi_port) + QUP_WRITE_CNT_CURRENT_OFFSET));
		printf("QUP_MX_INPUT_COUNT=0x%x\n", readl(GSBI_QUP_BASE(gsbi_port) + QUP_MX_INPUT_COUNT_OFFSET));
		printf("QUP_READ_COUNT=0x%x\n", readl(GSBI_QUP_BASE(gsbi_port) + QUP_MX_READ_COUNT_OFFSET));
		printf("QUP_READ_CNT_CURRENT_OFFSET=0x%x\n", readl(GSBI_QUP_BASE(gsbi_port) + QUP_MX_READ_CNT_CURRENT_OFFSET));
		printf("QUP_INPUT_DEBUG=0x%x\n", readl(GSBI_QUP_BASE(gsbi_port) + QUP_INPUT_DEBUG_OFFSET));
		printf("QUP_INPUT_FIFO_WORD_CNT=0x%x\n", readl(GSBI_QUP_BASE(gsbi_port) + QUP_INPUT_FIFO_WORD_CNT_OFFSET));
		printf("QUP_INPUT_FIFO=0x%x\n", readl(GSBI_QUP_BASE(gsbi_port) + QUP_INPUT_FIFO_OFFSET));
		printf("QUP_I2C_MASTER_CLK_CTL=0x%x\n", readl(GSBI_QUP_BASE(gsbi_port) + QUP_I2C_MASTER_CLK_CTL_OFFSET));
		printf("QUP_I2C_MASTER_STATUS=0x%x\n", readl(GSBI_QUP_BASE(gsbi_port) + QUP_I2C_MASTER_STATUS_OFFSET));
		while (1);
	}
#endif
	/*
...
#ifdef CONFIG_EIGER
int i2c_read_half_word(uchar chip, uint addr, int alen, uchar *buffer)
{
	int ret = 0;
	unsigned int data = 0;
	int len = 2;
