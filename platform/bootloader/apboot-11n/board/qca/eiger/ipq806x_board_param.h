...
gpio_func_data_t i2c_input_gpio[] = {
	{
		.gpio = GPIO_SFP_TX_FAULT,
		.dir = GPIO_INPUT,
	},
	{
		.gpio = GPIO_SFP_LOS,
...
board_ipq806x_params_t board_params[] = {
	...
	},
	{
		...
		.mnd_value = { 12, 625, 313 },
#ifdef CONFIG_EIGER
        /* CM on RGMII with PHY Address = 0, GMAC1(.base = 0); 
           Marvell 88E1512 on RGMII with PHY Address = 0, GMAC2(.base = 1) */
		.gmac_gpio_count = ARRAY_SIZE(gmac1_gpio),
		.gmac_gpio = gmac1_gpio,
		.gmac_cfg = {
			gmac_board_cfg(0, 1, RGMII, 0, 0, 0, 1, 31),
			gmac_board_cfg(1, 1, RGMII, 0, 0, 0, 1, 0),
			//gmac_board_cfg_invalid(),
			gmac_board_cfg_invalid(),
			gmac_board_cfg_invalid(),
		},
