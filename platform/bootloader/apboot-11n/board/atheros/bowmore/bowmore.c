...
/* scratch-pad address used for Bowmore */
/* (aka, last word of 32k On-Chip SRAM) */
#define BOWMORE_SCRATCH_PAD 0xbd007ffc
...
{
    ath_reg_wr(BOWMORE_SCRATCH_PAD, byte);
}

unsigned char
scratch_read(void)
{
    return (ath_reg_rd(BOWMORE_SCRATCH_PAD) & 0xff);
}
...
#ifdef CONFIG_AR5416
    ar5416_install_callback(bowmore_radio_callback);
    ar5416_initialize();
#endif
#ifdef CONFIG_ATH_WMAC
    ath_wmac_install_callback(bowmore_radio_callback);
