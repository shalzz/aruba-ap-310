...
int ath_gmac_miiphy_read_wrap(char *devname, unsigned char phy_addr, unsigned char reg, unsigned short *value);
int ath_gmac_miiphy_write_wrap(char *devname, unsigned char phy_addr, unsigned char reg, unsigned short value);
...
unsigned xmii_10_val  = 0xa4001313;
unsigned xmii_100_val = 0xa4000101;
...
#ifdef ATH_RGMII_CAL
extern void rgmii_cal_alg(void *);
#else
//extern void rgmii_cal_alg(void);
#endif
...
//		printf("Scorpion  ----> S17 PHY *\n");
                ...
                /* 40MHz / 58 = 670KHz (400KHz(min) < 700KHz(typ) < 1MKHz(max)) */
                mgmt_cfg_val = 0xb;
#else
		mgmt_cfg_val = 7;
#endif
...
static void ath_gmac_hw_start(ath_gmac_mac_t *mac)
{
	...
	ath_gmac_reg_wr(mac, ATH_MAC_FIFO_CFG_5, 0x66be2);
#else
...
#define TXFIFO_TH_ADDRESS               0x000001a4
#define TXFIFO_TH_VAL			0x01d80160
#define TXFIFO_TH_HD_VAL		0x00880060

#define AMCXFIF_CFG_3_VAL		0x01f00140
#define AMCXFIF_CFG_3_HD_VAL		0x00f00040

static int ath_gmac_check_link(ath_gmac_mac_t *mac, int verbose)
{
	...
	switch (speed)
	{
...
#ifndef CONFIG_BOWMORE
			ath_reg_wr(ETH_XMII_ADDRESS, ETH_XMII_TX_INVERT_SET(1) |
						     ETH_XMII_RX_DELAY_SET(2)  |
						     ETH_XMII_TX_DELAY_SET(1)  |
						     ETH_XMII_GIGE_SET(1));
#if 1
			if (mac->mac_unit == 0 && duplex == 0) {
			   ath_gmac_reg_wr(mac, TXFIFO_TH_ADDRESS, TXFIFO_TH_HD_VAL);
			   ath_gmac_reg_wr(mac, ATH_MAC_FIFO_CFG_3, AMCXFIF_CFG_3_HD_VAL);
			} else if (mac->mac_unit == 0 && duplex == 1) {
			   ath_gmac_reg_wr(mac, TXFIFO_TH_ADDRESS, TXFIFO_TH_VAL);
			   ath_gmac_reg_wr(mac, ATH_MAC_FIFO_CFG_3, AMCXFIF_CFG_3_VAL);
			}
#endif
#endif
...
#if defined(CONFIG_ARDBEG)
                        ath_reg_wr(ATH_ETH_CFG, ETH_CFG_ETH_RXDV_DELAY_SET(3) | 
                                ETH_CFG_ETH_RXD_DELAY_SET(3) | 0x1);
 
		        val = ath_gmac_miiphy_read("eth0", 0, 0x19);
			val &= ~0xc0;
			ath_gmac_miiphy_write("eth0", 0, 0x19, val);
#if 0
			/* disable TX delay */
			ath_gmac_miiphy_write("eth0", 0, 0x1d, 0x5);
		        val = ath_gmac_miiphy_read("eth0", 0, 0x1e);
			val &= ~0x100;
			ath_gmac_miiphy_write("eth0", 0, 0x1e, val);
#endif
#endif
#ifdef CONFIG_BOWMORE
			if (mac->mac_unit == 0) {
		...
		case _100BASET:
			...
			ath_reg_wr(ETH_XMII_ADDRESS, ETH_XMII_TX_INVERT_SET(1) |
						     ...
						     ETH_XMII_TX_DELAY_SET(1));
#endif
			ath_reg_wr(ETH_XMII_ADDRESS, xmii_100_val);
		        val = ath_gmac_miiphy_read("eth0", 0, 0x19);
			val |= 0xc0;
			ath_gmac_miiphy_write("eth0", 0, 0x19, val);
#if 0 /* looks the PHY delay isn't needed */
			/* add TX delay */
			ath_gmac_miiphy_write("eth0", 0, 0x1d, 0x5);
		        val = ath_gmac_miiphy_read("eth0", 0, 0x1e);
			val |= 0x100;
		...
		case _10BASET:
			...
			ath_reg_wr(ETH_XMII_ADDRESS, ETH_XMII_TX_INVERT_SET(1) |
						     ...
						     ETH_XMII_TX_DELAY_SET(1));
#endif
			ath_reg_wr(ETH_XMII_ADDRESS, xmii_10_val);
		        val = ath_gmac_miiphy_read("eth0", 0, 0x19);
			val |= 0xc0;
			ath_gmac_miiphy_write("eth0", 0, 0x19, val);
#if 0 /* looks the PHY delay isn't needed */
...
//			printf("%s: reset mask:%x \n", __func__, mask);
		...
		eth_register(dev[i]);
#if(CONFIG_COMMANDS & CFG_CMD_MII)
		miiphy_register(dev[i]->name, ath_gmac_miiphy_read_wrap, ath_gmac_miiphy_write_wrap);
#endif
...
#ifdef CONFIG_ARDBEG_notyet
	ath_gmac_check_link(ath_gmac_macs[0], 0);
#endif
	return 1;
}
...
int
ath_gmac_miiphy_read_wrap(char *devname, unsigned char phy_addr, unsigned char reg, unsigned short *value)
{
	*value = ath_gmac_miiphy_read(devname, phy_addr, reg);
	return 0;
}
...
int
ath_gmac_miiphy_write_wrap(char *devname, unsigned char phy_addr, unsigned char reg, unsigned short value)
{
	ath_gmac_miiphy_write(devname, phy_addr, reg, value);
	return 0;
}
...
int 
do_xmii( cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
    uint32_t val;
    uint32_t type;
    ...
    if (type == 100) {
    	xmii_100_val = val;
        printf("XMII for 100 Mb/s is now 0x%x\n", xmii_100_val);
    } else if (type == 10) {
    	xmii_10_val = val;
        printf("XMII for 10 Mb/s is now 0x%x\n", xmii_10_val);
    } else {
    	printf("bad type: must be 10 or 100\n");
	return 1;
    }
...
U_BOOT_CMD(
	xmii,    4,    1,     do_xmii,
	"xmii    - set XMII register value for 10 or 100\n",
	"\n    - set XMII register value for 10 or 100\n"
