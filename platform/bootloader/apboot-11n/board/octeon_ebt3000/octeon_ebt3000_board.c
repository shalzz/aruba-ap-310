...
int checkboard (void)
{
...
#endif
    if (octeon_show_info() && octeon_ebt3000_get_board_major_rev() != 1)
    {
    ...
    if (octeon_show_info())
    {
        ...
        {
            char tmp[10];
            sprintf(tmp, "%dMHz    ", gd->cpu_clock_mhz);
            octeon_led_str_write(tmp);
        }
    }
    else
...
int early_board_init(void)
{
    ...
    else
    {
        ...
        gd->board_desc.board_type = CVMX_BOARD_TYPE_EBT3000;
        /* Try to determine board rev by looking at PAL */
        if (octeon_ebt3000_rev1())
        {
    ...
    if (gd->board_desc.rev_major == 1)
    {
        ...
        else
        {
            ...
            /* Default values */
            cpu_ref = 33;
            gd->ddr_clock_mhz = EBT3000_REV1_DEF_DRAM_FREQ;
        }
    }
    else
    {
