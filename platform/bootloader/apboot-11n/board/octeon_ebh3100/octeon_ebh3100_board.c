...
int checkboard (void)
{
    ...
    ** by the bootloader, and will configure the interface accordingly */
    ...
    uint16_t value, device, reg;

    if (marvell_sense_multi_chip_mode())
    {
        /* Early boards had the switch (mis)configured, so handle that case here */
...
int early_board_init(void)
{
    ...
    else
    {
        ...
        gd->board_desc.board_type = CVMX_BOARD_TYPE_EBH3100;
        gd->board_desc.rev_major = 2;   /* Default to rev 2, as rev 1 was limited production due to ddr chip select miswiring */
    }
