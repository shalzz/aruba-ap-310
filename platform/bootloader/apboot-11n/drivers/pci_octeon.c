				    ...
				    unsigned long flags)
{
	...
	int i;
        ...
        DEBUGF("pci_hp2b: looking up phys: 0x%08x, flags: 0x%x\n", phys_addr, flags);
	if (!hose) {
	...
	for (i = 0; i < hose->region_count; i++) {
		res = &hose->regions[i];
                DEBUGF("pci_hp2b: checking region %d, phys:0x%08x, bus: 0x%08x, size: 0x%08x, flags: 0x%x\n",
                       i, res->phys_start, res->bus_start, res->size, res->flags);
				   ...
				   unsigned long flags)
{
	...
	for (i = 0; i < hose->region_count; i++) {
		...
		if (bus_addr >= res->bus_start &&
			bus_addr < res->bus_start + res->size) {
                    unsigned long phys = bus_addr - res->bus_start + res->phys_start;
                    DEBUGF("pci_hose_bus_to_phys: Hose: %p, bus_addr:0x%08x, flags: 0x%x, phys: 0x%08x\n",
                           hose,  bus_addr, flags, phys);
                    return phys;
		}
	}

	printf ("pci_hose_bus_to_phys: %s: 0x%08x\n", "invalid bus address", bus_addr);
...
int pci_hose_scan_bus(struct pci_controller *hose, int bus)
{
	     ...
	     dev += PCI_BDF(0,0,1))
	{
		...
		DEBUGF("%s: Probing Bus %d, Device %d, Function %d\n", __FUNCTION__,
		...
		if (vendor != 0xffff && vendor != 0x0000) {
			...
			pci_hose_read_config_word(hose, dev, PCI_CLASS_DEVICE, &class);
                        ...
                        if (device == 0x686 && vendor == 0x1106)
                        {
                            uint8_t tmp;
                            printf("Found VIA southbridge, enabling IDE controller\n");
                            /* Enable IDE controller so that it is enumerated */
                            pci_hose_read_config_byte(hose, dev, 0x48, &tmp);
                            tmp &= ~0x2;
                            pci_hose_write_config_byte(hose, dev, 0x48, tmp);

                        } 
                        else if (device == 0x0571 && vendor == 0x1106)
                        {
                            //printf("Found VIA IDE...\n");
                            /* don't need to to anything here for now */
                        }
