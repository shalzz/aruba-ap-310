...
pci_dev_t pci_find_devices(struct pci_device_id *ids, int index)
{
	...
	for (hose = hose_head; hose; hose = hose->next)
	{
//printf("hose %p, f %d, l %d, c %d\n", hose, hose->first_busno, hose->last_busno, hose->current_busno);
...
#else
			     bdf < PCI_BDF(bus,0,PCI_MAX_PCI_FUNCTIONS-1);
#endif
			     bdf += PCI_BDF(0,0,1))
			{
//printf("bdf %x %u/%u/%u\n", bdf, PCI_BUS(bdf), PCI_DEV(bdf), PCI_FUNC(bdf));
				if (!PCI_FUNC(bdf)) {
...
#ifdef CONFIG_PCI_SCAN_SHOW
	unsigned int bar0, bar1, bar2, bar3;
	unsigned char  rev, subclass, code;
#endif
	unsigned char header_type;
//	struct pci_config_table *cfg;
	pci_dev_t dev;
#ifdef CONFIG_PCI_SCAN_SHOW_old
	static int indent = 0;
#endif
...
//			       vendor, device, pci_class_str(class >> 8));
			...
			printf("%02x:%02x.%02x - %04x:%04x - %s\n",
			       PCI_BUS(dev), PCI_DEV(dev), PCI_FUNC(dev), vendor, device, pci_class_str(class >> 8) );			
		}
#endif
...
#ifdef CONFIG_PCI_SCAN_SHOW
		pci_hose_read_config_byte(hose, dev, PCI_CLASS_CODE, &code);
		pci_hose_read_config_byte(hose, dev, PCI_CLASS_SUB_CODE, &subclass);
		pci_hose_read_config_byte(hose, dev, PCI_REVISION_ID, &rev);
		pci_hose_read_config_dword(hose, dev, PCI_BASE_ADDRESS_0, &bar0);
		pci_hose_read_config_dword(hose, dev, PCI_BASE_ADDRESS_1, &bar1);
		pci_hose_read_config_dword(hose, dev, PCI_BASE_ADDRESS_2, &bar2);
		pci_hose_read_config_dword(hose, dev, PCI_BASE_ADDRESS_3, &bar3);

		printf("       %02x  %02x  %04x  %04x %05x   %02x %08x %08x %08x %08x\n", 
		   PCI_DEV(dev), PCI_FUNC(dev), vendor, device, code, rev,
		   bar0, bar1, bar2, bar3);
#endif
...
extern void pci_iproc_init_board (struct pci_controller * hose, unsigned bus);
...
#ifdef CONFIG_PCIE0
	hose = calloc(1, sizeof(*hose));
	if (hose) {
		pci_iproc_init_board(hose, 0);
	}
#endif
#ifdef CONFIG_PCIE1
	hose = calloc(1, sizeof(*hose));
	if (hose) {
		pci_iproc_init_board(hose, 1);
	}
#endif
}
#endif
