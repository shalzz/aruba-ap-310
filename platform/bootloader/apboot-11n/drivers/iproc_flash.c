...
int flash_erase(flash_info_t *info, int s_first, int s_last)
{
    ...
    if (info->is_uniform_sector) {
        len = flash_info[0].size / flash_info[0].sector_count;
    }
    for (i = s_first; i <= s_last; i++) {
        printf(".");
        WATCHDOG_RESET();
        /* if it is non-uniform sector */
        if (!info->is_uniform_sector) {
            /* if we are at the last sector, we need to find out the last sector size 
               by findind the flash ending address - the last sector starting address */
            if (i == CFG_MAX_FLASH_SECT - 1) {
                len = flash_info[0].size + CONFIG_SYS_FLASH_BASE - flash_info[0].start[i-1];
            } else {
                len = flash_info[0].start[i+1] - flash_info[0].start[i] ;
            }
        }
        (void)spi_flash_erase(flash, flash_info[0].start[i] - flash_info[0].start[0], len);
    }
    printf("done\n");
    return 0;
}
