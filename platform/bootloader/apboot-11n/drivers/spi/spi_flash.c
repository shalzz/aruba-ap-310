...
#ifdef CONFIG_APBOOT
void
aruba_fill_in_flash_info(struct spi_flash *spi, unsigned num_sectors, unsigned id)
{
	int i, sect_desc_idx = 0;
	unsigned base = CFG_FLASH_BASE;
	unsigned sec_size = spi->size / num_sectors;
	extern flash_info_t flash_info[];

	flash_info[0].flash_id = id;	// lie
	flash_info[0].size = spi->size;
	flash_info[0].sector_count = num_sectors;
    flash_info[0].is_uniform_sector = spi->is_uniform_sector;
    
	memset(&flash_info[0].protect, 0, sizeof(flash_info[0].protect));

	if (num_sectors > CFG_MAX_FLASH_SECT) {
		num_sectors = CFG_MAX_FLASH_SECT;
	}
	for (i = 0; i < num_sectors; i++) {
		flash_info[0].start[i] = base;
        if (!spi->is_uniform_sector) {
            if (spi->sect_grp_desc == NULL) {
                printf("%s: Flash is not uniform. Sector descriptor is NULL\n",
                         __func__);
                return;
            }
            if (i == spi->sect_grp_desc[sect_desc_idx].start_sector + 
                    spi->sect_grp_desc[sect_desc_idx].nr_sector) {
                sect_desc_idx++;
            }
            if (spi->sect_grp_desc[sect_desc_idx].sect_size != SECT_SIZE_INVAL) {
                sec_size = spi->sect_grp_desc[sect_desc_idx].sect_size; 
            } else {
                printf("%s: Invalid sector size sect_desc_idx=%d\n", __func__, sect_desc_idx);
                return;
            }
        }
		base += sec_size;
