...
static int ahci_host_init(struct ahci_probe_ent *probe_ent)
{
	...
	tmp = readl(mmio + HOST_CTL);
	if (tmp & HOST_RESET) {
		debug("controller reset failed (0x%x) addr (0x%x) \n", tmp, (mmio + HOST_CTL));
		return -1;
	}
	...
	for (i = 0; i < probe_ent->n_ports; i++) {
		...
		/* writel(PORT_CMD_SPIN_UP, port_mmio + PORT_CMD); Yotam */
		...
		/* writel(DEF_PORT_IRQ, port_mmio + PORT_IRQ_MASK); yotam */
	...
	tmp = readl(mmio + HOST_CTL);
	debug("HOST_CTL 0x%x\n", tmp);
	/* writel(tmp | HOST_IRQ_EN, mmio + HOST_CTL); yotam */
	tmp = readl(mmio + HOST_CTL);
	debug("HOST_CTL 0x%x\n", tmp);
...
#if (defined(CONFIG_MARVELL) && defined(__ARM__))
#define bus_to_phys(a) pci_mem_to_phys((pci_dev_t)pdev, a)
#define phys_to_bus(a) (a)
#else
#define bus_to_phys(a)	pci_mem_to_phys((pci_dev_t)pdev, a)
#define phys_to_bus(a)	pci_phys_to_mem((pci_dev_t)pdev, a)
#endif

static int ahci_init_one(pci_dev_t pdev)
{
	u32 iobase, vendor, device, rmw;
	int rc;
	...
	probe_ent->mmio_base = bus_to_phys (iobase);
	...
	/* Take from kernel:
	 * Marvell specific fixup:
	 * Change SATA phy to scontroller mode
	 */
/*
	pci_read_config_word(pdev, PCI_VENDOR_ID, &vendor);
	printf ("ahci_match : vendor 0x%x\n",vendor);
	if (vendor == 0x11ab)
	{
		pci_read_config_word(pdev, PCI_DEVICE_ID, &device);
		device &= 0xffff;
		printf ("ahci_match : device 0x%x\n",device);

		if ((device == 0x6121) || (device == 0x6145)) 
		{
			printf ("ahci_match : Switching SATA phy to be controlled by Scontrol\n");
			pci_read_config_word(pdev, PCI_CB_SUBSYSTEM_VENDOR_ID, &rmw);
			printf ("ahci_match addr 0x40 value 0x%x\n", rmw);
*/
//			rmw = rmw & 0xff3fffff; /* Clear bit 22 and 23 */
/*			printf ("ahci_match addr 0x40 value 0x%x\n", rmw);
			pci_write_config_byte(pdev, PCI_CB_SUBSYSTEM_VENDOR_ID, rmw);
		}
	}
*/
	/* initialize adapter */
...
static int ahci_port_start(u8 port)
{
	...
	mem = (u32) malloc(AHCI_PORT_PRIV_DMA_SZ + 4096);
	if (!mem) {
		free(pp);
...
*/
	writel_with_flush(PORT_CMD_FIS_RX | PORT_CMD_START | 0x20, port_mmio + PORT_CMD);
				...
				int buf_len)
{
	...
	debug("Enter get_ahci_device_data: for port %d addr mmio 0x%x\n", port, port_mmio);
	...
	debug("Enter get_ahci_device_data: fis data\n");
	for (i=0; i < fis_len; i++)
	{
		debug("%p:%02x\n", &fis[i], fis[i]);
	}
	...
	debug("Enter get_ahci_device_data: pp->cmd_tbl 0x%p \n",pp->cmd_tbl );
	for (i=0; i < fis_len; i++)
	{
		debug("%p:%02x\n", &(((unsigned char *)(pp->cmd_tbl))[i]), ((unsigned char *)(pp->cmd_tbl))[i]);
	}
	debug("Enter get_ahci_device_data: buf 0x%x buf_len 0x%x\n",buf,buf_len );
...
static char *ata_id_strcpy(u16 *target, u16 *src, int len)
{
	int i;
	for (i = 0; i < len / 2; i++)
		/* Swap 16bit for display - on memory this data is */
 		/* in big endian */
		target[i] = __swab16(src[i]);
	return (char *)target;
}
...
static void dump_ataid(hd_driveid_t *ataid)
{
	...
	debug("(60)ataid->lba_capacity =0x%x\n", ataid->lba_capacity);
...
static int ata_scsiop_read_capacity10(ccb *pccb)
{
	...
	/* Swap 32bit for display - on memory this data is */
 	/* in little endian but this field in the buffer is in big endian format */
	*(u32 *) buf = __swab32(ataid[pccb->target]->lba_capacity);
