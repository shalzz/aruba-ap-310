...
#ifdef ARUBA_ARMV7
	/* Don't alert upper layer to less than 4 bit-flips */
	if (num_errors >= ECC_REQ_4BIT)
#else
	if (num_errors)
#endif
...
#ifdef CONFIG_APBOOT
static uint32_t calculate_bitflip_cnt(int data)
{
    uint32_t check_ruler;
    uint32_t i,cnt = 0;
		       ...
		       struct mtd_oob_ops *ops)
{
	...
	if (ops->datbuf != NULL) {
        ...
        if (ret < 0)
        {
            p = (uint32_t *)(ops->datbuf);
            for (i=0; i< cwl->data_size/4; i++)
            {
                if (p[i] != 0xffffffff)
                    bitflip_cnt += calculate_bitflip_cnt(p[i]);
                    
            }        
            if(ret == -EBADMSG && bitflip_cnt < 4) 
                debug("%s : %d bitflip found in bank page cw\n",__func__,bitflip_cnt);
            else
                return ret;
        }       
#else
