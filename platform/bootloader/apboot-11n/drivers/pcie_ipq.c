...
#if defined(CONFIG_MACH_IPQ806x) && defined(CONFIG_IPQ806X_PCI)
...
void pci_init_board (void)
{
	...
	for (i = 0; i < PCI_MAX_DEVICES; i++) {
		...
		if (cfg->linkup) {
			...
			/* skip bridge device */
			pci_hose[i].current_busno = pci_hose[i].first_busno + 1;
			pci_hose[i].last_busno = pci_hose_scan_bus(&pci_hose[i], pci_hose[i].current_busno);;
