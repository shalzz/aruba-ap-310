			  ...
			  struct pci_region *io)
{
	...
	for (bar = PCI_BASE_ADDRESS_0; bar <= PCI_BASE_ADDRESS_0 + (bars_num*4); bar += 4) {
		...
		if (!bar_response) {
#if 0
                    DEBUGF("PCI Autoconfig: Addr 0x%02x, response=0x%08x  ", bar, bar_response);
			DEBUGF("\n");
#endif
			continue;
		}
		...
		if (bar_response & PCI_BASE_ADDRESS_SPACE) {
			...
			DEBUGF(" BAR %d, I/O, size=0x%x, ", bar_nr, bar_size);
		} else {
			...
			DEBUGF(" BAR %d, Mem, size=0x%x, ", bar_nr, bar_size);
		}

		bar_allocated = 0;
		if ((bar_allocated = (pciauto_region_allocate(bar_res, bar_size, &bar_value) == 0))) {
