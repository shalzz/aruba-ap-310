...
int ag7240_s27_read(char *devname, unsigned char phy_addr, unsigned char reg, 
	unsigned short *value);
int ag7240_s27_write(char *devname, unsigned char phy_addr, unsigned char reg, 
	unsigned short data);
#endif
...
{
    u32 link=0, duplex=0, speed=0;
#if(CONFIG_COMMANDS & CFG_CMD_MII) && (defined(CONFIG_HAZELBURN))
    unsigned short val;
#endif
    ...
    switch (speed)
    {
...
#if(CONFIG_COMMANDS & CFG_CMD_MII) && (defined(CONFIG_HAZELBURN))
           /* control AR8035 LEDs */
           ag7240_miiphy_read("eth0", 4, 0x19, &val);
           val &= ~0xc0;
           ag7240_miiphy_write("eth0", 4, 0x19, val);

           /* add TX delay for AR8035 */
           ag7240_miiphy_write("eth0", 4, 0x1d, 0x5);
           ag7240_miiphy_read("eth0", 4, 0x1e, &val);
           val &= ~0x100;
           ag7240_miiphy_write("eth0", 4, 0x1e, val);
#endif
#if defined(CONFIG_HAZELBURN_H) || defined(CONFIG_HAZELBURN)
          if (is_wasp() && mac->mac_unit == 0) {
       ...
       case _100BASET:
           ...
           if ((is_ar7242() || is_wasp()) && (mac->mac_unit == 0) && !is_f2e())
               ...
               ar7240_reg_wr(AR7242_ETH_XMII_CONFIG,0xc000101);
#else
...
#ifdef CONFIG_DALMORE
           if ((is_ar7242() || is_wasp()) && (mac->mac_unit == 0) && !is_f2e())
               ar7240_reg_wr(AR7242_ETH_XMII_CONFIG,0xc001313);
#endif
...
/** 
 * @ provide 3 Sec delay for s17 link to settle
 * @ down. Suggested by systems team.
 */
...
int ag7240_enet_initialize(bd_t * bis)
{
	...
	/*
	 * don't register eth1, since we don't want to try to boot from it;
	 * we initialize it, since we want to use it for PHY initialization
	 */
	if (i == 0)
#endif
        eth_register(dev[i]);
#if(CONFIG_COMMANDS & CFG_CMD_MII) && !(defined(CONFIG_CARDHU))
...
#if(CONFIG_COMMANDS & CFG_CMD_MII) && (defined(CONFIG_CARDHU))
    /*
     * Atheros code seems to use eth1 for all accesses to the MDIO registers
     * but this seems to work for our purposes
     */
    miiphy_register("eth0", ag7240_s27_read, ag7240_s27_write);
#endif
...
extern void s27_wr_phy(unsigned int phy_addr, unsigned int reg_addr, 
	unsigned int write_data);

int
ag7240_s27_read(char *devname, unsigned char phy_addr, unsigned char reg, unsigned short *value)
{
	uint16_t val;
	if (phy_addr > 4) {
		*value = 0xffff;
	...
	val = s27_rd_phy(phy_addr, reg);
	*value = val;
	return 0;
}

int
ag7240_s27_write(char *devname, unsigned char phy_addr, unsigned char reg, unsigned short data)
{
	if (phy_addr > 4) {
		return 0;
	}
	s27_wr_phy(phy_addr, reg, data);
	return 0;
}
