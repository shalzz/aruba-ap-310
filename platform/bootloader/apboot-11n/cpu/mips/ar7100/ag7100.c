...
void pkt_dump(char *data,int pkt_size) {
    ...
    int i;
    printf("**********Packet dump******* %p size:%d\n", data, pkt_size);
    for(i=0;i<=pkt_size;i++) {
       printf("%x:", (unsigned)(data[i]&0xff));
       if(!(i%20))
          printf("\n");
    }
    printf("\n");
...
#if defined(CFG_ATHRS16_PHY) || defined(CONFIG_TALISKER) || defined(CONFIG_PB44) || defined(CONFIG_JURA_R) || defined(CONFIG_MSR2K)
...
#define ETH_PLL_CONFIG		0xb8050004
    ...
    if ((mac->link == olink) 
        && mac->link 
	&& (duplex == mac->duplex) && (speed == mac->speed))
        return 1;
...
#ifdef CONFIG_JURA_R
    udelay(10 * 1000);  /* give PHY more time to become ready */
#endif
