...
#if 1
#define BOOTP_DEBUG		1	/* general debug */
#endif
#if 1
...
#define debug_ext(fmt,args...)	if (dhcp_verbose()) printf (fmt ,##args)
#else
#define debug_ext(fmt,args...)
#endif
#ifdef BOOTP_DEBUG
#define debug_dhcp(fmt,args...)	if (dhcp_verbose()) printf (fmt ,##args)
#else
#define debug_dhcp(fmt,args...)
#endif
...
static inline int dhcp_verbose(void)
{
    DECLARE_GLOBAL_DATA_PTR;
    return (!!(gd->flags & GD_FLG_DHCP_VERBOSE));
}
...
static int BootpCheckPkt(uchar *pkt, unsigned dest, unsigned src, unsigned len)
{
	...
	if (memcmp(bp->bp_chaddr, NetOurEther, 6)) {
		retval = -7;
	}

	debug_dhcp ("Filtering pkt = %d\n", retval);
...
static void BootpVendorFieldProcess (u8 * ext)
{
	...
	case 40:		/* NIS Domain name                              */
        ...
        case 43:	/* Vendor specific info */
                if (size >= 7 && size <= 16) {
                    char str[16];
                    memcpy(str, ext+2, size);
                    str[size] = 0;
                    VenServerIP = string_to_ip(str);
                } else if (size == sizeof(VenServerIP)) {
                    memcpy(&VenServerIP, ext+2, oplen);
                } else {
		    VenServerIP = 0;
		}
		break;
		...
		classok = 0;
                if ((size >= strlen(CONFIG_BOOTP_VENDOR)) &&
                    !strncmp(ext+2, CONFIG_BOOTP_VENDOR, size)) {
                    classok = 1;
                }
                break;
#else
...
BootpHandler(uchar * pkt, unsigned dest, unsigned src, unsigned len)
{
	...
	debug_dhcp ("got BOOTP packet (src=%d, dst=%d, len=%d want_len=%d)\n",
...
#ifdef CONFIG_APBOOT
	else if (((s = getenv("autoload")) == 0) || BootFile[0] == '\0') {
		//
		// declare victory if 'autoload' is not set, or there is no
		// bootfile
		//
		NetState = NETLOOP_SUCCESS;
		return;
...
#ifdef CONFIG_APBOOT
	//
	// SCA; this is different than how idtboot had it,
	// but by sending the client ID, I get the same
	// IP address in the boot and in Linux, so I'm
	// doing it this way

	// client ID
	*e++ = 61;
	*e++ = 7;	// 6 for address + 1 for type
	*e++ = 1;	// type (Ethernet)
	memcpy (e, NetOurEther, 6);
	e += 6;
#endif
...
#ifdef CONFIG_APBOOT
    *e++ = 43;		/* Vendor encapsulated options */
    *cnt += 1;

    *e++ = 60;		/* Vendor class id */
    *cnt += 1;
#endif
...
static void DhcpOptionsProcess (uchar * popt, Bootp_t *bp)
{
	...
	while (popt < end && *popt != 0xff) {
		...
		debug_ext ("[DHCP] Processing extension %d... (%d bytes)\n", 
			*popt, oplen);
...
#ifdef CONFIG_APBOOT
		case 15:
			size = truncate_sz ("Domain Name",
					    sizeof(NetOurDomainName),
					    oplen);
			memcpy(&NetOurDomainName, popt+2, size);
			NetOurDomainName[size] = 0 ;
			break;
#else
		...
		case 43:	/* Vendor specific info */
		        if (oplen >= 7 && oplen <= 16) {
		            ...
		            str[oplen] = 0;
		            VenServerIP = string_to_ip(str);
		        } else if (oplen == sizeof(VenServerIP)) {
		            memcpy(&VenServerIP, popt+2, oplen);
		        } else {
			    VenServerIP = 0;
			}
		        break;
		...
		case 60:         /* Vendor class ID */
			classok = 0;
		        if ((oplen >= strlen(CONFIG_BOOTP_VENDOR)) &&
		             !strncmp((char *)(popt+2), CONFIG_BOOTP_VENDOR, oplen)) {
		          classok = 1;
		        }
		        break;
#endif
...
static void DhcpSendRequestPkt(Bootp_t *bp_offer)
{
	...
	debug_dhcp ("DhcpSendRequestPkt: Sending DHCPREQUEST\n");
	pkt = NetTxPacket;
	memset ((void*)pkt, 0, PKTSIZE);
	...
	debug_dhcp ("Transmitting DHCPREQUEST packet: len = %d\n", pktlen);
	NetSendPacket(NetTxPacket, pktlen);
}
...
DhcpHandler(uchar * pkt, unsigned dest, unsigned src, unsigned len)
{
	...
	debug_dhcp ("DHCPHandler: got packet: (src=%d, dst=%d, len=%d) state: %d\n",
...
#ifndef CONFIG_APBOOT
	debug_dhcp ("DHCPHandler: got DHCP packet: (src=%d, dst=%d, len=%d) state: %d\n",
		src, dest, len, dhcp_state);
#endif
	...
	case SELECTING:
		 ...
		 */
		debug_dhcp ("DHCP: state=SELECTING bp_file: \"%s\"\n", bp->bp_file);
		...
		if (strncmp(bp->bp_file,
			...
			debug_dhcp ("TRANSITIONING TO REQUESTING STATE\n");
...
#ifndef CONFIG_APBOOT
			// SCA; should not use address until ACK received
		...
		break;
	case REQUESTING:
		debug_dhcp ("DHCP State: REQUESTING\n");
		...
		if ( DhcpMessageType((u8 *)bp->bp_vend) == DHCP_ACK ) {
			...
			dhcp_state = BOUND;
#ifdef CONFIG_APBOOT
			printf("DHCP IP address: ");
			print_IPaddr(NetOurIP);
			printf("\n");
			printf("DHCP subnet mask: ");
			print_IPaddr(NetOurSubnetMask);
			printf("\n");
			printf("DHCP def gateway: ");
			print_IPaddr(NetOurGatewayIP);
			printf("\n");
			printf("DHCP DNS server: ");
			print_IPaddr(NetOurDNSIP);
			printf("\n");
			printf("DHCP DNS domain: %s\n",
			       NetOurDomainName);
			/* 1. ARP for DNS server or gateway
			 * 2. Issue DNS request for master mswitch
			 * 3. ARP for master mswitch
			 * 4. TFTP filename
			 */
                        if (VenServerIP && classok) {
                            printf("Controller address: ");
                            print_IPaddr(VenServerIP);
                            printf("\n");
                            NetServerIP = VenServerIP;
                        }
           
	                //
			// SCA; removed check for DNS server so that ADP will
			// kick in
			//
                        if (!NetOurIP) {
                            puts("** ERROR: Missing DHCP address(es), OurDNSIP = "); 
                            print_IPaddr(NetOurDNSIP);
			...
			if (BootFile[0] == '\0') {
				if (!(s = getenv ("bootfile"))) {
				    puts ("*** ERROR: `bootfile' not set\n");
				    NetState = NETLOOP_FAIL;
				    return;
				}
				strcpy(BootFile, s);
			}
                        if (NetServerIP || getenv("serverip")) {
                            if (getenv("serverip")) {
...
//                            ArpRequest();
			    ...
			    //
			    // not really, but we don't want to try to boot if
			    // we're just getting a DHCP address
			    //
			    NetState = NETLOOP_SUCCESS;
			}
			return;
...
{
	DECLARE_GLOBAL_DATA_PTR;
	if ((getenv("dhcp_verbose") != NULL))
	{
	    gd->flags |= GD_FLG_DHCP_VERBOSE;
	}
	BootpRequest();
}
#endif	/* CFG_CMD_DHCP */
