...
#include <command.h>
#include <net.h>
#if defined(CONFIG_RMI_CPU)
#include <packet.h> // nebbiolo
#endif
#include "bootp.h"
#include "tftp.h"
#include "rarp.h"
...
#define ARP_TIMEOUT		5ULL		/* Seconds before trying ARP again */
...
#endif
#else
#define ARP_TIMEOUT		2ULL		/* Seconds before trying ARP again */
#ifndef	CONFIG_NET_RETRY_COUNT
...
char            NetServerName[64]={0,};
char 		NetOurDomainName[255]={0,};
...
volatile uchar	PktBuf[(PKTBUFSRX+1) * PKTSIZE_ALIGN + PKTALIGN] __attribute__ ((__section__(".packets")));
volatile uchar *NetRxPackets[PKTBUFSRX]; /* Receive packets			*/
//volatile uchar *NetRxPackets[PKTBUFSRX] __attribute__ ((__section__(".packets"))); /* Receive packets			*/
#else
...
int net_check_prereq (proto_t protocol);
...
//	(void) eth_send (pkt_start, (pkt - pkt_start) + ARP_HDR_SIZE);
...
//	ulong t;
	...
	if ((t - NetArpWaitTimerStart) > ((uint64_t) (ARP_TIMEOUT * CFG_HZ))) {
		NetArpWaitTry++;
...
restart:
	...
	case TFTP:
		//NetCopyIP(&NetOurIP, &bd->bi_ip_addr);
   		//copy_filename (BootFile, getenv("bootfile"), sizeof(BootFile));
	...
	case CDP:
		...
		NetOurNativeVLAN = getenv_VLAN("nvlan");
		break;
#if 0 // (CONFIG_COMMANDS & CFG_CMD_RCVR)
	case RCVR:
		break;
#endif
	default:
...
void NetStartAgain (void)
{
	...
	if (NetRestartWrap) {
		...
		if (NetDevExists && !once) {
			NetSetTimeout ((uint64_t)10 * (uint64_t)CFG_HZ, startAgainTimeout);
			NetSetHandler (startAgainHandler);
		} else {
			NetState = NETLOOP_FAIL;
		}
...
NetSendUDPPacket(uchar *ether, IPaddr_t dest, int dport, int sport, int len)
{
    ...
    for (retry_cnt = 0; retry_cnt < 10; retry_cnt++) {
	    if (eth_send(NetTxPacket, (pkt - NetTxPacket) + IP_HDR_SIZE + len) == 0) {
            break;
        }
    }
#endif
...
#endif	/* CONFIG_NET_MULTI */
	NetSetTimeout ((uint64_t)10 * (uint64_t)CFG_HZ, PingTimeout);
...
static void
handle_redirect(IPaddr_t addr, IPaddr_t src)
{
	...
	if (src != NetOurGatewayIP) {
		printf("; ignored due to incorrect source address\n");
		return;
	}
	if (NetOurSubnetMask && NetOurIP) {
		if ((addr & NetOurSubnetMask) != (NetOurIP & NetOurSubnetMask)) {
			printf("; ignored due to prefix mismatch\n");
			return;
		}
	}
	printf("\n");
	NetOurGatewayIP = addr;
...
NetReceive(volatile uchar * inpkt, int len)
{
	...
	case PROT_IP:
		...
		if (ip->ip_p == IPPROTO_ICMP) {
			...
			switch (icmph->type) {
			case ICMP_REDIRECT:
			/* APBoot; accept all redirects per host requirements */
#ifndef CONFIG_APBOOT
				if (icmph->code != ICMP_REDIR_HOST)
					return;
#endif
				NetCopyIP(&src, &ip->ip_src);
				NetCopyIP(&addr, &icmph->un.gateway);
				handle_redirect(addr, src);
				return;
#if (CONFIG_COMMANDS & CFG_CMD_PING)
...
#if defined(CONFIG_UDP_CHECKSUM) || defined(CONFIG_APBOOT)
		if (ip->udp_xsum != 0) {
			ulong   xsum;
			ushort *sumptr;
			ushort  sumlen;
...
int net_check_prereq (proto_t protocol)
{
	...
	case DNS:
		if (NetOurDNSIP == 0) {
			puts ("*** ERROR: `dnsip' not set\n");
			return (1);
		}
		if (*NetServerName == 0) {
			puts ("*** ERROR: `servername' not set\n");
			return (1);
		}
		goto dns;
		/* Fall through */
...
uint8_t *ether_aton(char *addr_string, uint8_t *addr_bytes)
{
  size_t cnt;
