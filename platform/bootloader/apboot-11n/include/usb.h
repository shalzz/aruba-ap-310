...
struct usb_device {
    ...
    int request_len;	    /* Actual Request size*/
#endif
	struct usb_device *parent;
	struct usb_device *children[USB_MAXCHILDREN];
#ifdef CONFIG_GRENACHE
    struct ehci_qh *qh[32];
    /*Index this using below bit map:
      0   - Direction - 0 - In and 1 - Out 
      1-4 - Endpoint No.          
     */
    int is_present; /*
                     ...
                     */
    int port_no; /*0 - Connected to port zero;
                1 - Connected to port one;
               */
#endif    
};
