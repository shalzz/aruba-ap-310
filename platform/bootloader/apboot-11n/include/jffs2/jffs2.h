...
/* Added this for Grenache - but it is quietly ignored by the JFFS2 NAND code for now */
...
{
	__u16 magic;
	__u16 nodetype; /* == JFFS2_NODETYPE_ERASEBLOCK_HEADER */
	__u32 totlen;
	__u32 hdr_crc;
	__u32 node_crc;
	__u8  reserved; /* reserved for future use and alignment */
	__u8  compat_fset;
	__u8  incompat_fset;
	__u8  rocompat_fset;
	__u32 erase_count; /* the erase count of this erase block */
	__u32 data[0];
} __attribute__((packed));
