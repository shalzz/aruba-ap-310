...
extern int eth_unregister(struct eth_device *dev);
...
extern char            NetOurBootCommand[256];  /* Our boot command             */
...
extern char             NetServerName[64];
extern char             NetOurDomainName[255];
...
typedef enum { BOOTP, RARP, ARP, TFTP, DHCP, PING, DNS, NFS, CDP, NETCONS, SNTP, ADP } proto_t;
...
static inline void NetPutShort(ushort *p, ushort val)
{
	uchar *cp = (uchar *)p;
	*cp = ((val >> 8) & 0xff);
	cp++;
...
char *ether_ntoa(uint8_t *addr_bytes, char *addr_string);
